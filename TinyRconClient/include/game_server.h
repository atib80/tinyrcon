...
class game_server
{
  public:
    ...
    //	for (auto& pd : this->previous_players_data) {
    //		if (pid == pd.pid)
    //			return pd;
    //	}
    ...
    bool remove_ip_address_from_temp_banned_ip_addresses(const string &new_ip_address)
    {
        ...
        temp_banned_ip_addresses_vector.erase(
            find_if(cbegin(temp_banned_ip_addresses_vector), cend(temp_banned_ip_addresses_vector),
                    [&new_ip_address](const player &pd) { return new_ip_address == pd.ip_address; }),
    ...
    bool remove_ip_address_from_banned_ip_addresses(const string &new_ip_address)
    {
        ...
        banned_ip_addresses_vector.erase(
            find_if(cbegin(banned_ip_addresses_vector), cend(banned_ip_addresses_vector),
                    [&new_ip_address](const player &pd) { return new_ip_address == pd.ip_address; }),
    ...
    bool remove_ip_address_range_from_banned_ip_address_ranges(const string &new_ip_address_range)
    {
        ...
        banned_ip_address_ranges_vector.erase(
            find_if(cbegin(banned_ip_address_ranges_vector), cend(banned_ip_address_ranges_vector),
                    [&new_ip_address_range](const player &pd) { return new_ip_address_range == pd.ip_address; }),
  ...
  private:
    ...
    string rcon_password{"abc123"};
    string private_slot_password{"abc123"};
