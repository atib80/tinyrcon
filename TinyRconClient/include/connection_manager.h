...
class connection_manager
{
  ...
  public:
    ...
    void prepare_rcon_command(char *send_buffer, const std::size_t send_buffer_capacity, const char *command_to_send,
    ...
    size_t send_rcon_command(const std::string &send_buffer, const char *remote_ip,
                             const uint_least16_t remote_port) const;

    size_t send_non_rcon_command(const std::string &send_buffer, const char *remote_ip,
                                 const uint_least16_t remote_port) const;

    size_t receive_rcon_reply_from_server(const char *remote_ip, const uint_least16_t remote_port, game_server &gs,
                                          std::string &reply_buffer, const bool is_process_reply = true) const;

    size_t process_rcon_status_messages(game_server &gs, const std::string &rcon_status_data) const;

    size_t receive_non_rcon_reply_from_server(const char *remote_ip, const uint_least16_t remote_port, game_server &gs,
                                              std::string &reply_buffer, const bool is_process_reply = true) const;
    ...
    void send_and_receive_non_rcon_data(const char *command_to_send, std::string &reply_buffer, const char *remote_ip,
                                        const uint_least16_t remote_port, game_server &gs,
                                        const bool is_wait_for_reply = true, const bool is_process_reply = true) const;

    inline std::vector<geoip_data> &get_geoip_data()
    {
        return geoip_db;
    }
