...
class bitmap_image_handler : public disabled_copy_operations, public disabled_move_operations
{
  ...
  public:
    ...
    {
        if ((!bitmap_images_.contains(bitmap_image_name) || bitmap_images_.at(bitmap_image_name) == NULL) &&
            is_load_if_not_in_cache)
        {
            // const std::string bitmap_file_path{ format("{}\\{}.bmp",
            // bitmap_images_folder_path_, bitmap_image_name) };
        ...
        return bitmap_images_.contains(bitmap_image_name)
                   ? bitmap_images_.at(bitmap_image_name)
                   : (bitmap_images_.contains("nomap") ? bitmap_images_.at("nomap") : NULL);
    }
    ...
    {
        // const string bitmap_images_folder_path{ format("{}data\\images\\maps",
        // main_app_->get_current_working_directory()) };
        ...
        if (read_file_data_handle != INVALID_HANDLE_VALUE)
        {
            do
            {
                ...
                if (image_name.ends_with(".bmp"))
                {
                    ...
                    bitmap_images_[image_name] =
                        (HBITMAP)LoadImageA(NULL, bmp_image_file_path.c_str(), IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
                }
    ...
    bool load_bitmap_image(const std::string &image_name)
    {
        ...
        {
            if (!download_bitmap_image_file(image_name.c_str(), bitmap_file_path.c_str()) ||
                !check_if_file_path_exists(bitmap_file_path.c_str()))
                return false;
        }
        ...
        {
            bitmap_images_[image_name] =
                (HBITMAP)LoadImageA(NULL, bitmap_file_path.c_str(), IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
        }
