...
public:
	...
	HBITMAP get_bitmap_image(const std::string& bitmap_image_name, const bool is_load_if_not_in_cache = true)
	{
		if ((!bitmap_images_.contains(bitmap_image_name) || bitmap_images_.at(bitmap_image_name) == NULL) && is_load_if_not_in_cache) {
			...
			if (!load_bitmap_image(bitmap_image_name) || !bitmap_images_.contains(bitmap_image_name)) return NULL;
	...
	bool load_bitmap_image(const std::string& image_name)
	{
		...
		if (!check_if_file_path_exists(bitmap_file_path.c_str())) return false;
		const std::filesystem::path image_file_entry{ bitmap_file_path };
		if (image_file_entry.has_filename() && image_file_entry.has_extension() && stl::helper::str_ends_with(image_file_entry.extension().string(), "bmp", true)) {
