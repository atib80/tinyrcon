...
class tiny_rcon_client_application : public disabled_copy_operations, public disabled_move_operations
{
	...
	// std::string images_data_file_pattern{ LR"(^images_data-[0-9a-fA-F]{32}\.7z$)" };
...
public:
	...
	void set_current_working_directory()
	{
		...
		if (const auto path_len = GetModuleFileName(nullptr, exe_file_path.get(), max_path_limit); path_len != 0)
		{
			...
			current_working_directory.assign(std::string(cbegin(exe_file_path_sv), cbegin(exe_file_path_sv) + exe_file_path_sv.rfind('\\') + 1));
		}
		else
		{
	...
	const std::unordered_map<std::string, std::function<void(const std::string &, const time_t, const std::string &, const bool)>> &get_remote_message_handlers() const noexcept
	{
	...
	const std::function<void(const std::string &, const time_t, const std::string &, const bool)> &get_remote_message_handler(const std::string &remote_message_name) const noexcept
	{
