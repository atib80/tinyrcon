    ...
    try
    {
        ...
        if (lpFfi && uLen)
        {
            ...
            /*if (is_print_information) {
              snprintf(message_buffer, std::size(message_buffer), "^2Current version
            of ^5Tiny^6Rcon ^2is ^5%d.%d.%d.%d\n", ver.major, ver.minor, ver.revision,
            ver.sub_revision); print_colored_text(app_handles.hwnd_re_messages_data,
            message_buffer, is_append_message_to_richedit_control::yes,
            is_log_message::yes, is_log_datetime::yes);
              main_app.get_connection_manager_for_messages().process_and_send_message("tinyrcon-info",
            format("{}\\{}\\{}", main_app.get_username(),
            main_app.get_user_ip_address(), message_buffer), true,
...
vector<string> auto_update_manager::get_file_name_matches_for_specified_file_pattern(
    internet_handle &internet_connect_handle, const char *relative_path, const char *file_pattern) const
{
    ...
    std::snprintf(download_file_path_pattern, std::size(download_file_path_pattern), "%s%s", relative_path,
                  file_pattern);
...
void auto_update_manager::check_for_updates()
{
    ...
    if (nullptr != threadParam.internet_open_handle.get())
    {
        ...
        if (nullptr != threadParam.internet_connect_handle.get())
        {
            ...
            auto available_file_names = get_file_name_matches_for_specified_file_pattern(
            ...
            available_file_names = get_file_name_matches_for_specified_file_pattern(
                ...
                if (current_geo_dat_file_md5 != new_geo_dat_file_md5)
                {
                    ...
                    if (!check_if_file_path_exists(seven_zip_dll_file_path.c_str()))
                    {
                        ...
                        if (!available_file_names.empty())
                        {
                            ...
                            snprintf(download_url_buffer, 256, "ftp://%s/%s/7za.dll",
                    ...
                    // plugins/geoIP/geo.dat
                    const string old_geo_dat_file_parent_folder_path{self_current_working_directory +
                                                                     "plugins\\geoIP\\"};
                    ...
                    Sleep(20);
                    char download_url_buffer[256];
                    snprintf(download_url_buffer, 256, "ftp://%s/%s/%s",
                             main_app.get_ftp_download_site_ip_address().c_str(),
...
void auto_update_manager::downloaded_latest_version_of_program() const
{
        ...
        next_version_number > current_version_number)
    {
        ...
        char download_url_buffer[256];
        snprintf(download_url_buffer, 256, "ftp://%s/%s/%s", main_app.get_ftp_download_site_ip_address().c_str(),
                 main_app.get_ftp_download_folder_path().c_str(), latest_version_to_download.c_str());
...
DWORD WINAPI worker_function1(void *param)
{
    ...
    if (pThreadParm->internet_open_handle.get() != nullptr)
    {
        ...
        pThreadParm->internet_connect_handle.set(InternetConnectA(
            pThreadParm->internet_open_handle.get(), main_app.get_ftp_download_site_ip_address().c_str(),
            INTERNET_DEFAULT_FTP_PORT, nullptr, nullptr, INTERNET_SERVICE_FTP, INTERNET_FLAG_PASSIVE, 0));
