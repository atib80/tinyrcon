...
void rename_myself(const char *src_file_path, const char *dst_file_path)
{
    ...
    CreateProcess(nullptr, command_to_run, nullptr, nullptr, FALSE, CREATE_NO_WINDOW, nullptr, nullptr,
                  &process_startup_info, &pr_info);
...
bool auto_update_manager::get_file_version(const string &exe_file, version_data &ver,
                                           unsigned long &version_number /*, const bool is_print_information*/) const
{
...
vector<string> auto_update_manager::get_file_name_matches_for_specified_ftp_path_and_file_pattern(
    internet_handle &internet_connect_handle, const char *ftp_path, const char *file_pattern) const
{
    ...
    internet_handle read_file_data_handle{FtpFindFirstFileA(internet_connect_handle.get(), download_file_path_pattern,
                                                            &file_data, INTERNET_FLAG_NEED_FILE, INTERNET_NO_CALLBACK)};
...
// vector<wstring>
// auto_update_manager::get_file_name_matches_for_specified_ftp_path_and_file_pattern(
...
//
//	swprintf_s(download_file_path_pattern,
// std::size(download_file_path_pattern), L"%s%s", ftp_path, file_pattern);
//	WIN32_FIND_DATA file_data{};
//	internet_handle read_file_data_handle{
// FtpFindFirstFile(internet_connect_handle.get(), download_file_path_pattern,
//&file_data, INTERNET_FLAG_NEED_FILE, INTERNET_NO_CALLBACK) };
//
//	if (read_file_data_handle.get() != nullptr) {
//		while (stl::helper::len(file_data.cFileName) > 0) {
...
//			Sleep(20);
//			ZeroMemory(&file_data, sizeof(WIN32_FIND_DATA));
//			InternetFindNextFile(read_file_data_handle.get(),
//&file_data);
//		}
//	}
//
...
void auto_update_manager::check_for_updates(const char *exe_file_path)
{
    ...
    const string ftp_download_site_info{"ftp://"s + main_app.get_ftp_download_site_ip_address() +
                                        (!main_app.get_ftp_download_folder_path().empty()
                                             ? "/"s + main_app.get_ftp_download_folder_path() + "/"s
                                             : "/"s)};

    std::snprintf(message_buffer, std::size(message_buffer), "^3Searching for updates at ^1%s\n",
                  ftp_download_site_info.c_str());
    print_colored_text(app_handles.hwnd_re_messages_data, message_buffer, is_append_message_to_richedit_control::yes,
    ...
    if (nullptr != threadParam.internet_open_handle.get())
    {
        ...
        if (nullptr != threadParam.internet_connect_handle.get())
        {
            ...
            auto available_file_names = get_file_name_matches_for_specified_ftp_path_and_file_pattern(
                threadParam.internet_connect_handle, ftp_download_folder_path.c_str(), "_U_TinyRcon*.exe");
            ...
            {
                snprintf(message_buffer, std::size(message_buffer),
                         "^2There is no newer version of ^5Tiny^6Rcon ^2at ^5%s\n", ftp_download_site_info.c_str());
                print_colored_text(app_handles.hwnd_re_messages_data, message_buffer,
                                   is_append_message_to_richedit_control::yes, is_log_message::yes,
                                   is_log_datetime::yes);
            }
            ...
            if (!available_file_names.empty())
            {
                ...
                const string new_images_file_md5{
                    string(new_images_file_name.cbegin() + start_pos, new_images_file_name.cbegin() + end_pos)};
                ...
                if (main_app.get_images_data_md5() != new_images_file_md5)
                {
                    ...
                    if (!check_if_file_path_exists(seven_zip_dll_file_path.c_str()))
                    {
                        ...
                        if (!available_file_names.empty())
                        {
                            ...
                            snprintf(download_url_buffer, std::size(download_url_buffer), "ftp://%s/%s/7za.dll",
                                     main_app.get_ftp_download_site_ip_address().c_str(),
                                     main_app.get_ftp_download_folder_path().c_str());
                            snprintf(message_buffer, std::size(message_buffer),
                            ...
                            {
                                snprintf(message_buffer, std::size(message_buffer),
                                         "^2Successfully downloaded ^57za.dll ^2from ^5%s\n",
                                         ftp_download_site_info.c_str());
                                print_colored_text(app_handles.hwnd_re_messages_data, message_buffer,
                                                   is_append_message_to_richedit_control::yes, is_log_message::yes,
                                                   is_log_datetime::yes);
                            ...
                            {
                                snprintf(message_buffer, std::size(message_buffer),
                                         "^3Failed to download ^57za.dll ^2from ^5%s\n",
                                         ftp_download_site_info.c_str());
                                print_colored_text(app_handles.hwnd_re_messages_data, message_buffer,
                                                   is_append_message_to_richedit_control::yes, is_log_message::yes,
                                                   is_log_datetime::yes);
                    ...
                    const string new_updated_images_data_7zip_file_path{self_current_working_directory +
                                                                        R"(data\images\maps\)" + new_images_file_name};
                    ...
                    Sleep(20);
                    char download_url_buffer[512]{};
                    snprintf(download_url_buffer, std::size(download_url_buffer), "ftp://%s/%s/%s",
                             main_app.get_ftp_download_site_ip_address().c_str(),
                             main_app.get_ftp_download_folder_path().c_str(), new_images_file_name.c_str());
                    snprintf(message_buffer, std::size(message_buffer),
                             "^2Downloading updated compressed ^5%s ^2images data file\n from "
                             "^5%s...\n",
                             new_images_file_name.c_str(), ftp_download_site_info.c_str());
                    print_colored_text(app_handles.hwnd_re_messages_data, message_buffer,
                                       is_append_message_to_richedit_control::yes, is_log_message::yes,
                                       is_log_datetime::yes);
                    ...
                    if (download_file(download_url_buffer, new_updated_images_data_7zip_file_path.c_str()))
                    {
                        ...
                        // wstring
                        // dir_path_to_images_data_maps{format(L"{}data\\images\\maps\\",
                        // main_app.get_current_working_directory())
                    };
                    auto [status, message] = extract_7z_file_to_specified_path(
                        new_updated_images_data_7zip_file_path.c_str(), data_images_maps_folder_path.c_str());
                    if (status)
                    {
                        DeleteFile(new_updated_images_data_7zip_file_path.c_str());
                        snprintf(message_buffer, std::size(message_buffer),
                    ...
                    else
                    {
                        snprintf(message_buffer, std::size(message_buffer),
                                 ...
                                 new_images_file_name.c_str(), message.c_str());
                        print_colored_text(app_handles.hwnd_re_messages_data, message_buffer,
                                           is_append_message_to_richedit_control::yes, is_log_message::yes,
                                           is_log_datetime::yes);
                    }
                ...
                message_buffer, is_append_message_to_richedit_control::yes,
                is_log_message::yes, is_log_datetime::yes);
                }*/
            }
            ...
            if (!available_file_names.empty())
            {
                ...
                if (current_geo_dat_file_md5 != new_geo_dat_file_md5)
                {
                    ...
                    const string new_geo_dat_7zip_file_path{self_current_working_directory + R"(plugins\geoIP\)" +
                                                            new_geo_dat_file_name};
                    ...
                    snprintf(download_url_buffer, std::size(download_url_buffer), "ftp://%s/%s/%s",
                             main_app.get_ftp_download_site_ip_address().c_str(),
                             main_app.get_ftp_download_folder_path().c_str(), new_geo_dat_file_name.c_str());
                    snprintf(message_buffer, std::size(message_buffer),
...
{
    if (!is_current_instance_temporary_version && !latest_version_to_download.empty() &&
        next_version_number > current_version_number)
    {
        const string exe_file_name{self_current_working_directory + "_U_"s + self_file_name};
        char download_url_buffer[512];
        snprintf(download_url_buffer, std::size(download_url_buffer), "ftp://%s/%s/%s",
                 main_app.get_ftp_download_site_ip_address().c_str(), main_app.get_ftp_download_folder_path().c_str(),
                 latest_version_to_download.c_str());
        DeleteFile(exe_file_name.c_str());
        if (download_file(download_url_buffer, exe_file_name.c_str()))
        {
            snprintf(message_buffer, std::size(message_buffer),
                     "^2Downloaded latest version of ^5Tiny^6Rcon^2: ^1v%s\n", next_version_number_str.c_str());
            print_colored_text(app_handles.hwnd_re_messages_data, message_buffer,
                               is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
...
DWORD WINAPI worker_function1(void *param)
{
    ...
    if (pThreadParm->internet_open_handle.get() != nullptr)
    {
        ...
        InternetSetOptionA(pThreadParm->internet_open_handle.get(), INTERNET_OPTION_CONNECT_TIMEOUT,
                           &internet_option_connect_timeout, sizeof(internet_option_connect_timeout));
        const string ftp_site_url{main_app.get_ftp_download_site_ip_address()};
        pThreadParm->internet_connect_handle.set(
            InternetConnectA(pThreadParm->internet_open_handle.get(), ftp_site_url.c_str(), INTERNET_DEFAULT_FTP_PORT,
                             nullptr, nullptr, INTERNET_SERVICE_FTP, INTERNET_FLAG_PASSIVE, 0));
...
DWORD WINAPI worker_function3(void *param)
{
    ...
    if (pThreadParm->internet_open_handle.get() != nullptr)
    {
        ...
        pThreadParm->internet_connect_handle.set(InternetConnectA(
            pThreadParm->internet_open_handle.get(), pThreadParm->ftp_host, INTERNET_DEFAULT_FTP_PORT,
            pThreadParm->user_name, pThreadParm->user_pass, INTERNET_SERVICE_FTP, INTERNET_FLAG_PASSIVE, 0));

        if (!pThreadParm->internet_connect_handle.get())
            return 1;
        if (TRUE == FtpPutFileA(pThreadParm->internet_connect_handle.get(), pThreadParm->upload_file_path,
                                pThreadParm->ftp_file_path, FTP_TRANSFER_TYPE_BINARY, 0))
            return 0;
        return 1;
    }
...
DWORD WINAPI worker_function4(void *param)
{
    ...
    if (pThreadParm->internet_open_handle.get() != nullptr)
    {
        ...
        if (!pThreadParm->internet_connect_handle.get())
            return 1;
        if (TRUE == FtpGetFileA(pThreadParm->internet_connect_handle.get(), pThreadParm->ftp_file_path,
                                pThreadParm->download_file_path, FALSE, FILE_ATTRIBUTE_NORMAL, FTP_TRANSFER_TYPE_BINARY,
                                0))
        {
            Sleep(100);
