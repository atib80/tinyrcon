...
size_t connection_manager::send_non_rcon_command(const string &outgoing_data, const char *remote_ip, const uint_least16_t remote_port) const
{
...
void connection_manager::prepare_rcon_command(char *buffer, const size_t buffer_size, const char *rcon_command_to_send, const char *rcon_password) const
{
	(void)snprintf(buffer, buffer_size, "\xFF\xFF\xFF\xFFrcon %s %s", rcon_password, rcon_command_to_send);
}

size_t connection_manager::send_rcon_command(const string &outgoing_data, const char *remote_ip, const uint_least16_t remote_port) const
{
	const ip::udp::endpoint dst{ip::address::from_string(remote_ip), remote_port};
	const size_t sent_bytes =
...
size_t connection_manager::receive_rcon_reply_from_server(const char *remote_ip, const uint_least16_t remote_port, game_server &gs, std::string &received_reply, const bool) const
{
	char incoming_data_buffer[receive_buffer_size];
	size_t noOfReceivedBytes{}, noOfAllReceivedBytes{};
	...
	if (noOfAllReceivedBytes > 0)
	{
		...
		else
		{
			...
			if (received_reply.find(rcon_status_response_needle1) != string::npos || (rcon_status_response_needle2 != nullptr && received_reply.find(rcon_status_response_needle2) != string::npos))
			{
				...
				if (!is_server_empty_or_error_receiving_udp_datagrams)
				{
					...
					{
						if (!regex_search(player_line, player_ip_address_regex) && !regex_search(player_line, bot_ip_address_regex))
							continue;
						int j{}, i{3};
						int digit_count{};
						int player_pid{};
			...
			else if (received_reply.find("map_rotate...\n\n") != string::npos)
			{
				...
				print_colored_text(app_handles.hwnd_re_messages_data, received_reply.c_str());
					 ...
					 (start_pos = received_reply.find(R"("sv_mapRotationCurrent" is: ")")) != string::npos && received_reply.find(R"(default: ")") != string::npos)
			{
				main_app.get_current_game_server().set_is_connection_settings_valid(true);
				...
				if (last_pos != string::npos)
				{
					...
					if (strstr(current, "gametype") != nullptr)
					{
					 ...
					 (first_pos1 = received_reply.find(R"("sv_mapRotation" is: ")")) != string::npos && received_reply.find(R"(default: ")") != string::npos)
			{
				main_app.get_current_game_server().set_is_connection_settings_valid(true);
...
void connection_manager::send_and_receive_rcon_data(const char *command_to_send, std::string &received_reply, const char *remote_ip, const uint_least16_t remote_port, const char *rcon_password, game_server &gs, const bool is_wait_for_reply, const bool is_process_reply) const
{
