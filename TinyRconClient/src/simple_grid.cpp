...
///         hosted on Codeguru and titled: "Win32 Grid Control with Low Overhead
///         (BABYGRID)." Over the course of rewriting the grid into something
///         more to my liking; the code went through many modifications but some
///         pieces of the original may remain.  Mr. Hillard's code provided a
///         great starting point for this project.
///
/// @par Disclaimer:
...
// DWM 1.1: Removed unused field "editstring" from INSTANCEDATA and all
// references to it. DWM 1.2: Fixed potential bug condition under x64 DWM 1.3:
// Rewrote Grid_OnResetContent() and removed unused method Vector_Clear().
// DWM 1.4: Added code to clean up any active in-cell editor to the
// Grid_OnResetContent() method. DWM 1.5: (1) Added message handler for new
// message SG_SELECTCELL
//          (2) Redefined message handler for SG_SETCURSORPOS so that the WPARAM
//          is treated
//              as the column argument and the LPARAM is treated as the row
//              argument in order that they might be consistent with the rest of
//              the messages.
//          (3) Changed the behavior of the grid so that it doesn't initiate a
//          cell edit if
//              just any key is pressed.  Now if the windows key is pressed the
//              selected cell will not go into edit mode when the windows menu
//              launches.
//          (4) Fixed the Factory method New_SimpleGrid() so that it should work
//          if it is
//              compiled into a DLL.
//          (5) Fixed bug where mousing away from a cell during edit resulted in
//          the cell not
//              being updated.
// DWM 1.6: Changes labled thus.
// DWM 1.7: Changes labled thus.
// DWM 1.8: Refactored Notify calls from FORWARD_WM_NOTIFY(GetParent(hwnd)...,
// to FORWARD_WM_NOTIFY(g_lpInst->hwndParent...,
//           on principle.
// DWM 1.9: Changes labled thus.
//          (1) Fixed bug where edited text would be reset after resizing a
//          column. (2) Refactored portions to make source easier to port to
//          MSVC++. (3) Editor and button fonts now set to parent grid font.
// DWM 2.0: Changes labeled thus.
//          (1) Fixed bug introduced in 1.9 where button not destroyed  when
//          resizing a button column.
// DWM 2.1: Changes labled thus.
//          (1) Added support for new message SG_INSERTROW provided by Ferenc
//          Hechler (2) Added support for new message SG_DELETEROW
...
#define WPRC "Wprc"         ///< Generic property tag
#define EDITMODE "EDITMODE" ///< Edit mode property tag

#define CHECKED "T"   ///< GCT_CHECK checked
...
#define String_Replace(lpszTarget, lpszReplace) (free((lpszTarget)), (lpszTarget) = NewString(lpszReplace))
...
#define StringArray_Replace(lpszTarget, lpszReplace) (free((lpszTarget)), (lpszTarget) = NewStringArray(lpszReplace))
...
//   char *tbuffer = (char *)_alloca(sizeof(char) * (k + 1));// use _alloca
//   instead BOOL DIGIT, ALPHA, WHITESPACE; strcpy_s(tbuffer, k + 1, data); k =
//   len(tbuffer); to_upper_case_in_place(tbuffer); DIGIT = FALSE; ALPHA =
...
static BOOL Vector_Insert(const LPVECTOR pVector, int nPos,
                          PVOID object) // DWM 2.1: Added
{
    ...
    {
        new_capacity = pVector->_current_capacity + VECTOR_CAPACITY_DELTA;
        new_data = (PVOID *)checked_malloc(sizeof(PVOID) * new_capacity);
...
typedef struct tagINSTANCEDATA
{
    ...
    DWORD selectionMode;       ///< The Selection mode (full row, cell, or row header
                               ///< click)
    ...
    BOOL REMEMBERINTEGRALROWS;  ///< Remember the integral rows flag during
                                ///< resizing
    ...
    INT columntoresizeinitsize; ///< Field to hold the initial size of a column
                                ///< to resize
    INT columntoresizeinitx;    ///< Field to hold the x coord of the mouse button
                                ///< during resize
...
static char *NewString(const char *str)
{
    if (nullptr == str)
        str = "";
    char *tmp = (char *)calloc(len(str) + 1, sizeof(char));

    if (nullptr == tmp)
    {
        return (char *)calloc(1, sizeof(char));
    }
    return (char *)memmove(tmp, str, len(str));
}
...
static char *NewStringArray(const char *szzStr)
{
    if (nullptr == szzStr)
        szzStr = "";
    ...
    for (const char *p = szzStr; *p; (p += len(p) + 1, iLen = p - szzStr))
        ;

    // Allocate for array
    char *tmp = (char *)calloc(iLen + 1, sizeof(char));
...
static LPGRIDITEM New_Item(const char *szCurValue)
{
...
///
/// @returns The row number of the mouse pointer if successful, otherwise
/// SG_ERROR
...
///
/// @returns The col number of the mouse pointer if successful, otherwise
/// SG_ERROR
...
///
/// @returns VOID
static VOID DisplayTitle(HWND, HFONT hfont, HDC hdc, RECT rc)
{
    HFONT hOldfont;
    ...
    // DrawTextEx(hdc, g_lpInst->title, -1, &rc, DT_END_ELLIPSIS | DT_CENTER |
    // DT_WORDBREAK | DT_NOPREFIX, NULL);
    print_colored_text_to_grid_cell(hdc, rc, g_lpInst->title, DT_END_ELLIPSIS | DT_CENTER | DT_WORDBREAK | DT_NOPREFIX);
...
static VOID DisplayColumn(HWND hwnd, int col, int offset, HFONT hfont, HFONT hcolumnheadingfont, HDC hdc)
{
    ...
    if (col > 0)
    {
        ...
        else // Print column header text (if any)
        {
            ...
            strcpy_s(buffer, nullptr == lpgi ? "" : lpgi->lpszCurValue);
        }
    }
    rectsave = rect;

    SetTextColor(hdc, IsWindowEnabled(hwnd) ? GetSysColor(COLOR_WINDOWTEXT) : GetSysColor(COLOR_GRAYTEXT));

    DrawEdge(hdc, &rect, EDGE_ETCHED, BF_MIDDLE | BF_RECT | BF_ADJUST);

    holdfont = (HFONT)SelectObject(hdc, hcolumnheadingfont);
    // DrawTextEx(hdc, buffer, -1, &rect, DT_END_ELLIPSIS | DT_CENTER |
    // DT_WORDBREAK | DT_NOPREFIX, NULL);
    print_colored_text_to_grid_cell(hdc, rect, buffer, DT_END_ELLIPSIS | DT_CENTER | DT_WORDBREAK | DT_NOPREFIX);
    ...
    while (row <= g_lpInst->bottomvisiblerow)
    {
        ...
        case GCT_BUTTON: {
            ...
            {
                // DrawTextEx(hdc, buffer, -1, &rect, DT_END_ELLIPSIS | DT_LEFT |
                // DT_VCENTER | DT_CENTER | DT_SINGLELINE | DT_NOPREFIX, NULL);
                print_colored_text_to_grid_cell(hdc, rect, buffer,
                                                DT_END_ELLIPSIS | DT_LEFT | DT_VCENTER | DT_CENTER | DT_SINGLELINE |
                                                    DT_NOPREFIX);
            }
            else
            {
                // DrawTextEx(hdc, buffer, -1, &rect, DT_LEFT | DT_VCENTER |
                // DT_WORDBREAK | DT_EDITCONTROL | DT_NOPREFIX, NULL);
                print_colored_text_to_grid_cell(hdc, rect, buffer,
        ...
        case GCT_COMBO: {
            ...
            {
                // DrawTextEx(hdc, buffer, -1, &rect, DT_END_ELLIPSIS | DT_LEFT |
                // DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX, NULL);
                print_colored_text_to_grid_cell(hdc, rect, buffer,
                                                DT_END_ELLIPSIS | DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX);
            }
            else
            {
                // DrawTextEx(hdc, buffer, -1, &rect, DT_LEFT | DT_VCENTER |
        ...
        case GCT_CHECK: {
            ...
            DrawFrameControl(hdc, &rectCheck, DFC_BUTTON,
        ...
        case GCT_IMAGE: {
            ...
            if (0 < strlen(end)) // we failed but I don't know how this could have
                                 // happened
            {
                index = 0;
            }
            ...
            ImageList_DrawEx(hImageList, index, hdc, rectImage.left, rectImage.top, WIDTH(rectImage), HEIGHT(rectImage),
        ...
        default: // Text
        {
            ...
            if (g_lpInst->ELLIPSIS)
            {
                ...
                // if (is_main_window_constructed && strstri(title_buffer, "name") !=
                // nullptr) {
                uFormat |= DT_LEFT;
                // } else {
                // uFormat |= GSA_LEFT == dwAllignment ? DT_LEFT : GSA_RIGHT ==
                // dwAllignment ? DT_RIGHT : IsNumeric(buffer) ? DT_RIGHT : DT_LEFT;
                // }
                // DrawTextEx(hdc, buffer, -1, &rect, uFormat, NULL);
            ...
            else
            {
                ...
                /*} else {
                 uFormat |= GSA_LEFT == dwAllignment ? DT_LEFT : GSA_RIGHT ==
                 dwAllignment ? DT_RIGHT : IsNumeric(buffer)         ? DT_RIGHT :
                 DT_LEFT;   }*/
                // DrawTextEx(hdc, buffer, -1, &rect, uFormat, NULL);
                print_colored_text_to_grid_cell(hdc, rect, buffer, uFormat);
            }
...
static VOID SetHomeRow(HWND hwnd, int col, int row)
{
    ...
    if ((cellrect.bottom > gridrect.bottom) &&
        ((cellrect.bottom - cellrect.top) < (gridrect.bottom - (g_lpInst->headerrowheight + g_lpInst->titleheight))))
    {
...
/// @brief Sets the home column field of the grid based on current cursor
/// position.
///
/// @param hwnd Handle of the grid
/// @param col The column that the current cell belongs to
...
static int GetNthVisibleColumn(HWND, int n)
{
    int j, count;
    int cols = ColCount();
    int value;
...
static int FindLongestLine(HDC hdc, const char *text, PSIZE size)
{
    // const size_t str_len{ len(text) };
    // char *temptext = (char *)_alloca(sizeof(char) * (str_len + 1));
...
///
/// @returns VOID
static VOID AdjustParentColWidth(HWND hwnd, INT col, INT row, const char *lpszValue)
{
    HDC hdc;
    SIZE size = {0, 0};
    int required_width;
    ...
    {
        int count{1};
        const size_t str_len{len(lpszValue)};
        // char *tbuffer = (char *)_alloca(sizeof(char) * (str_len + 1));
        // strcpy_s(tbuffer, str_len + 1, lpszValue);
        for (size_t j{}; j < str_len + 1; ++j) // DWM 2.2.1: Replaced NELEMS with len + 1 per suggestion of
                                               // Hans-Peter Kalb
        {
            // if (tbuffer[j] == '\n') {
            if (lpszValue[j] == '\n')
...
static LRESULT CALLBACK Edit_Proc(HWND hEdit, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    else if (WM_KEYDOWN == msg)
    {
        ...
        case VK_LEFT:
        case VK_RIGHT:
            if (str_compare_i(CHECKED, (const char *)GetPropA(hEdit, EDITMODE)) == 0)
                break; // DWM 1.7: Added
                       // else fallthrough
        case VK_NEXT:
        case VK_PRIOR:
...
static HWND CreateEdit(HINSTANCE hInstance, HWND hwndParent, INT id,
                       BOOL fEditMode) // DWM 1.7: Added fEditMode argument
{
    ...
    hwnd = CreateWindowEx(dwExStyle, WC_EDIT, TEXT(""), dwStyle, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT,
                          CW_USEDEFAULT, hwndParent, (HMENU)id, hInstance, NULL);
...
static HWND CreateButton(HINSTANCE hInstance, HWND hwndParent, INT id)
{
    ...
    hwnd = CreateWindowEx(0, WC_BUTTON, TEXT("..."), dwStyle, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT,
                          CW_USEDEFAULT, hwndParent, (HMENU)id, hInstance, NULL);
...
/// @param prc The address of a rectangle with the bounds in which to center the
/// combobox.
///
/// @returns VOID.
...
///       child (if it is an editable list box).

/// @par Warning
///       The drop down list control of a combobox is not a child of the
///       combobox it is the child of the desktop so that the list is not
///       clipped by the combobox's client area.  Do not sub class it to this
///       procedure since doing so will cause the instance data pointer to be
///       reset to NULL!  The instance data pointer is attached to the Grid's
///       window.
///
/// @returns LRESULT depends on message.
...
static HWND CreateCombo(HINSTANCE hInstance, HWND hwndParent, INT id)
{
    ...
    hwnd = CreateWindowEx(dwExStyle, WC_COMBOBOX, NULL, dwStyle, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT,
                          CW_USEDEFAULT, hwndParent, (HMENU)id, hInstance, NULL);
...
HWND New_SimpleGrid(HWND hParent, DWORD dwID)
{
    ...
    // Get hinstance if this code is compiled into and called from a dll as well
    // as if it were
    ...
    hControl =
        CreateWindowEx(0, WC_SIMPLEGRID, _T(""), WS_CHILD | WS_TABSTOP, 0, 0, 0, 0, hParent, (HMENU)dwID, hinst, NULL);
...
/// @param code The scroll bar value that indicates the user's scrolling
/// request.
/// @param pos The current position of the scroll box if the @b code is
///             SB_THUMBPOSITION or SB_THUMBTRACK
///
/// @returns VOID
static VOID Grid_OnHScroll(HWND hwnd, HWND, UINT code, int pos)
{
    SetFocus(hwnd);
...
///
/// @returns VOID.
static VOID Grid_OnSelectEdit(HWND hwnd, RECT rc, char *text,
                              BOOL fEditMode) // DWM 1.7: Added fEditMode argument
{
...
///
/// @returns VOID.
static void Grid_OnKeyDown(HWND hwnd, UINT vk, BOOL, int cRepeat, UINT flags)
{
    ...
    default: {
        ...
        if ((VK_F1 <= vk && vk <= VK_F24) || VK_TAB == vk || // DWM 1.7: Added
            ...
            VK_NUMLOCK == vk || VK_LWIN == vk || VK_RWIN == vk || VK_APPS == vk)
            break;
        ...
        case GCT_EDIT: // DWM 1.6: Added CONTEXTCODE_ALT check
        {
            ...
            Grid_OnSelectEdit(hwnd,
                              cellrect, // if CONTEXTCODE_ALT then edit mode
                              fEditMode ? lpgi->lpszCurValue : buf,
    ...
    case ID_EDIT: {
        ...
        if (EN_EDITENDOK == codeNotify)
        {
            ...
            if (nullptr != lpi)
            {
                ...
                GetWindowTextA(hwndCtl, edit_buffer,
                               std::size(edit_buffer)); // DWM 2.2.1: Replaced NELEMS with len +
                                                        // 1 per suggestion of Hans-Peter Kalbf
...
static VOID Grid_OnLButtonDown(HWND hwnd, BOOL fDoubleClick, int x, int y, UINT keyFlags)
{
    ...
    if (GetFocus() == hwnd)
    {
        ...
        fClickSelect = GCT_BUTTON == GetColType(g_lpInst->cursorcol) || GCT_COMBO == GetColType(g_lpInst->cursorcol) ||
...
///
/// @returns VOID.
static VOID Grid_OnLButtonUp(HWND, int, int, UINT)
{
    if (g_lpInst->COLUMNSIZING)
    {
...
///
/// @returns VOID.
static VOID Grid_OnMouseMove(HWND hwnd, int x, int y, UINT)
{
    int r, c, t, z;
    r = GetRowOfMouse(y);
    c = GetColOfMouse(x);
    ...
    else if (GCT_LINK == GetColType(c) && (!g_lpInst->COLUMNSIZING) && 0 < r)
    {
        ...
        if (NULL != lpgi)
        {
            ...
            // print_colored_text_to_grid_cell(hdc, rcTxt, lpgi->lpszCurValue,
            // DT_END_ELLIPSIS | DT_CENTER | DT_WORDBREAK | DT_NOPREFIX);
            ...
            {
                // rcHitBox.left += ((WIDTH(rcHitBox) / 2) - (WIDTH(rcTxt) / 2));
                // //Centered text
                rcHitBox.right = rcHitBox.left + WIDTH(rcTxt);
            }
            if ((rcHitBox.left <= x) && (x <= rcHitBox.right) && (rcHitBox.top <= y) && (y <= rcHitBox.bottom))
            {
...
static VOID Grid_OnPaint(HWND hwnd)
{
    ...
    if (hbmp)
    {
        BitBlt(hdcOrig, ps.rcPaint.left, ps.rcPaint.top, WIDTH(ps.rcPaint), HEIGHT(ps.rcPaint), hdc, ps.rcPaint.left,
...
///
/// @param hwnd Handle of grid.
/// @param lpszText A pointer to a null-terminated string that is the window
/// text.
///
/// @returns VOID.
...
///
/// @returns VOID.
static void Grid_OnSize(HWND hwnd, UINT, int cx, int cy)
{
    static int cheight;
...
///
/// @returns VOID
static VOID Grid_OnVScroll(HWND hwnd, HWND, UINT code, int pos)
{
    SetFocus(hwnd);
    if (code == SB_THUMBTRACK)
    {
...
///
/// @returns VOID.
static VOID Grid_OnMouseWheel(HWND hwnd, int, int, int zDelta, UINT keyFlags)
{
    UINT code = 0;
    UINT uScrollLines = 3;
    ...
    if ((zDelta < 0 && g_lpInst->cWheelRemainder < 0) || (zDelta > 0 && g_lpInst->cWheelRemainder > 0))
...
static LRESULT Grid_OnAddColumn(HWND, WPARAM, LPARAM lParam)
{
    BOOL fSuccess = TRUE;
    LPSGCOLUMN lpColumn;
    LPVECTOR temp;
...
static LRESULT Grid_OnInsertRow(HWND, WPARAM wParam,
                                LPARAM lParam) // DWM 2.1: Added
{
    // Author: Ferenc Hechler
    int nPos = wParam + 1;
    BOOL fSuccess = FALSE;
...
static LRESULT Grid_OnDeleteRow(HWND, WPARAM wParam) // DWM 2.1: Added
{
    int nPos = wParam + 1;
    BOOL fSuccess = FALSE;
    LPVECTOR temp;
...
static LRESULT Grid_OnSelectCell(HWND hwnd, INT col, INT row, BOOL fOverwritemode)
{
    ...
    if (ERROR_SUCCESS == lResult)
    {
        ...
        FORWARD_WM_KEYDOWN(hwnd, VK_SHIFT, 0, fOverwritemode ? 0 : CONTEXTCODE_ALT, SNDMSG);
    }
    return lResult;
}
...
///
/// @returns ERROR_SUCCESS otherwise SG_ERROR if desired cell is out of bounds
static LRESULT Grid_OnSetItemProtection(HWND, WPARAM wParam, LPARAM lParam)
{
    LPGRIDITEM lpgi;
...
static LRESULT Grid_OnSetItemData(HWND hwnd, WPARAM, LPARAM lParam)
{
    ...
    char buf[5]; // Allow for indexes up to 9999 (more images than an image list
                 // likely can hold)
    LPGRIDITEM lpgi;
    LPSGITEM lpSGitem = (LPSGITEM)lParam;
    DWORD dwColType;
    ...
    switch (dwColType)
    {
    case GCT_BUTTON: // Make sure that the button text is the same as the cell
                     // if updated
        if (NULL != g_lpInst->hwndControl)
    ...
    case GCT_COMBO:
    case GCT_LINK:
        lpszValue = (const char *)lpSGitem->lpCurValue;
        break;
    case GCT_CHECK:
        lpszValue = ((BOOL)lpSGitem->lpCurValue) ? CHECKED : UNCHECKED;
        break;
    case GCT_IMAGE:
        snprintf(buf, NELEMS(buf), "%ld",
                 lpSGitem->lpCurValue); // DWM 1.9: was _stprintf
        lpszValue = buf;
    ...
    {
        // The Link display text and hyperlink are allocated in a double null
        ...
        lpgi->lpszMisc = lpgi->lpszCurValue + len(lpgi->lpszCurValue) + 1;
    }
    else
    {
...
///
/// @returns ERROR_SUCCESS otherwise SG_ERROR if desired cell is out of bounds
static LRESULT Grid_OnSetItemTextAlignment(HWND, WPARAM wParam, LPARAM lParam)
{
...
static LRESULT Grid_OnGetItemDataLength(HWND, WPARAM wParam, LPARAM lParam)
{
    LPGRIDITEM lpgi;
    ...
    if (NULL != lpgi)
    {
        ...
        case GCT_LINK: {
            ...
            INT iLen = 0;
            // Walk the buffer to find the terminating empty string.
            for (const char *p = lpgi->lpszCurValue; *p; (p += strlen(p) + 1, iLen = p - lpgi->lpszCurValue))
                ;
...
static LRESULT Grid_OnGetItemData(HWND, WPARAM, LPARAM lParam)
{
    ...
    case GCT_LINK: {
        ...
        memcpy((char *)lpSGitem->lpCurValue, lpgi->lpszCurValue,
               iLen + 1); // copy the terminating null also
        return iLen;
    }
    case GCT_CHECK:
...
///
/// @returns VOID
static void Grid_OnSetHeaderRowHeight(HWND hwnd, WPARAM wParam, LPARAM)
{
    if ((int)wParam >= 0)
    {
        RECT rect = {0, 0, 0, 0};
...
/// @param lParam This parameter is not used
///
/// @returns SG_ERROR if desired cell is out of bounds, TRUE if it is protected,
/// otherwise FALSE
static LRESULT Grid_OnGetItemProtection(HWND, WPARAM wParam, LPARAM)
{
    LPGRIDITEM lpgi;
...
///
/// @returns VOID
static void Grid_OnSetTitleHeight(HWND hwnd, WPARAM wParam, LPARAM)
{
    if ((int)wParam < 0)
    {
        wParam = 0;
...
static LRESULT Grid_OnGetImageList(WPARAM wParam, LPARAM)
{
    HIMAGELIST himl = 0;
...
static LRESULT CALLBACK Grid_Proc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case SG_GETCOLUMNHEADERTEXT: {
        ...
        {
            strcpy_s((char *)lParam, 1024, (const char *)sgi.lpCurValue);
            return len((const char *)lParam);
        }
        return dwRtn;
    }
...
int findLongestTextWidthInColumn(HWND hwnd, const int col, const int start_row, const int last_row,
                                 const int min_col_width, const int max_col_width)
{
    char buffer[512];
    HDC hdc;
    SIZE size = {0, 0};
