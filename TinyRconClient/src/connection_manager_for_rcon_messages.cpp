...
connection_manager_for_rcon_messages::connection_manager_for_rcon_messages()
    : udp_socket_for_messages{udp_service_for_messages}
{
    try
    {
...
size_t connection_manager_for_rcon_messages::process_and_send_message(
    const std::string &command_name, const std::string &data, const bool is_show_in_messages,
    const std::string &remote_ip, const uint_least16_t remote_port, const bool is_call_msg_handler) const
{
    ...
    const size_t sent_bytes =
        udp_socket_for_messages.send_to(buffer(outgoing_data.c_str(), outgoing_data.length()), dst);
    main_app.add_to_next_uploaded_data_in_bytes(sent_bytes);
    return sent_bytes;
}

bool connection_manager_for_rcon_messages::wait_for_and_process_response_message(const std::string &remote_ip,
                                                                                 const uint_least16_t remote_port) const
{
    ...
    asio::error_code erc{};
    const size_t noOfReceivedBytes =
        udp_socket_for_messages.receive_from(buffer(incoming_data_buffer, receive_buffer_size), destination, 0, erc);
    if (erc)
        return false;
    ...
    if (noOfReceivedBytes > 0)
    {
        ...
        if (header_last == string::npos)
            return false;
        const string message_header{message.substr(header_start + 2, header_last - (header_start + 2))};
        message.erase(0, header_last + 2);
        auto msg_numbers =
            str_split(message_header, ":", nullptr, split_on_whole_needle_t::yes, ignore_empty_string_t::yes);
        if (msg_numbers.size() != 3)
            return false;
        ...
        if (received_message_sequences.contains(message_id))
        {
            ...
            std::sort(
                begin(received_message_sequences[message_id]), end(received_message_sequences[message_id]),
                [](const pair<size_t, string> &lp, const pair<size_t, string> &rp) { return lp.first < rp.first; });
        }
        else
        {
            received_message_sequences[message_id] =
                vector<std::pair<size_t, string>>{make_pair(current_message_number, message)};
        }
