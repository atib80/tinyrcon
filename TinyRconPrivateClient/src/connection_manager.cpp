...
connection_manager::connection_manager() : udp_socket_for_non_rcon_commands{ udp_service_for_non_rcon_commands }
{
  try {
  ...
  const bool) const
{
  ...
  const size_t noOfAllReceivedBytes{ received_reply.length() };

  if (noOfAllReceivedBytes > 0) {

    if (str_contains(received_reply, "js_pid_to_guid", 0U, true)) {

      constexpr const int64_t upper_limit{ 1LL << 32 };
      const auto last_two_newline_chars_pos = received_reply.rfind("\n\n");
      if (last_two_newline_chars_pos == string::npos) 
          return noOfAllReceivedBytes;
      unordered_map<int, std::string> pid_to_ip_address;

      received_reply.erase(begin(received_reply) + last_two_newline_chars_pos, end(received_reply));
      auto pid_to_guid_segments = str_split(received_reply, ";", nullptr, split_on_whole_needle_t::yes, ignore_empty_string_t::yes);
      for (auto &line : pid_to_guid_segments) {
        trim_in_place(line);
        auto pid_and_guid = str_split(received_reply, "=", nullptr, split_on_whole_needle_t::yes, ignore_empty_string_t::yes);
        if (pid_and_guid.size() >= 2u) {
          if (int pid{ -1 }; is_valid_decimal_whole_number(pid_and_guid[0], pid) && pid >= 0 && pid < 64) {

            const int64_t guid = std::stoll(pid_and_guid[1]);

            unsigned long guid_key = static_cast<unsigned long>(guid < 0 ? upper_limit + guid : guid);
            pid_to_ip_address.emplace(pid, convert_guid_to_ip_address(guid_key));
          }
        }
      }
    ...
    if (received_reply.starts_with("Invalid password.") || received_reply.starts_with("Bad rcon")) {
      ...
      if (received_reply.find(rcon_status_response_needle1) != string::npos
        ...
        main_app.get_connection_manager_for_messages().get_last_rcon_status_received() = get_current_time_stamp();
        ...
        if (!is_server_empty_or_error_receiving_udp_datagrams) {
          ...
          for (const string &player_line : lines) {
            ...
            i = j + ((gn == game_name_t::cod1 || gn == game_name_t::cod2) ? 11 : 33);
            ...
            players_data[pl_index].ip_address = main_app.get_pid_to_ip_address().contains(player_pid) ? main_app.get_pid_to_ip_address().at(player_pid) : ip_address;
