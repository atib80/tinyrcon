  ...
  _In_ int nCmdShow)
{
  ...
  main_app.add_command_handler({ "!rc" }, [](const vector<string> &user_cmd) {
    ...
    if (user_cmd.size() >= 2 && !user_cmd[1].empty()) {
      const string rcon_command{ stl::helper::to_lower_case(stl::helper::trim(stl::helper::str_join(cbegin(user_cmd), cend(user_cmd), " "))) };
  ...
  const string program_title{ "Private remote TinyRcon client | "s + main_app.get_current_game_server().get_server_name() + " | "s + "version: "s + program_version };
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
  ...
  case WM_TIMER: {
    ...
    if (counter % 30 == 0) {
      ...
      const auto current_ts{ get_current_time_stamp() };
      if (main_app.get_is_connection_settings_valid()) {
        // auto &me = main_app.get_remote_user_for_name(main_app.get_username(), main_app.get_user_ip_address());
