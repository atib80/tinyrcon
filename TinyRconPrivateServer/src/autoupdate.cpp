...
void auto_update_manager::check_for_updates()
{
  ...
  if (nullptr != threadParam.internet_open_handle.get()) {
    ...
    if (nullptr != threadParam.internet_connect_handle.get()) {
      ...
      auto available_file_names = get_file_name_matches_for_specified_ftp_path_and_file_pattern(threadParam.internet_connect_handle, ftp_download_folder_path.c_str(), "_U_PrivateTinyRconServer*.exe");
      ...
      available_file_names = get_file_name_matches_for_specified_ftp_path_and_file_pattern(threadParam.internet_connect_handle, ftp_download_folder_path.c_str(), "geo-*.7z");
      if (!available_file_names.empty()) {
        ...
        if (current_geo_dat_file_md5 != new_geo_dat_file_md5) {
          ...
          if (!check_if_file_path_exists(seven_zip_dll_file_path.c_str())) {
            Sleep(20);
            available_file_names = get_file_name_matches_for_specified_ftp_path_and_file_pattern(threadParam.internet_connect_handle, ftp_download_folder_path.c_str(), "7za.dll");
            if (!available_file_names.empty()) {
...
DWORD WINAPI worker_function5(void *param)
{
  internet_connection_handles *pThreadParm{ reinterpret_cast<internet_connection_handles *>(param) };
  if (pThreadParm->internet_open_handle.get() != nullptr) {
    DWORD internet_option_connect_timeout{ 11000 };
    InternetSetOptionA(pThreadParm->internet_open_handle.get(), INTERNET_OPTION_CONNECT_TIMEOUT, &internet_option_connect_timeout, sizeof(internet_option_connect_timeout));
    /*pThreadParm->internet_connect_handle.set(InternetConnectA(pThreadParm->internet_open_handle.get(), main_app.get_ftp_download_site_ip_address().c_str(), INTERNET_DEFAULT_FTP_PORT, nullptr, nullptr, INTERNET_SERVICE_FTP, INTERNET_FLAG_PASSIVE, 0));*/
    pThreadParm->internet_connect_handle.set(InternetOpenUrl(pThreadParm->internet_open_handle.get(),
      /* "http://myexternalip.com/raw",*/
