...
volatile std::atomic<bool> is_display_public_admins_data_event{ false };
volatile std::atomic<bool> is_display_private_admins_data_event{ false };
...
std::mutex protect_ip_address_processing;
std::mutex save_players_stats_data_mutex;
std::recursive_mutex welcome_messages_mutex_for_players;
std::recursive_mutex welcome_messages_mutex_for_admins;
...
{
  return ban_entry_type::count == std::accumulate(cbegin(ban_entries_synchronized_for_admins_flag), cend(ban_entries_synchronized_for_admins_flag), 0);
}

bool is_ban_entries_synchronized_for_players()
{
  return ban_entry_type::count == std::accumulate(cbegin(ban_entries_synchronized_for_players_flag), cend(ban_entries_synchronized_for_players_flag), 0);
}
...
^3Type ^1!allow 123.123.123.123 ^3[Enter] to ^3allow (exclude) IP address ^1123.123.123.123 ^3from ^5Tiny^6Rcon^3's 
  automatic ^1VPN/proxy IP address ^3check.
^5Type ^1!disallow 123.123.123.123 ^5[Enter] to add IP address ^1123.123.123.123 ^5to Tiny^6Rcon^5's list 
  of ^1disallowed IP addresses ^5(automatic kick).
...
std::unordered_map<int, string> button_id_to_label_text{
  ...
  _In_ int nCmdShow)
{
  ...
  //// 3586413476 -> 213.196.83.164
  // const string ip_address1 = convert_guid_to_ip_address(3586413476);
  // if (ip_address1 == "213.196.83.164") {
  //   MessageBoxA(nullptr, "The IP addresses are equal.", "Information", MB_ICONINFORMATION | MB_OK);
  // } else {
  //   MessageBoxA(nullptr, "The IP addresses are not equal!", "Warning", MB_ICONWARNING | MB_OK);
  // }

  //// 1432277709 -> 85.94.210.205
  // const string ip_address2 = convert_guid_to_ip_address(1432277709);
  // if (ip_address2 == "85.94.210.205") {
  //   MessageBoxA(nullptr, "The IP addresses are equal.", "Information", MB_ICONINFORMATION | MB_OK);
  // } else {
  //   MessageBoxA(nullptr, "The IP addresses are not equal!", "Warning", MB_ICONWARNING | MB_OK);
  // }

  //// 1310635317 -> 78.30.181.53
  // const string ip_address3 = convert_guid_to_ip_address(1310635317);
  // if (ip_address3 == "78.30.181.53") {
  //   MessageBoxA(nullptr, "The IP addresses are equal.", "Information", MB_ICONINFORMATION | MB_OK);
  // } else {
  //   MessageBoxA(nullptr, "The IP addresses are not equal!", "Warning", MB_ICONWARNING | MB_OK);
  // }

  //// 2393999716 -> 142.177.137.100
  // const string ip_address4 = convert_guid_to_ip_address(2393999716);
  // if (ip_address4 == "142.177.137.100") {
  //   MessageBoxA(nullptr, "The IP addresses are equal.", "Information", MB_ICONINFORMATION | MB_OK);
  // } else {
  //   MessageBoxA(nullptr, "The IP addresses are not equal!", "Warning", MB_ICONWARNING | MB_OK);
  // }
  ...
  // const string temp_folder_path{ format("{}{}", main_app.get_current_working_directory(), "temp") };
  ...
  if (auto [status, file_path] = create_necessary_folders_and_files({ "config",
        "data",
        "data\\images\\maps",
        "log",
        "plugins\\geoIP",
    ...
    [&](stop_token st) {
      ...
      while (!st.stop_requested() && !is_terminate_program.load()) {
        ...
        try {
          ...
          print_message(app_handles.hwnd_re_messages_data, error_message, is_log_message::yes, is_log_datetime::yes, true, false, false);
        } catch (...) {
          char buffer[512];
  ...
  load_private_users_login_credentials("data\\private_admins.cfg");
  ...
  if (main_app.get_is_enable_automatic_vpn_proxy_ip_address_detection()) {
    lock_guard lk{ protect_ip_address_processing };
    read_ip_v4_addresses_list("data\\allowed_ip_addresses.txt", main_app.get_allowed_ip_v4_addresses());
    read_ip_v4_addresses_list("data\\disallowed_ip_addresses.txt", main_app.get_disallowed_ip_v4_addresses());
  }

  if (unsigned long ip_key{}; check_ip_address_validity(main_app.get_current_game_server().get_server_ip_address(), ip_key)) {
    main_app.get_allowed_ip_v4_addresses().emplace(main_app.get_current_game_server().get_server_ip_address());
  }

  if (unsigned long ip_key{}; check_ip_address_validity(main_app.get_user_ip_address(), ip_key)) {
    main_app.get_allowed_ip_v4_addresses().emplace(main_app.get_user_ip_address());
  }

  const string stats_data_dir_path{ format("{}data", main_app.get_current_working_directory()) };
  auto file_name_matches = get_file_name_matches_for_specified_file_path_pattern(stats_data_dir_path.c_str(), "player_stats_for_year-*.dat");
  ...
  if (!stats_data_file_name.empty()) {
    ...
    load_players_stats_data(file_name_path_for_stats_data_for_year.c_str(), main_app.get_stats_data().get_scores_for_year_vector(), main_app.get_stats_data().get_scores_for_year_map());
  }

  file_name_matches = get_file_name_matches_for_specified_file_path_pattern(stats_data_dir_path.c_str(), "player_stats_for_month-*.dat");
  ...
  if (!stats_data_file_name.empty()) {
    ...
    load_players_stats_data(file_name_path_for_stats_data_for_month.c_str(), main_app.get_stats_data().get_scores_for_month_vector(), main_app.get_stats_data().get_scores_for_month_map());
  }

  file_name_matches = get_file_name_matches_for_specified_file_path_pattern(stats_data_dir_path.c_str(), "player_stats_for_day-*.dat");
  ...
  if (!stats_data_file_name.empty()) {
    ...
    load_players_stats_data(file_name_path_for_stats_data_for_day.c_str(), main_app.get_stats_data().get_scores_for_day_vector(), main_app.get_stats_data().get_scores_for_day_map());
  }

  load_players_stats_data("data\\player_stats.dat", main_app.get_stats_data().get_scores_vector(), main_app.get_stats_data().get_scores_map());
  ...
  main_app.add_command_handler({ "allow", "!allow" }, [](const vector<string> &user_cmd) {
    if (user_cmd.size() > 1u) {
      for (size_t i{ 1 }; i < user_cmd.size(); ++i) {
        std::string ip_address{ stl::helper::trim(user_cmd[i]) };
        if (unsigned long ip_key{}; check_ip_address_validity(ip_address, ip_key)) {
          {
            lock_guard lg{
              protect_ip_address_processing
            };

            if (main_app.get_disallowed_ip_v4_addresses().contains(ip_address)) {
              main_app.get_disallowed_ip_v4_addresses().erase(ip_address);
            }

            if (!main_app.get_allowed_ip_v4_addresses().contains(ip_address)) {
              main_app.get_allowed_ip_v4_addresses().emplace(std::move(ip_address));
            }
          }
        }
      }
  ...
  main_app.add_command_handler({ "disallow", "!disallow" }, [](const vector<string> &user_cmd) {
    if (user_cmd.size() > 1u) {
      for (size_t i{ 1 }; i < user_cmd.size(); ++i) {
        std::string ip_address{ stl::helper::trim(user_cmd[i]) };
        if (unsigned long ip_key{}; check_ip_address_validity(ip_address, ip_key)) {
          {
            ...
            if (main_app.get_allowed_ip_v4_addresses().contains(ip_address)) {
              main_app.get_allowed_ip_v4_addresses().erase(ip_address);
            }

            if (!main_app.get_disallowed_ip_v4_addresses().contains(ip_address)) {
              main_app.get_disallowed_ip_v4_addresses().emplace(std::move(ip_address));
            }
          }
        }
      }
  ...
  main_app.add_command_handler({ "!r", "!report" }, [](const vector<string> &user_cmd) {
    if (user_cmd.size() > 1 && !user_cmd[1].empty()) {
      ...
      if (check_if_user_provided_argument_is_valid_for_specified_command(
        ...
        string reason{ remove_disallowed_characters_in_string(user_cmd.size() > 2 ? str_join(cbegin(user_cmd) + 2, cend(user_cmd), " ") : "not specified") };
        stl::helper::trim_in_place(reason);
        pd.reason = reason;
        pd.banned_start_time = get_current_time_stamp();
        const string converted_ban_date_and_time_info{ get_date_and_time_for_time_t("{DD}.{MM}.{Y} {hh}:{mm}", pd.banned_start_time) };
        size_t no_of_chars_to_copy{ std::min<size_t>(std::size(pd.banned_date_time) - 1, converted_ban_date_and_time_info.length()) };
        strncpy_s(pd.banned_date_time, std::size(pd.banned_date_time), converted_ban_date_and_time_info.c_str(), no_of_chars_to_copy);
        pd.banned_by_user_name = main_app.get_username();
        {
          lock_guard lk{ reports_mutex };
          main_app.get_reported_players().push_back(pd);
        }

        me->no_of_reports++;
        if (me->is_admin) {
          save_tiny_rcon_users_data_to_json_file(main_app.get_remote_users_data_file_path());
        } else {
          save_tiny_rcon_players_data_to_json_file(main_app.get_remote_players_data_file_path());
        }

        const string message{ format("^3You have successfully reported player ^7{} ^3whose ^1pid ^3is ^1{}^3!\nInformation: ^7{}\n", pd.player_name, pid, get_player_information(pid, false, "Reported")) };
        print_colored_text(app_handles.hwnd_re_messages_data, message.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes, true, true, false);
      }
    } else {
      const string re_msg2{ format("^3Invalid command syntax for user command: ^2{}\n", user_cmd[0]) };
      print_colored_text(app_handles.hwnd_re_messages_data, re_msg2.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes, true, true, false);
      if (user_commands_help.contains(user_cmd[0])) {
        print_colored_text(app_handles.hwnd_re_messages_data, user_commands_help.at(user_cmd[0]).c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes, true, true, false);
        print_colored_text(app_handles.hwnd_re_messages_data, "\n", is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::no, true, true, false);
      }
    }
  });

  main_app.add_command_handler({ "!ur", "!unreport" }, [](const vector<string> &user_cmd) {
    if (!validate_admin_and_show_missing_admin_privileges_message(me, false)) return;
    ...
    if (user_cmd.size() > 1 && !user_cmd[1].empty()) {
      ...
      if (!check_if_user_provided_argument_is_valid_for_specified_command(
            user_cmd[0].c_str(), user_cmd[1])) {
        const string re_msg{ format("^5Provided IP address ^1{} ^5is not a valid IP address or there isn't a reported player entry whose ^1serial no. ^5is equal to provided number: ^1{}!\n", user_cmd[1], user_cmd[1]) };
        print_colored_text(app_handles.hwnd_re_messages_data, re_msg.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes, true, true, false);
      } else {
        string message;
        string report_key{ user_cmd[1] };
        auto [status, player]{ remove_reported_player(report_key, message, true) };
        if (status) {
          const auto &remove_report_message_handler = main_app.get_remote_message_handler("remove-report");
          remove_report_message_handler(main_app.get_username(), get_current_time_stamp(), format(R"({}\{}\{}\{}\{}\{}\{})", player.ip_address, player.player_name, player.banned_date_time, player.banned_start_time, player.reason, player.banned_by_user_name, main_app.get_username()), true, main_app.get_user_ip_address());
          const string private_msg{ format("^1Admin ^7{} ^2has successfully removed ^1reported player ^7{}\n ^3IP address: ^1{} ^3geoinfo: ^1{}, {}\n ^3Date/time: ^1{} ^5| ^3Reason: ^1{} ^5| ^3Reported by: ^1{}\n", main_app.get_username(), player.player_name, player.ip_address, player.country_name, player.city, player.banned_date_time, player.reason, player.banned_by_user_name) };
          ...
          const string re_msg{ format("^1Admin ^7{} ^3failed to remove ^1reported player ^7for IP address or player no. ^1{}!\n^1Reason: ^3Provided IP address ^1{} ^3hasn't been ^1reported ^3yet.\n", main_app.get_username(), user_cmd[1], user_cmd[1]) };
  ...
  main_app.add_command_handler({ "reports", "!reports" }, [](const vector<string> &user_cmd) {
    string ex_msg{ format("^1Exception ^3thrown from ^1command handler ^3for ^1'{}' ^3user command.", user_cmd[0]) };
    stack_trace_element ste{
      app_handles.hwnd_re_messages_data,
      std::move(ex_msg)
    };
    int number{};
    const size_t number_of_reports_to_retrieve{ (user_cmd.size() > 1u && !user_cmd[1].empty() && is_valid_decimal_whole_number(user_cmd[1], number)) ? static_cast<size_t>(number) : 25u };

    const string message{ format("^3You have successfully executed player-level command: ^1'{} {}'\n", user_cmd[0], number_of_reports_to_retrieve) };
    print_colored_text(app_handles.hwnd_re_messages_data, message.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes, true, true, false);
    display_reported_players(number_of_reports_to_retrieve);
  });
      ...
      if (check_if_user_provided_argument_is_valid_for_specified_command(
        ...
        if (is_online) {
          ...
          if (warned_players[pid].warned_times >= number_of_warnings_for_automatic_kick) {
            ...
            string reason2{ remove_disallowed_characters_in_string(format("^1Received {} warnings from admin: {}", main_app.get_current_game_server().get_maximum_number_of_warnings_for_automatic_kick(), main_app.get_username())) };
            ...
            save_current_user_data_to_json_file(main_app.get_user_data_file_path());
            main_app.get_connection_manager_for_messages().process_and_send_message("add-kick", format(R"({}\{}\{}\{}\{}\{})", warned_players[pid].ip_address, warned_players[pid].guid_key, warned_players[pid].player_name, warned_players[pid].banned_date_time, reason, main_app.get_username()), true, main_app.get_tiny_rcon_server_ip_address(), main_app.get_tiny_rcon_server_port(), false);
  ...
  main_app.add_command_handler({ "!k", "!kick" }, [](const std::vector<std::string> &user_cmd) {
    ...
    if (user_cmd.size() > 1 && !user_cmd[1].empty()) {
      ...
      if (check_if_user_provided_argument_is_valid_for_specified_command(
        ...
        const int pid{ stoi(user_cmd[1]) };
        string protected_player_msg;
        if (check_if_player_is_protected(get_player_data_for_pid(pid), user_cmd[0].c_str(), protected_player_msg)) {
          print_colored_text(app_handles.hwnd_re_messages_data, protected_player_msg.c_str());
          replace_br_with_new_line(protected_player_msg);
          const string inform_msg{ format("{}\\{}", main_app.get_username(), protected_player_msg) };
          main_app.add_message_to_queue(message_t("inform-message", inform_msg, true));
        ...
        const string date_time_info{ get_date_and_time_for_time_t("{DD}.{MM}.{Y} {hh}:{mm}", player.banned_start_time) };
    ...
    if (user_cmd.size() > 1 && !user_cmd[1].empty()) {
      ...
      if (check_if_user_provided_argument_is_valid_for_specified_command(
        ...
        if (int number{}; is_valid_decimal_whole_number(user_cmd[1], number)) {
          ...
          if (user_cmd.size() > 2) {
            if (is_valid_decimal_whole_number(user_cmd[2], number)) {
              ...
              if (user_cmd.size() > 3) {
                reason = remove_disallowed_characters_in_string(str_join(cbegin(user_cmd) + 3, cend(user_cmd), " "));
                stl::helper::trim_in_place(reason);
              }
            } else {
              reason = remove_disallowed_characters_in_string(str_join(cbegin(user_cmd) + 2, cend(user_cmd), " "));
              stl::helper::trim_in_place(reason);
            }
          }
          ...
          main_app.get_tinyrcon_dict()["{REASON}"] = reason;
          main_app.get_tinyrcon_dict()["{TEMPBAN_DURATION}"] = to_string(temp_ban_duration);
          const string message{ format("^3You have successfully executed ^5!tempban ^3on IP address ^1{} for ^1{} {}^3!\n", user_cmd[1], temp_ban_duration, temp_ban_duration != 1 ? "hours" : "hour") };
          print_colored_text(app_handles.hwnd_re_messages_data, message.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
          string command{ main_app.get_user_defined_tempban_message() };
          build_tiny_rcon_message(command);
          if (main_app.get_is_disable_automatic_kick_messages()) {
        ...
        if (int n{}; is_valid_decimal_whole_number(user_cmd[1], n)) {
          ...
          const string re_msg{ format("^2{} ^3is not a valid IP address for the ^2!tb ^3(^2!tempban^3) command!\n", user_cmd[1]) };
          print_colored_text(app_handles.hwnd_re_messages_data, re_msg.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
        }
      }
    } else {
        ...
        if (int pid{ -1 }; is_valid_decimal_whole_number(user_cmd[1], pid)) {
          ...
          if (pid == player.pid) {
            ...
            if (check_ip_address_validity(player.ip_address, ip_key)) {
              ...
              save_current_user_data_to_json_file(main_app.get_user_data_file_path());
              main_app.get_connection_manager_for_messages().process_and_send_message("add-muted-ip", format(R"({}\{}\{}\{}\{}\{})", player.ip_address, player.guid_key, player.player_name, player.banned_date_time, reason, main_app.get_username()), true, main_app.get_tiny_rcon_server_ip_address(), main_app.get_tiny_rcon_server_port(), false);
            }
          }
        } else {
          ...
          for (size_t i{}; i < main_app.get_current_game_server().get_number_of_players(); ++i) {
            ...
            if (player.ip_address == ip_address) {
              ...
              print_colored_text(app_handles.hwnd_re_messages_data, re_msg.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
              main_app.get_tinyrcon_dict()["{REASON}"] = reason;
              const string message{ format("^3You have successfully executed ^5{} ^3on player ({}^3)\n", user_cmd[0], get_player_information(player.pid, false, "Muted")) };
              print_colored_text(app_handles.hwnd_re_messages_data, message.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
              string command{ main_app.get_user_defined_mute_message() };
              if (main_app.get_is_disable_automatic_kick_messages()) {
                command.clear();
          ...
          if (!is_ip_address_already_muted) {
            ...
            string reason{ trim(remove_disallowed_characters_in_string(user_cmd.size() > 2 ? str_join(cbegin(user_cmd) + 2, cend(user_cmd), " ") : "not specified")) };
            ...
            if (name_start_pos != string::npos) {
              string player_name{ trim(reason.substr(name_start_pos + name_length, reason_start_pos - (name_start_pos + name_length))) };

              if (player_name.length() >= 2u && ((player_name.front() == '\'' && player_name.back() == '\'') || (player_name.front() == '"' && player_name.back() == '"'))) {
              ...
              const size_t no_of_chars_to_copy{ std::min<size_t>(std::size(player_offline.player_name) - 1, player_name.length()) };
              strncpy_s(player_offline.player_name, std::size(player_offline.player_name), player_name.c_str(), no_of_chars_to_copy);
            ...
            if (reason.length() >= 2 && ((reason.front() == '\'' && reason.back() == '\'') || (reason.front() == '"' && reason.back() == '"'))) {
              reason.pop_back();
              reason.erase(0, 1);
            }
            ...
            convert_guid_key_to_country_name(main_app.get_connection_manager().get_geoip_data(), player_offline.ip_address, player_offline);
            string protected_player_msg;
            ...
            const string message{ format(
              "^2You have successfully executed ^5{} ^2on player ({}^2)\n", user_cmd[0], get_player_information_for_player(player_offline, "Muted")) };
            print_colored_text(app_handles.hwnd_re_messages_data, message.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
            auto &current_user = main_app.get_user_for_name(main_app.get_username());
            current_user->no_of_muted_ip_addresses++;
            save_current_user_data_to_json_file(main_app.get_user_data_file_path());
            main_app.get_connection_manager_for_messages().process_and_send_message("add-muted-ip", format(R"({}\0\{}\{}\{}\{})", player_offline.ip_address, player_offline.player_name, player_offline.banned_date_time, player_offline.reason, main_app.get_username()), true, main_app.get_tiny_rcon_server_ip_address(), main_app.get_tiny_rcon_server_port(), false);
          }
          // }
        }
      }
  ...
  main_app.add_command_handler({ "!unmute" }, [](const vector<string> &user_cmd) {
    ...
    if (user_cmd.size() >= 2 && !user_cmd[1].empty()) {
      ...
      if (!is_ip_address_valid && !check_if_user_provided_pid_is_valid(user_cmd[1])) {
        const string re_msg{ format("^5Provided IP address ^1{} ^5is not a valid IP address or there isn't a muted player entry whose ^1serial no. ^5is equal to the provided number: ^1{}!\n", user_cmd[1], user_cmd[1]) };
        print_colored_text(app_handles.hwnd_re_messages_data, re_msg.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
      } else {
        string message;
        string ip_address{ user_cmd[1] };
        int pid{ -1 };
        if (!is_ip_address_valid && check_if_user_provided_pid_is_valid(user_cmd[1]) && is_valid_decimal_whole_number(user_cmd[1], pid)) {
        ...
        if (status) {
          ...
          main_app.get_connection_manager_for_messages().process_and_send_message("remove-muted-ip", format(R"({}\{}\{}\{}\{}\{}\{})", player.ip_address, player.guid_key, player.player_name, player.banned_date_time, player.reason, player.banned_by_user_name, main_app.get_username()), true, main_app.get_tiny_rcon_server_ip_address(), main_app.get_tiny_rcon_server_port(), false);
    ...
    if (user_cmd.size() > 1 && !user_cmd[1].empty()) {
      if (check_if_user_provided_argument_is_valid_for_specified_command(
        ...
        if (int pid{ -1 }; is_valid_decimal_whole_number(user_cmd[1], pid)) {
          ...
          if (pid == player.pid) {
            unsigned long ip_key{};
            if (check_ip_address_validity(player.ip_address, ip_key) && !banned_ip_addresses.contains(player.ip_address)) {
              main_app.get_tinyrcon_dict()["{ADMINNAME}"] = main_app.get_username();
              main_app.get_tinyrcon_dict()["{PLAYERNAME}"] = get_player_name_for_pid(player.pid);
              string reason{ remove_disallowed_characters_in_string(user_cmd.size() > 2 ? str_join(cbegin(user_cmd) + 2, cend(user_cmd), " ") : "not specified") };
              stl::helper::trim_in_place(reason);
              ...
              print_colored_text(app_handles.hwnd_re_messages_data, re_msg.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
              main_app.get_tinyrcon_dict()["{REASON}"] = reason;
              const string message{ format("^3You have successfully executed ^5{} ^3on player ({}^3)\n", user_cmd[0], get_player_information(pid, false, "IP address banned")) };
              print_colored_text(app_handles.hwnd_re_messages_data, message.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
          ...
          if (!banned_ip_addresses.contains(ip_address)) {
            ...
            if (!is_ip_address_already_banned) {
              ...
              print_colored_text(app_handles.hwnd_re_messages_data, re_msg.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
              const string message{ format(
                "^2You have successfully executed ^5{} ^2on player ({}^2)\n", user_cmd[0], get_player_information_for_player(player_offline, "Banned IP address")) };
              print_colored_text(app_handles.hwnd_re_messages_data, message.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
              auto &current_user = main_app.get_user_for_name(main_app.get_username());
              current_user->no_of_ipbans++;
              save_current_user_data_to_json_file(main_app.get_user_data_file_path());
  ...
  main_app.add_command_handler({ "!protectedcities" }, [](const vector<string> &) {
    is_display_protected_cities_data_event.store(true);
  });

  main_app.add_command_handler({ "!protectedcountries" }, [](const vector<string> &) {
  ...
  main_app.add_command_handler({ "!protectip" }, [](const vector<string> &user_cmd) {
    ...
    if (user_cmd.size() > 1 && !user_cmd[1].empty()) {
      if (check_if_user_provided_argument_is_valid_for_specified_command(
            "!protectip", user_cmd[1])) {
        string ex_msg{ format("^1Exception ^3thrown from ^1command handler ^3for ^1'{} {}' ^3user command.", user_cmd[0], user_cmd[1]) };
        stack_trace_element ste{
          app_handles.hwnd_re_messages_data,
          std::move(ex_msg)
        };
        auto &protected_ip_addresses =
          main_app.get_current_game_server().get_protected_ip_addresses();
        if (int pid{ -1 }; is_valid_decimal_whole_number(user_cmd[1], pid)) {
          auto &player = main_app.get_current_game_server().get_player_data(pid);
          if (pid == player.pid) {
            unsigned long ip_key{};
            if (check_ip_address_validity(player.ip_address, ip_key) && !protected_ip_addresses.contains(player.ip_address)) {
              ...
              stl::helper::trim_in_place(reason);
              specify_reason_for_player_pid(player.pid, reason);
              const string re_msg{ format("^2You have successfully protected ^1IP address: {}\n", player.ip_address) };
              print_colored_text(app_handles.hwnd_re_messages_data, re_msg.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
              main_app.get_tinyrcon_dict()["{REASON}"] = std::move(reason);
              const string message{ format("^3You have successfully executed ^5{} ^3on player ({}^3)\n", user_cmd[0], get_player_information(pid, false, "Protected IP address")) };
              print_colored_text(app_handles.hwnd_re_messages_data, message.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
              ...
              main_app.get_connection_manager_for_messages().process_and_send_message("protect-ipaddress", format("{}\\{}\\{}", main_app.get_username(), player.ip_address, player.reason), true, main_app.get_tiny_rcon_server_ip_address(), main_app.get_tiny_rcon_server_port(), false);
            }
          }
        } else {
          const auto &ip_address = user_cmd[1];
          ...
          if (!protected_ip_addresses.contains(ip_address)) {
            ...
            if (!is_ip_address_already_protected) {
              ...
              print_colored_text(app_handles.hwnd_re_messages_data, re_msg.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
              const string message{ format(
                "^3You have successfully executed ^5{} ^3on player ({}^3)\n", user_cmd[0], get_player_information_for_player(player_offline, "Protected IP address")) };
              print_colored_text(app_handles.hwnd_re_messages_data, message.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
              main_app.get_connection_manager_for_messages().process_and_send_message("protect-ipaddress", format("{}\\{}\\{}", main_app.get_username(), player_offline.ip_address, player_offline.reason), true, main_app.get_tiny_rcon_server_ip_address(), main_app.get_tiny_rcon_server_port(), false);
            }
          }
        }
      }
        ...
        if (int pid{ -1 }; is_valid_decimal_whole_number(user_cmd[1], pid)) {
          ...
          if (pid == player.pid) {
            ...
            if (check_ip_address_validity(player.ip_address, ip_key) && protected_ip_addresses.contains(player.ip_address)) {
              ...
              print_colored_text(app_handles.hwnd_re_messages_data, re_msg.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
              main_app.get_tinyrcon_dict()["{REASON}"] = std::move(reason);
              const string message{ format("^3You have successfully executed ^5{} ^3on player ({}^3)\n", user_cmd[0], get_player_information(pid, false, "Unprotected IP address")) };
              print_colored_text(app_handles.hwnd_re_messages_data, message.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
              ...
              main_app.get_connection_manager_for_messages().process_and_send_message("unprotect-ipaddress", format("{}\\{}\\{}", main_app.get_username(), player.ip_address, player.reason), true, main_app.get_tiny_rcon_server_ip_address(), main_app.get_tiny_rcon_server_port(), false);
            }
          }
        } else {
          const auto &ip_address = user_cmd[1];
          if (protected_ip_addresses.contains(ip_address)) {
            ...
            if (!is_ip_address_already_protected) {
              ...
              print_colored_text(app_handles.hwnd_re_messages_data, re_msg.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
              const string message{ format(
                "^3You have successfully executed ^5{} ^3on player ({}^3)\n", user_cmd[0], get_player_information_for_player(player_offline, "Unprotected IP address")) };
              print_colored_text(app_handles.hwnd_re_messages_data, message.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
              main_app.get_connection_manager_for_messages().process_and_send_message("unprotect-ipaddress", format("{}\\{}\\{}", main_app.get_username(), player_offline.ip_address, player_offline.reason), true, main_app.get_tiny_rcon_server_ip_address(), main_app.get_tiny_rcon_server_port(), false);
            }
          }
        }
      }
      ...
      if (check_if_user_provided_argument_is_valid_for_specified_command(
        ...
        if (int pid{ -1 }; is_valid_decimal_whole_number(user_cmd[1], pid)) {
          ...
          if (pid == player.pid) {
            unsigned long ip_key{};
            if (check_ip_address_validity(player.ip_address, ip_key)) {
              const string error_message{ format("^1{} ^3is not a valid IP address.\n", player.ip_address) };
            ...
            if (!protected_ip_address_ranges.contains(narrow_ip_address_range)) {
              ...
              print_colored_text(app_handles.hwnd_re_messages_data, re_msg.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
              main_app.get_tinyrcon_dict()["{REASON}"] = std::move(reason);
              const string message{ format("^3You have successfully executed ^5{} ^3on player ({}^3)\n", user_cmd[0], get_player_information(pid, false, "Protected IP address range")) };
              print_colored_text(app_handles.hwnd_re_messages_data, message.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
              ...
              main_app.get_connection_manager_for_messages().process_and_send_message("protect-ipaddressrange", format("{}\\{}\\{}", main_app.get_username(), narrow_ip_address_range, player.reason), true, main_app.get_tiny_rcon_server_ip_address(), main_app.get_tiny_rcon_server_port(), false);
            }
          }
        } else {
          ...
          if (!protected_ip_address_ranges.contains(ip_address_range)) {
            ...
            for (size_t i{}; i < main_app.get_current_game_server().get_number_of_players(); ++i) {
              auto &player = players_data[i];
              const string narrow_ip_address_range{ get_narrow_ip_address_range_for_specified_ip_address(player.ip_address) };
              const string wide_ip_address_range{ get_narrow_ip_address_range_for_specified_ip_address(player.ip_address) };
              if (ip_address_range == narrow_ip_address_range || ip_address_range == wide_ip_address_range) {
                main_app.get_tinyrcon_dict()["{ADMINNAME}"] = main_app.get_username();
                main_app.get_tinyrcon_dict()["{PLAYERNAME}"] = player.player_name;
                specify_reason_for_player_pid(player.pid, reason);
                const string re_msg{ format("^2You have successfully protected ^1IP address range: {}\n", ip_address_range) };
                print_colored_text(app_handles.hwnd_re_messages_data, re_msg.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
                main_app.get_tinyrcon_dict()["{REASON}"] = std::move(reason);
                const string message{ format("^3You have successfully executed ^5{} ^3on player ({}^3)\n", user_cmd[0], get_player_information(player.pid, false, "Protected IP address range")) };
                print_colored_text(app_handles.hwnd_re_messages_data, message.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
                string command{ main_app.get_user_defined_protect_ip_address_range_message() };
                if (!main_app.get_is_disable_automatic_kick_messages()) {
                  build_tiny_rcon_message(command);
                ...
                main_app.get_connection_manager_for_messages().process_and_send_message("protect-ipaddressrange", format("{}\\{}\\{}", main_app.get_username(), ip_address_range, player.reason), true, main_app.get_tiny_rcon_server_ip_address(), main_app.get_tiny_rcon_server_port(), false);
                break;
              }
            }
            ...
            if (!is_ip_address_range_already_protected) {
              ...
              print_colored_text(app_handles.hwnd_re_messages_data, re_msg.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
              const string message{ format(
                "^3You have successfully executed ^5{} ^3on player ({}^3)\n", user_cmd[0], get_player_information_for_player(player_offline, "Protected IP address range")) };
              print_colored_text(app_handles.hwnd_re_messages_data, message.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
              main_app.get_connection_manager_for_messages().process_and_send_message("protect-ipaddressrange", format("{}\\{}\\{}", main_app.get_username(), ip_address_range, player_offline.reason), true, main_app.get_tiny_rcon_server_ip_address(), main_app.get_tiny_rcon_server_port(), false);
            }
          }
        }
      }
        ...
        if (int pid{ -1 }; is_valid_decimal_whole_number(user_cmd[1], pid)) {
          ...
          if (pid == player.pid) {
            ...
            if (protected_ip_address_ranges.contains(narrow_ip_address_range)) {
              ...
              main_app.get_tinyrcon_dict()["{PLAYERNAME}"] = player.player_name;
              specify_reason_for_player_pid(player.pid, reason);
              const string re_msg{ format("^2You have successfully removed protected ^1IP address range: {}\n", narrow_ip_address_range) };
              print_colored_text(app_handles.hwnd_re_messages_data, re_msg.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
              main_app.get_tinyrcon_dict()["{REASON}"] = std::move(reason);
              const string message{ format("^3You have successfully executed ^5{} ^3on player ({}^3)\n", user_cmd[0], get_player_information(pid, false, "Unprotected IP address range")) };
              print_colored_text(app_handles.hwnd_re_messages_data, message.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
              ...
              main_app.get_connection_manager_for_messages().process_and_send_message("unprotect-ipaddressrange", format("{}\\{}\\{}", main_app.get_username(), narrow_ip_address_range, player.reason), true, main_app.get_tiny_rcon_server_ip_address(), main_app.get_tiny_rcon_server_port(), false);
            }
          }
        } else {
          const auto &ip_address_range = user_cmd[1];
          if (protected_ip_address_ranges.contains(ip_address_range)) {
            ...
            for (size_t i{}; i < main_app.get_current_game_server().get_number_of_players(); ++i) {
              ...
              if (ip_address_range == narrow_ip_address_range || ip_address_range == wide_ip_address_range) {
                ...
                print_colored_text(app_handles.hwnd_re_messages_data, re_msg.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
                main_app.get_tinyrcon_dict()["{REASON}"] = std::move(reason);
                const string message{ format("^3You have successfully executed ^5{} ^3on player ({}^3)\n", user_cmd[0], get_player_information(player.pid, false, "Unprotected IP address range")) };
                print_colored_text(app_handles.hwnd_re_messages_data, message.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
                string command{ main_app.get_user_defined_unprotect_ip_address_range_message() };
                if (!main_app.get_is_disable_automatic_kick_messages()) {
                  build_tiny_rcon_message(command);
                ...
                is_ip_address_range_already_protected = true;
                main_app.get_connection_manager_for_messages().process_and_send_message("unprotect-ipaddressrange", format("{}\\{}\\{}", main_app.get_username(), ip_address_range, player.reason), true, main_app.get_tiny_rcon_server_ip_address(), main_app.get_tiny_rcon_server_port(), false);
                break;
              }
            }
            ...
            if (!is_ip_address_range_already_protected) {
              ...
              print_colored_text(app_handles.hwnd_re_messages_data, re_msg.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
              const string message{ format(
                "^3You have successfully executed ^5{} ^3on player ({}^3)\n", user_cmd[0], get_player_information_for_player(player_offline, "Unprotected IP address range")) };
              print_colored_text(app_handles.hwnd_re_messages_data, message.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
              main_app.get_connection_manager_for_messages().process_and_send_message("unprotect-ipaddressrange", format("{}\\{}\\{}", main_app.get_username(), ip_address_range, player_offline.reason), true, main_app.get_tiny_rcon_server_ip_address(), main_app.get_tiny_rcon_server_port(), false);
            }
          }
        }
      }
      ...
      if (int pid{ -1 }; is_valid_decimal_whole_number(user_cmd[1], pid)) {
        ...
        if (pid == player.pid) {
          if (!protected_cities.contains(player.city)) {
            ...
            main_app.get_connection_manager_for_messages().process_and_send_message("protect-city", format("{}\\{}\\{}", main_app.get_username(), player.city, player.reason), true, main_app.get_tiny_rcon_server_ip_address(), main_app.get_tiny_rcon_server_port(), false);
          }
        }
      } else {
        ...
        if (!protected_cities.contains(city)) {
          ...
          for (size_t i{}; i < main_app.get_current_game_server().get_number_of_players(); ++i) {
            ...
            if (player.city == city) {
              ...
              const string re_msg{ format("^2You have successfully protected ^1city: {}\n", user_cmd[1]) };
              print_colored_text(app_handles.hwnd_re_messages_data, re_msg.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
              const string message{ format("^3You have successfully executed ^5{} ^3on player ({}^3)\n", user_cmd[0], get_player_information(player.pid, false, "Protected city")) };
              print_colored_text(app_handles.hwnd_re_messages_data, message.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
              string command{ main_app.get_user_defined_protect_city_message() };
              if (!main_app.get_is_disable_automatic_kick_messages()) {
                build_tiny_rcon_message(command);
              ...
              main_app.get_connection_manager_for_messages().process_and_send_message("protect-city", format("{}\\{}\\n/a", main_app.get_username(), player.city), true, main_app.get_tiny_rcon_server_ip_address(), main_app.get_tiny_rcon_server_port(), false);
              break;
            }
          }
          ...
          if (!is_city_already_protected) {
            ...
            print_colored_text(app_handles.hwnd_re_messages_data, re_msg.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
            const string message{ format(
              "^3You have successfully executed ^5{} ^3on player ({}^3)\n", user_cmd[0], get_player_information_for_player(player_offline, "Protected city")) };
            print_colored_text(app_handles.hwnd_re_messages_data, message.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
            main_app.get_connection_manager_for_messages().process_and_send_message("protect-city", format("{}\\{}\\n/a", main_app.get_username(), city), true, main_app.get_tiny_rcon_server_ip_address(), main_app.get_tiny_rcon_server_port(), false);
          }
        }
      }
    }
      ...
      if (int pid{ -1 }; is_valid_decimal_whole_number(user_cmd[1], pid)) {
        ...
        if (pid == player.pid) {
          if (protected_cities.contains(player.city)) {
            ...
            print_colored_text(app_handles.hwnd_re_messages_data, re_msg.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
            const string message{ format("^3You have successfully executed ^5{} ^3on player ({}^3)\n", user_cmd[0], get_player_information(pid, false, "Unprotected city")) };
            print_colored_text(app_handles.hwnd_re_messages_data, message.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
            ...
            main_app.get_connection_manager_for_messages().process_and_send_message("unprotect-city", format("{}\\{}\\n/a", main_app.get_username(), player.city), true, main_app.get_tiny_rcon_server_ip_address(), main_app.get_tiny_rcon_server_port(), false);
          }
        }
      } else {
        const string city{ stl::helper::trim(str_join(cbegin(user_cmd) + 1, cend(user_cmd), " ")) };
        if (protected_cities.contains(city)) {
          ...
          for (size_t i{}; i < main_app.get_current_game_server().get_number_of_players(); ++i) {
            ...
            if (player.city == city) {
              ...
              const string re_msg{ format("^2You have successfully removed protected ^1city: {}\n", city) };
              print_colored_text(app_handles.hwnd_re_messages_data, re_msg.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
              const string message{ format("^3You have successfully executed ^5{} ^3on player ({}^3)\n", user_cmd[0], get_player_information(player.pid, false, "Unprotected city")) };
              print_colored_text(app_handles.hwnd_re_messages_data, message.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
              string command{ main_app.get_user_defined_unprotect_city_message() };
              if (!main_app.get_is_disable_automatic_kick_messages()) {
                build_tiny_rcon_message(command);
      ...
      if (int pid{ -1 }; is_valid_decimal_whole_number(user_cmd[1], pid)) {
        ...
        if (pid == player.pid) {
          if (!protected_countries.contains(player.country_name)) {
            ...
            print_colored_text(app_handles.hwnd_re_messages_data, re_msg.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
            const string message{ format("^3You have successfully executed ^5{} ^3on player ({}^3)\n", user_cmd[0], get_player_information(pid, false, "Protected country")) };
            print_colored_text(app_handles.hwnd_re_messages_data, message.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
            ...
            main_app.get_connection_manager_for_messages().process_and_send_message("protect-country", format("{}\\{}\\n/a", main_app.get_username(), player.country_name), true, main_app.get_tiny_rcon_server_ip_address(), main_app.get_tiny_rcon_server_port(), false);
          }
        }
      } else {
        ...
        if (!protected_countries.contains(country_name)) {
          ...
          for (size_t i{}; i < main_app.get_current_game_server().get_number_of_players(); ++i) {
            ...
            if (player.country_name == country_name) {
              ...
              const string re_msg{ format("^2You have successfully protected ^1country: {}\n", user_cmd[1]) };
              print_colored_text(app_handles.hwnd_re_messages_data, re_msg.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
              const string message{ format("^3You have successfully executed ^5{} ^3on player ({}^3)\n", user_cmd[0], get_player_information(player.pid, false, "Protected country")) };
              print_colored_text(app_handles.hwnd_re_messages_data, message.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
              string command{ main_app.get_user_defined_protect_country_message() };
              if (!main_app.get_is_disable_automatic_kick_messages()) {
                build_tiny_rcon_message(command);
              ...
              main_app.get_connection_manager_for_messages().process_and_send_message("protect-country", format("{}\\{}\\n/a", main_app.get_username(), country_name), true, main_app.get_tiny_rcon_server_ip_address(), main_app.get_tiny_rcon_server_port(), false);
              break;
            }
          }
          ...
          if (!is_country_already_protected) {
            ...
            print_colored_text(app_handles.hwnd_re_messages_data, re_msg.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
            const string message{ format(
              "^3You have successfully executed ^5{} ^3on player ({}^3)\n", user_cmd[0], get_player_information_for_player(player_offline, "Protected country")) };
            print_colored_text(app_handles.hwnd_re_messages_data, message.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
            main_app.get_connection_manager_for_messages().process_and_send_message("protect-country", format("{}\\{}\\n/a", main_app.get_username(), country_name), true, main_app.get_tiny_rcon_server_ip_address(), main_app.get_tiny_rcon_server_port(), false);
          }
        }
      ...
      if (int pid{ -1 }; is_valid_decimal_whole_number(user_cmd[1], pid)) {
        ...
        if (pid == player.pid) {
          if (protected_countries.contains(player.country_name)) {
            ...
            main_app.get_tinyrcon_dict()["{PLAYERNAME}"] = player.player_name;
            main_app.get_tinyrcon_dict()["{COUNTRY_NAME}"] = player.country_name;
            const string re_msg{ format("^2You have successfully removed protected ^1country: {}\n", player.country_name) };
            print_colored_text(app_handles.hwnd_re_messages_data, re_msg.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
            main_app.get_tinyrcon_dict()["{REASON}"] = reason;
            const string message{ format("^3You have successfully executed ^5{} ^3on player ({}^3)\n", user_cmd[0], get_player_information(pid, false, "Unprotected country")) };
            print_colored_text(app_handles.hwnd_re_messages_data, message.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
            ...
            main_app.get_connection_manager_for_messages().process_and_send_message("unprotect-country", format("{}\\{}\\{}", main_app.get_username(), player.country_name, reason), true, main_app.get_tiny_rcon_server_ip_address(), main_app.get_tiny_rcon_server_port(), false);
          }
        }
      } else {
        const string country_name{ stl::helper::trim(str_join(cbegin(user_cmd) + 1, cend(user_cmd), " ")) };
        if (protected_countries.contains(country_name)) {
          ...
          for (size_t i{}; i < main_app.get_current_game_server().get_number_of_players(); ++i) {
            ...
            if (player.country_name == country_name) {
              ...
              const string re_msg{ format("^2You have successfully removed protected ^1country: {}\n", country_name) };
              print_colored_text(app_handles.hwnd_re_messages_data, re_msg.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
              const string message{ format("^3You have successfully executed ^5{} ^3on player ({}^3)\n", user_cmd[0], get_player_information(player.pid, false, "Unprotected country")) };
              print_colored_text(app_handles.hwnd_re_messages_data, message.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
              string command{ main_app.get_user_defined_unprotect_country_message() };
              if (!main_app.get_is_disable_automatic_kick_messages()) {
                build_tiny_rcon_message(command);
              ...
              is_city_already_protected = true;
              main_app.get_connection_manager_for_messages().process_and_send_message("unprotect-country", format("{}\\{}\\{}", main_app.get_username(), country_name, reason), true, main_app.get_tiny_rcon_server_ip_address(), main_app.get_tiny_rcon_server_port(), false);
              break;
            }
          }
          ...
          if (!is_city_already_protected) {
            ...
            const string re_msg{ format("^2You have successfully removed protected ^1country: {}\n", country_name) };
            print_colored_text(app_handles.hwnd_re_messages_data, re_msg.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
            main_app.get_connection_manager_for_messages().process_and_send_message("unprotect-country", format("{}\\{}\\n/a", main_app.get_username(), country_name), true, main_app.get_tiny_rcon_server_ip_address(), main_app.get_tiny_rcon_server_port(), false);
          }
        }
      }
    }
      ...
      if (check_if_user_provided_argument_is_valid_for_specified_command(
        ...
        if (int pid{ -1 }; is_valid_decimal_whole_number(user_cmd[1], pid)) {
          ...
          if (pid == player.pid) {
            ...
            unsigned long ip_key{};
            const string ip_address_range{ get_narrow_ip_address_range_for_specified_ip_address(player.ip_address) };
            if (check_ip_address_validity(player.ip_address, ip_key) && !main_app.get_current_game_server().get_banned_ip_address_ranges_map().contains(ip_address_range)) {
              main_app.get_tinyrcon_dict()["{ADMINNAME}"] = main_app.get_username();
              main_app.get_tinyrcon_dict()["{IP_ADDRESS_RANGE}"] = ip_address_range;
              player.banned_start_time = get_current_time_stamp();
              const string datetimeinfo{ get_date_and_time_for_time_t("{DD}.{MM}.{Y} {hh}:{mm}", player.banned_start_time) };
              ...
              main_app.get_current_game_server().get_banned_ip_address_ranges_map().emplace(ip_address_range, player);
          ...
          if (main_app.get_current_game_server().get_protected_ip_address_ranges().contains(user_cmd[1])) {
            const string error_msg{ format("^3Cannot execute command ^1{} ^3on player whose ^1IP address range ^3is ^1protected.\n^5Remove the protected ^1IP address range ^5first by using the ^1!unprotectiprange {} ^5command.", user_cmd[0], user_cmd[1]) };
            print_colored_text(app_handles.hwnd_re_messages_data, error_msg.c_str());
            return;
          }
          if (!main_app.get_current_game_server().get_banned_ip_address_ranges_map().contains(user_cmd[1])) {
            ...
            player.reason = reason;
            player.banned_by_user_name = main_app.get_username();
            main_app.get_current_game_server().get_banned_ip_address_ranges_map().emplace(user_cmd[1], player);
            main_app.get_current_game_server().get_banned_ip_address_ranges_vector().emplace_back(player);
            string command{ main_app.get_user_defined_ip_address_range_ban_message() };
            if (!main_app.get_is_disable_automatic_kick_messages()) {
              build_tiny_rcon_message(command);
            ...
            current_user->no_of_iprangebans++;
            save_current_user_data_to_json_file(main_app.get_user_data_file_path());
            main_app.get_connection_manager_for_messages().process_and_send_message("add-iprangeban", format(R"({}\0\{}\{}\{}\{})", user_cmd[1], player.player_name, player.banned_date_time, player.reason, main_app.get_username()), true, main_app.get_tiny_rcon_server_ip_address(), main_app.get_tiny_rcon_server_port(), false);
          }
        }
      }
    }
  ...
  main_app.add_command_handler({ "!bn", "!banname" }, [](const vector<string> &user_cmd) {
    ...
    if (user_cmd.size() >= 2 && !user_cmd[1].empty()) {
      ...
      if (int pid{ -1 }; pid_needle_pos != string::npos && is_valid_decimal_whole_number(user_cmd[1].substr(pid_needle_pos + len("pid:")), pid)) {
        auto &player = main_app.get_current_game_server().get_player_data(pid);
        if (pid == player.pid) {
          string error_msg;
          if (check_if_player_is_protected(player, user_cmd[0].c_str(), error_msg)) {
          ...
          if (!banned_player_name.empty() && !main_app.get_current_game_server().get_banned_names_map().contains(banned_player_name)) {
            main_app.get_tinyrcon_dict()["{ADMINNAME}"] = main_app.get_username();
            ...
            string reason{ remove_disallowed_characters_in_string(user_cmd.size() > 2 ? str_join(cbegin(user_cmd) + 2, cend(user_cmd), " ") : "not specified") };
            stl::helper::trim_in_place(reason);
            const string re_msg{ format("^2You have successfully banned player name: ^1{}\n", banned_player_name) };
            print_colored_text(app_handles.hwnd_re_messages_data, re_msg.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
            main_app.get_tinyrcon_dict()["{REASON}"] = reason;
            const string message{ format("^2You have successfully executed ^1{} ^2on player ({}^3)\n", user_cmd[0], get_player_information(pid, false, "Banned player name")) };
            print_colored_text(app_handles.hwnd_re_messages_data, message.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
            ...
            save_current_user_data_to_json_file(main_app.get_user_data_file_path());
            main_app.get_connection_manager_for_messages().process_and_send_message("add-nameban", format(R"({}\{}\{}\{}\{}\{})", player.ip_address, player.guid_key, banned_player_name, player.banned_date_time, player.reason, player.banned_by_user_name), true, main_app.get_tiny_rcon_server_ip_address(), main_app.get_tiny_rcon_server_port(), false);
            kick_player(pid, command);
          }
        }
      } else {
        ...
        if (!main_app.get_current_game_server().get_banned_names_map().contains(banned_player_name)) {
          ...
          current_user->no_of_namebans++;
          save_current_user_data_to_json_file(main_app.get_user_data_file_path());
          main_app.get_connection_manager_for_messages().process_and_send_message("add-nameban", format(R"({}\0\{}\{}\{}\{})", p.ip_address, banned_player_name, p.banned_date_time, p.reason, main_app.get_username()), true, main_app.get_tiny_rcon_server_ip_address(), main_app.get_tiny_rcon_server_port(), false);
        }
      }
    }
  });
  ...
  main_app.add_command_handler({ "!ubn", "!unbanname" }, [](const vector<string> &user_cmd) {
    ...
    if (user_cmd.size() >= 2 && !user_cmd[1].empty()) {
      ...
      auto status = remove_permanently_banned_player_name(pd, main_app.get_current_game_server().get_banned_names_vector(), main_app.get_current_game_server().get_banned_names_map());

      if (status) {
        const string re_msg{ format(
          "^2You have successfully removed previously banned ^1player name: ^5{}\n", banned_player_name) };
        print_colored_text(app_handles.hwnd_re_messages_data, re_msg.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
        main_app.get_connection_manager_for_messages().process_and_send_message("remove-nameban", format(R"({}\{}\{}\{}\{}\{}\{})", pd.ip_address, pd.guid_key, banned_player_name, pd.banned_date_time, pd.reason, pd.banned_by_user_name, main_app.get_username()), true, main_app.get_tiny_rcon_server_ip_address(), main_app.get_tiny_rcon_server_port(), false);
      } else {
        const string re_msg{ format("^3Provided player name (^1{}^3) hasn't been ^1banned ^3yet!\n", banned_player_name) };
        print_colored_text(app_handles.hwnd_re_messages_data,
          re_msg.c_str(),
  ...
  main_app.add_command_handler({ "!stats" }, [](const vector<string> &) {
    ...
    string ex_msg{ "^1Exception ^3thrown from ^1command handler ^3for the ^1!stats ^3user command." };
    stack_trace_element ste{
      app_handles.hwnd_re_messages_data,
      std::move(ex_msg)
    };

    main_app.get_connection_manager_for_messages().process_and_send_message("request-welcome-message", format("{}\\{}\\{}", main_app.get_username(), main_app.get_user_ip_address(), get_current_time_stamp()), true, main_app.get_tiny_rcon_server_ip_address(), main_app.get_tiny_rcon_server_port(), false);

    {
      lock_guard lk{ welcome_messages_mutex_for_admins };
      main_app.get_welcome_messages_to_admins().emplace(me);
    }
  });
  ...
  main_app.add_command_handler({ "!bancity" }, [](const vector<string> &user_cmd) {
    ...
    if (user_cmd.size() > 1 && !user_cmd[1].empty()) {
      ...
      if (!main_app.get_current_game_server().get_banned_cities_set().contains(banned_city)) {
        ...
        save_banned_entries_to_file(main_app.get_banned_cities_file_path(), main_app.get_current_game_server().get_banned_cities_set());
        ...
        print_colored_text(app_handles.hwnd_re_messages_data, format("^3City ^1{} ^3is already banned.", banned_city).c_str());
      }
  ...
  main_app.add_command_handler({ "!unbancity" }, [](const vector<string> &user_cmd) {
    ...
    if (user_cmd.size() > 1 && !user_cmd[1].empty()) {
      ...
      if (main_app.get_current_game_server().get_banned_cities_set().contains(banned_city_to_unban)) {
        ...
        print_colored_text(app_handles.hwnd_re_messages_data, format("^2City ^1{} ^2is not banned.", banned_city_to_unban).c_str());
      }
    }
  });
  ...
  main_app.add_command_handler({ "!bancountry" }, [](const vector<string> &user_cmd) {
    ...
    if (user_cmd.size() > 1 && !user_cmd[1].empty()) {
      ...
      if (!main_app.get_current_game_server().get_banned_countries_set().contains(banned_country)) {
        ...
        print_colored_text(app_handles.hwnd_re_messages_data, format("^3Country ^1{} ^3is already banned.", banned_country).c_str());
      }
  ...
  main_app.add_command_handler({ "!unbancountry" }, [](const vector<string> &user_cmd) {
    ...
    if (user_cmd.size() > 1 && !user_cmd[1].empty()) {
      ...
      if (main_app.get_current_game_server().get_banned_countries_set().contains(banned_country_to_unban)) {
        ...
        save_banned_entries_to_file(main_app.get_banned_countries_file_path(), main_app.get_current_game_server().get_banned_countries_set());
        const string message{ format(
        ...
        print_colored_text(app_handles.hwnd_re_messages_data, format("^2Country ^1{} ^2is not banned.", banned_country_to_unban).c_str());
      }
    }
  });
  ...
  main_app.add_command_handler({ "!remoteadmins" }, [](const vector<string> &) {
    is_display_private_admins_data_event.store(true);
  });
  ...
  main_app.add_command_handler({ "!s" }, [](const vector<string> &user_cmd) {
    if (user_cmd.size() >= 2 && !user_cmd[1].empty()) {
      process_topplayers_request(format("!s\\{}", str_join(cbegin(user_cmd) + 1, cend(user_cmd), " ")));
    }
  });

  main_app.add_command_handler({ "!savestats" }, [](const vector<string> &user_cmd) {
    const string stats_dat_file_path{ user_cmd.size() >= 2 && !user_cmd[1].empty() ? format("data\\{}.dat", user_cmd[1]) : "data\\player_stats.dat"s };
    save_players_stats_data(stats_dat_file_path.c_str(), main_app.get_stats_data().get_scores_vector(), main_app.get_stats_data().get_scores_map());
  });

  main_app.add_command_handler({ "!rsfp", "!remove_stats_for_player" }, [](const vector<string> &user_cmd) {
    if (user_cmd.size() >= 2 && !user_cmd[1].empty()) {
      const string player_name_index{ get_cleaned_user_name(str_join(cbegin(user_cmd) + 1, cend(user_cmd), " ")) };
      const string status_msg{ remove_stats_for_player_name(player_name_index) ? format("^2Successfully removed all player stats data for ^2'^7{}^2'!", user_cmd[1])
                                                                               : format("^3Could not find any player stats data for ^3'^7{}^3'!", user_cmd[1]) };
      print_colored_text(app_handles.hwnd_re_messages_data, status_msg.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
    }
  });
  ...
  main_app.add_command_handler({ "tp", "!tp" }, [](const vector<string> &user_cmd) {
    ...
    process_topplayers_request(format("!tp\\{}", number_of_top_players_to_display));
  });
  ...
  main_app.add_command_handler({ "tpy", "!tpy" }, [](const vector<string> &user_cmd) {
    ...
    process_topplayers_request(format("!tpy\\{}", number_of_top_players_to_display));
  });
  ...
  main_app.add_command_handler({ "tpm", "!tpm" }, [](const vector<string> &user_cmd) {
    ...
    process_topplayers_request(format("!tpm\\{}", number_of_top_players_to_display));
  });
  ...
  main_app.add_command_handler({ "tpd", "!tpd" }, [](const vector<string> &user_cmd) {
    ...
    process_topplayers_request(format("!tpd\\{}", number_of_top_players_to_display));
  });
  ...
  main_app.add_command_handler({ "!spec" }, [](const vector<string> &user_cmd) {
    ...
    if (is_player_being_spectated.load()) {
      ...
      if (selected_player.pid != -1) {
        ...
        print_colored_text(app_handles.hwnd_re_messages_data, warning_message.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes, true, true, true);
      }
  ...
  main_app.add_command_handler({ "!ub", "!unban" }, [](const vector<string> &user_cmd) {
    ...
    if (user_cmd.size() >= 2 && !user_cmd[1].empty()) {
      ...
      if (!check_ip_address_validity(user_cmd[1], ip_key) && !check_if_user_provided_argument_is_valid_for_specified_command("!unban", user_cmd[1])) {
        const string re_msg{ format("^5Provided IP address ^1{} ^5is not a valid IP address or there isn't a banned player entry whose ^1serial no. ^5is equal to the provided number: ^1{}!\n", user_cmd[1], user_cmd[1]) };
        print_colored_text(app_handles.hwnd_re_messages_data, re_msg.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
      } else {
        string message;
        int no{ -1 };
        if (!is_valid_decimal_whole_number(user_cmd[1], no)) {
          ...
          if (status) {
            ...
            const string private_msg{ format("^1Admin ^7{} ^2has successfully removed ^1temporarily banned IP address: ^5{}\n ^3Player name: ^7{} ^5| ^3geoinfo: ^1{}, {} ^5| ^3Date of tempban: ^1{}\n ^3Tempban duration: ^1{} hours ^5|^5Reason: ^1{} ^5| ^3Banned by: ^1{}\n", main_app.get_username(), user_cmd[1], player.player_name, player.country_name, player.city, player.banned_date_time, player.ban_duration_in_hours, player.reason, player.banned_by_user_name) };
            const string inform_msg{ format("{}\\{}\\{}", main_app.get_username(), message, private_msg) };
            main_app.add_message_to_queue(message_t("inform-message", inform_msg, true));
          } else {
            const string re_msg{ format("^1Admin ^7{} ^3failed to remove ^1temporarily banned IP address: ^5{}\n^1Reason: ^3Provided IP address (^1{}^3) hasn't been ^1temporarily banned ^3yet.\n", main_app.get_username(), user_cmd[1], user_cmd[1]) };
            const string inform_msg{ format("{}\\{}", main_app.get_username(), re_msg) };
            main_app.add_message_to_queue(message_t("inform-message", inform_msg, true));
          }
        }
        ...
        if (status) {
          main_app.get_connection_manager_for_messages().process_and_send_message("remove-ipban", format(R"({}\{}\{}\{}\{}\{}\{})", player.ip_address, player.guid_key, player.player_name, player.banned_date_time, player.reason, player.banned_by_user_name, main_app.get_username()), true, main_app.get_tiny_rcon_server_ip_address(), main_app.get_tiny_rcon_server_port(), false);
          const string private_msg{ format(
            "^1Admin ^7{} ^2has successfully removed ^1permanently banned IP address: ^5{} ^2for player name: ^7{}\n", main_app.get_username(), ip_address, player.player_name) };
          const string inform_msg{ format("{}\\{}\\{}", main_app.get_username(), message, private_msg) };
          main_app.add_message_to_queue(message_t("inform-message", inform_msg, true));
        } else {
          const string re_msg{ format("^1Admin ^7{} ^3failed to remove ^1permanently banned IP address: ^5{}\n^1Reason: ^3Provided IP address (^1{}^3) hasn't been ^1permanently banned ^3yet.\n", main_app.get_username(), user_cmd[1], user_cmd[1]) };
          const string inform_msg{ format("{}\\{}", main_app.get_username(), re_msg) };
          main_app.add_message_to_queue(message_t("inform-message", inform_msg, true));
        }
      }
  ...
  main_app.add_command_handler({ "!ubr", "!unbanrange" }, [](const vector<string> &user_cmd) {
    ...
    if (user_cmd.size() >= 2 && !user_cmd[1].empty()) {
      ...
      if (!check_if_user_provided_argument_is_valid_for_specified_command("!unbanrange", user_cmd[1])) {
        ...
        auto status = remove_permanently_banned_ip_address_range(pd, main_app.get_current_game_server().get_banned_ip_address_ranges_vector(), main_app.get_current_game_server().get_banned_ip_address_ranges_map());

        if (status) {
          const string re_msg{ format(
            "^2You have successfully removed previously banned ^1IP address range: ^5{}\n", ip_address_range) };
          print_colored_text(app_handles.hwnd_re_messages_data, re_msg.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
          main_app.get_connection_manager_for_messages().process_and_send_message("remove-iprangeban", format(R"({}\{}\{}\{}\{}\{}\{})", ip_address_range, pd.guid_key, pd.player_name, pd.banned_date_time, pd.reason, pd.banned_by_user_name, main_app.get_username()), true, main_app.get_tiny_rcon_server_ip_address(), main_app.get_tiny_rcon_server_port(), false);
        } else {
  ...
  main_app.add_command_handler({ "!c", "!cp" }, [](const vector<string> &user_cmd) {
    ...
    const auto result = check_if_specified_server_ip_port_and_rcon_password_are_valid(ip_address.c_str(), port_number, main_app.get_current_game_server().get_rcon_password().c_str());
  ...
  main_app.add_command_handler({ "!rc" }, [](const vector<string> &user_cmd) {
    ...
    if (user_cmd.size() >= 2 && !user_cmd[1].empty()) {
      ...
      if (!validate_admin_and_show_missing_admin_privileges_message(me, false)) return;
      string rcon_command{ stl::helper::to_lower_case(stl::helper::trim(stl::helper::str_join(cbegin(user_cmd) + 1, cend(user_cmd), " "))) };
      if (rcon_command.find("rcon_password") != string::npos || rcon_command.find("sv_privatepassword") != string::npos) {
        const string warning_msg{ format("^3Cannot send disallowed rcon command ^1'{}' ^3to currently viewed game server!\n", rcon_command) };
      ...
      const bool is_hide_information_message{ rcon_command.find("--silent") != string::npos || rcon_command.find("/S") != string::npos };
      if (!is_hide_information_message) {
        ...
        stl::helper::trim_in_place(rcon_command);
      }

      string reply;
      main_app.get_connection_manager().send_and_receive_rcon_data(
        rcon_command.c_str(), reply, main_app.get_current_game_server().get_server_ip_address().c_str(), main_app.get_current_game_server().get_server_port(), main_app.get_current_game_server().get_rcon_password().c_str(), main_app.get_current_game_server(), true, false);

      if (!is_hide_information_message && reply.find("js_mute_guid") == string::npos && reply.find("js_unmute_guid") == string::npos) {
        print_colored_text(app_handles.hwnd_re_messages_data, reply.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes, true, true);
      }

      if (str_contains(reply, "g_muted_guids", 0U, true)) {
        ...
        if (!guid_segments.empty()) {
          ...
          for (auto &guid_key : guid_segments) {
            ...
            if (!main_app.get_muted_players_map().contains(ip_address)) {
              ...
              pd.banned_start_time = get_current_time_stamp();
              const string date_time_info{ get_date_and_time_for_time_t("{DD}.{MM}.{Y} {hh}:{mm}", pd.banned_start_time) };
  ...
  main_app.add_command_handler({ "!restart" }, [](const vector<string> &) {
    for (const auto &u : main_app.get_remote_players()) {
      unsigned long ip_key{};
      if (check_ip_address_validity(u->ip_address, ip_key)) {
        const string info_message{ format("^5Sent a request to ^3player ^7{} ^5with IP address ^1{} ^5from ^1{} ^5to restart their ^5Tiny^6Rcon ^5program.\n", u->user_name, u->ip_address, u->geo_information) };
        print_colored_text(app_handles.hwnd_re_messages_data, info_message.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes, true, true, false);
        main_app.get_connection_manager_for_remote_messages().process_and_send_message("restart-tinyrcon", "^3Restarting your ^5Tiny^6Rcon ^3client program.", true, u);
      }
    }
  });
  ...
  main_app.add_command_handler({ "say", "!say" }, [](const vector<string> &rcon_cmd) {
    ...
    if (rcon_cmd.size() >= 2) {
      ...
      if (!command.empty() && '!' == command[0])
        command.erase(0, 1);
      string reply;
      main_app.get_connection_manager().send_and_receive_rcon_data(command.c_str(), reply, main_app.get_current_game_server().get_server_ip_address().c_str(), main_app.get_current_game_server().get_server_port(), main_app.get_current_game_server().get_rcon_password().c_str(), main_app.get_current_game_server(), false, false);
    }
  });
  ...
  main_app.add_command_handler({ "!pt" }, [](const vector<string> &rcon_cmd) {
    if (rcon_cmd.size() >= 2) {
      const string message{ format("{}\\{}", main_app.get_username(), stl::helper::str_join(cbegin(rcon_cmd) + 1, cend(rcon_cmd), " ")) };
      main_app.get_remote_message_handler("remote-public-message")(main_app.get_username(), get_current_time_stamp(), message, true, main_app.get_user_ip_address());
    }
  });

  main_app.add_command_handler({ "!py" }, [](const vector<string> &rcon_cmd) {
    if (!validate_admin_and_show_missing_admin_privileges_message(me, false)) return;
    if (rcon_cmd.size() >= 3) {
      string send_to{ rcon_cmd[1] };
      remove_disallowed_characters_in_string(send_to);
  ...
  main_app.add_command_handler({ "clientkick" }, [](const vector<string> &rcon_cmd) {
    ...
    if (rcon_cmd.size() > 1 && !rcon_cmd[1].empty()) {
      ...
      if (check_if_user_provided_argument_is_valid_for_specified_command(
        ...
        string protected_player_msg;
        if (check_if_player_is_protected(get_player_data_for_pid(pid), rcon_cmd[0].c_str(), protected_player_msg)) {
          print_colored_text(app_handles.hwnd_re_messages_data, protected_player_msg.c_str());
          replace_br_with_new_line(protected_player_msg);
          const string inform_msg{ format("{}\\{}", main_app.get_username(), protected_player_msg) };
          main_app.add_message_to_queue(message_t("inform-message", inform_msg, true));
        ...
        main_app.get_tinyrcon_dict()["{ADMINNAME}"] = main_app.get_username();
        main_app.get_tinyrcon_dict()["{PLAYERNAME}"] = get_player_name_for_pid(pid);
        string reason{ remove_disallowed_characters_in_string(rcon_cmd.size() > 2 ? str_join(cbegin(rcon_cmd) + 2, cend(rcon_cmd), " ") : "not specified") };
        stl::helper::trim_in_place(reason);
        specify_reason_for_player_pid(pid, reason);
        main_app.get_tinyrcon_dict()["{REASON}"] = reason;
      ...
      if (rcon_commands_help.contains(rcon_cmd[0])) {
        print_colored_text(app_handles.hwnd_re_messages_data, rcon_commands_help.at(rcon_cmd[0]).c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
        print_colored_text(app_handles.hwnd_re_messages_data, "\n", is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::no);
      }
    }
  });
      ...
      if (check_if_user_provided_argument_is_valid_for_specified_command(
        ...
        if (rcon_cmd.size() > 2) {
          if (is_valid_decimal_whole_number(rcon_cmd[2], number)) {
            temp_ban_duration = number > 0 && number <= 9999 ? number : 24;
            if (rcon_cmd.size() > 3) {
              reason = remove_disallowed_characters_in_string(str_join(cbegin(rcon_cmd) + 3, cend(rcon_cmd), " "));
              stl::helper::trim_in_place(reason);
            }
          } else {
            reason = remove_disallowed_characters_in_string(str_join(cbegin(rcon_cmd) + 2, cend(rcon_cmd), " "));
            stl::helper::trim_in_place(reason);
          }
        }
        main_app.get_tinyrcon_dict()["{REASON}"] = reason;
    ...
    if (rcon_cmd.size() > 1 && !rcon_cmd[1].empty()) {
      ...
      for (size_t i{}; i < main_app.get_current_game_server().get_number_of_players(); ++i) {
        ...
        if (rcon_cmd[1] == player_name) {
          string protected_player_msg;
          if (check_if_player_is_protected(get_player_data_for_pid(player.pid), rcon_cmd[0].c_str(), protected_player_msg)) {
            print_colored_text(app_handles.hwnd_re_messages_data, protected_player_msg.c_str());
            replace_br_with_new_line(protected_player_msg);
            const string inform_msg{ format("{}\\{}", main_app.get_username(), protected_player_msg) };
            main_app.add_message_to_queue(message_t("inform-message", inform_msg, true));
          ...
          string reply;
          main_app.get_connection_manager().send_and_receive_rcon_data(command.c_str(), reply, main_app.get_current_game_server().get_server_ip_address().c_str(), main_app.get_current_game_server().get_server_port(), main_app.get_current_game_server().get_rcon_password().c_str(), main_app.get_current_game_server(), true, false);
  ...
  main_app.add_command_handler({ "unknown-rcon" }, [](const vector<string> &rcon_cmd) {
    ...
    print_colored_text(app_handles.hwnd_re_messages_data, re_msg.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes, true, false, false);
    string reply;
    main_app.get_connection_manager().send_and_receive_rcon_data(command.c_str(), reply, main_app.get_current_game_server().get_server_ip_address().c_str(), main_app.get_current_game_server().get_server_port(), main_app.get_current_game_server().get_rcon_password().c_str(), main_app.get_current_game_server(), true, false);
    print_colored_text(app_handles.hwnd_re_messages_data, reply.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes, true, false, false);
  });
  ...
  main_app.add_message_handler("confirm-login", [](const string &, const time_t, const string &data, bool) {
    ...
    if (parts.size() >= 3) {
      ...
      me->no_of_logins++;
      me->last_login_time_stamp = get_current_time_stamp();
      me->is_admin = true;
      me->is_logged_in = true;
  ...
  main_app.add_message_handler("welcome-message", [](const string &, const time_t, const string &data, bool) {
    ...
    if (parts.size() >= 3) {
      ...
      print_colored_text(app_handles.hwnd_re_messages_data, parts[2].c_str(), is_append_message_to_richedit_control::yes, is_log_message::no, is_log_datetime::yes, true, true, false);
    }
  });

  main_app.add_message_handler("receive-welcome-message", [](const string &, const time_t, const string &data, bool) {
    {
      lock_guard lk{ welcome_messages_mutex_for_players };
      if (!main_app.get_welcome_messages_to_players().empty()) {
        auto &current_player = main_app.get_welcome_messages_to_players().front();
        string player_data{ data };
        if (size_t start_pos{}, end_pos{}; ((start_pos = player_data.find("^1admin ")) != string::npos) && ((end_pos = player_data.find('\n', start_pos + 1)) != string::npos)) {
          player_data.replace(start_pos, end_pos - start_pos, format("^7{}", current_player->user_name));
        }
        main_app.add_remote_message_to_queue(server_message_t("receive-welcome-message-player", player_data, current_player, true));
        main_app.get_welcome_messages_to_players().pop();
      }
    }

    {
      lock_guard lk{ welcome_messages_mutex_for_admins };
      if (!main_app.get_welcome_messages_to_admins().empty()) {
        auto &current_admin = main_app.get_welcome_messages_to_admins().front();
        if (current_admin.get() == me.get()) {
          print_colored_text(app_handles.hwnd_re_messages_data, data.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes, true, true, false);
        } else {
          string admin_data{ data };
          if (size_t start_pos{}, end_pos{}; ((start_pos = admin_data.find("^1admin ")) != string::npos) && ((end_pos = admin_data.find('\n', start_pos + 1)) != string::npos)) {
            admin_data.replace(start_pos, end_pos - start_pos, format("^1private admin ^7{}", current_admin->user_name));
          }
          print_colored_text(app_handles.hwnd_re_messages_data, admin_data.c_str(), is_append_message_to_richedit_control::yes, is_log_message::no, is_log_datetime::yes, true, true, false);
        }
        main_app.get_welcome_messages_to_admins().pop();
      }
    }
  });
  ...
  main_app.add_message_handler("heartbeat", [](const string &, const time_t, const string &, bool) {
    ...
    main_app.get_connection_manager_for_messages().process_and_send_message("heartbeat", format("{}\\{}", main_app.get_username(), current_ts), true, main_app.get_tiny_rcon_server_ip_address(), main_app.get_tiny_rcon_server_port(), false);
  });
  ...
  main_app.add_remote_message_handler("remote-public-message", [](const string &, const time_t, const string &data, bool, const string &) {
    auto parts = stl::helper::str_split(data, "\\", nullptr, split_on_whole_needle_t::yes, ignore_empty_string_t::no);
    if (parts.size() >= 2) {
      const string public_msg{ format("^7{} ^7sent all ^1private admins ^7a ^1public message:\n^5Public message: ^7{}\n", parts[0], parts[1]) };
      print_colored_text(app_handles.hwnd_re_messages_data, public_msg.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes, true, true, false);
    }
  });
  ...
  main_app.add_remote_message_handler("remote-private-message", [](const string &, const time_t, const string &data, bool, const string &) {
    auto parts = stl::helper::str_split(data, "\\", nullptr, split_on_whole_needle_t::yes, ignore_empty_string_t::no);
    // for (auto &part : parts) stl::helper::trim_in_place(part);
    if (parts.size() >= 3) {
      const string private_msg{ format("^1Private admin ^7{} ^7sent you a ^1private message.\n^5Private message: ^7{}\n", parts[0], parts[2]) };
      print_colored_text(app_handles.hwnd_re_messages_data, private_msg.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes, true, false, false);
    }
  });

  // request-topplayers
  main_app.add_remote_message_handler("request-topplayers", [](const string &, const time_t, const string &data, bool, const string &) {
    process_topplayers_request(data);
  });

  main_app.add_message_handler("inform-login", [](const string &, const time_t, const string &message, bool) {
    print_colored_text(app_handles.hwnd_re_messages_data, message.c_str());

    size_t start_pos = message.find("^2Player name: ");
    if (start_pos != string::npos) {

      start_pos += len("^2Player name: ");
      const size_t end_pos{ message.find('\n', start_pos) };
      if (end_pos != string::npos) {
        const string player_name{ message.substr(start_pos, end_pos - start_pos) };
        if (!player_name.empty()) {
          const string player_stats_information{ get_player_stats_information_message(player_name) };
          if (!player_stats_information.empty()) {
            print_colored_text(app_handles.hwnd_re_messages_data, player_stats_information.c_str());
          }
        }
      }
    }
  ...
  main_app.add_remote_message_handler("request-tempbans-player", [](const string &user, const time_t, const string &, bool, const string &sender_ip) {
    const auto &temp_banned_players =
      main_app.get_game_servers()[0].get_temp_banned_ip_addresses_vector();
    const size_t start_index{ 100U <= temp_banned_players.size() ? temp_banned_players.size() - 100U : 0U };
    const auto &sender_user = get_user_for_specified_username_and_ip_address(user, sender_ip);
    sender_user->is_logged_in = true;
    // print_colored_text(app_handles.hwnd_re_messages_data, format("^5Received request from ^7{} ^5to send them the ^1last {} temporary ban ^5entries.", sender_user->user_name, temp_banned_players.size() < 100U ? temp_banned_players.size() : 100U).c_str());
    // size_t message_length{};
    ostringstream oss;
    for (size_t i{ start_index }; i < temp_banned_players.size(); ++i) {
      const auto &tb_player = temp_banned_players[i];
      const string data_line{ format("{}\\{}\\{}\\{}\\{}\\{}\\{}\n", tb_player.ip_address, tb_player.player_name, tb_player.banned_date_time, tb_player.banned_start_time, tb_player.ban_duration_in_hours, remove_disallowed_characters_in_string(tb_player.reason), tb_player.banned_by_user_name) };
      oss << data_line;
      /*message_length += data_line.length();
      if (message_length > 1300) {
        main_app.add_remote_message_to_queue(server_message_t("receive-tempbans-player", oss.str(), sender_user));
        oss.str(string{});
        oss.clear();
        message_length = 0U;
      }*/
    ...
    main_app.add_remote_message_to_queue(server_message_t("receive-tempbans-player", last_data_line, sender_user));
    // }
  });

  // request-ipaddressbans-player
  main_app.add_remote_message_handler("request-ipaddressbans-player", [](const string &user, const time_t, const string &, bool, const string &sender_ip) {
    const auto &banned_ip_addresses =
      main_app.get_game_servers()[0].get_banned_ip_addresses_vector();
    const size_t start_index{ 100U <= banned_ip_addresses.size() ? banned_ip_addresses.size() - 100U : 0U };
    const auto &sender_user = get_user_for_specified_username_and_ip_address(user, sender_ip);
    sender_user->is_logged_in = true;
    // print_colored_text(app_handles.hwnd_re_messages_data, format("^5Received request from ^7{} ^5to send them the ^1last {} IP address ban ^5entries.", sender_user->user_name, banned_ip_addresses.size() < 100U ? banned_ip_addresses.size() : 100U).c_str());
    // size_t message_length{};
    ostringstream oss;
    for (size_t i{ start_index }; i < banned_ip_addresses.size(); ++i) {
      const auto &banned_player = banned_ip_addresses[i];
      const string data_line{ format("{}\\{}\\{}\\{}\\{}\\{}\n", banned_player.ip_address, banned_player.guid_key, banned_player.player_name, banned_player.banned_date_time, remove_disallowed_characters_in_string(banned_player.reason), banned_player.banned_by_user_name) };
      oss << data_line;
      /*message_length += data_line.length();
      if (message_length > 1300) {
        main_app.add_remote_message_to_queue(server_message_t("receive-ipaddressbans-player", oss.str(), sender_user));
        oss.str(string{});
        oss.clear();
        message_length = 0U;
      }*/
    ...
    // if (message_length > 0U) {
    const string last_data_line{ oss.str() };
    main_app.add_remote_message_to_queue(server_message_t("receive-ipaddressbans-player", last_data_line, sender_user));
    // }
  });

  // request-ipaddressrangebans-player
  main_app.add_remote_message_handler("request-ipaddressrangebans-player", [](const string &user, const time_t, const string &, bool, const string &sender_ip) {
    const auto &banned_ip_address_ranges =
      main_app.get_game_servers()[0].get_banned_ip_address_ranges_vector();
    const size_t start_index{ 100U <= banned_ip_address_ranges.size() ? banned_ip_address_ranges.size() - 100U : 0U };
    const auto &sender_user = get_user_for_specified_username_and_ip_address(user, sender_ip);
    sender_user->is_logged_in = true;
    // print_colored_text(app_handles.hwnd_re_messages_data, format("^5Received request from ^7{} ^5to send them the ^1last {} IP address range ban ^5entries.", sender_user->user_name, banned_ip_address_ranges.size() < 100U ? banned_ip_address_ranges.size() : 100U).c_str());
    // size_t message_length{};
    ostringstream oss;
    for (size_t i{ start_index }; i < banned_ip_address_ranges.size(); ++i) {
      ...
      const string data_line{ format("{}\\{}\\{}\\{}\\{}\\{}\n", ip, banned_player.guid_key, banned_player.player_name, banned_player.banned_date_time, remove_disallowed_characters_in_string(banned_player.reason), banned_player.banned_by_user_name) };
      oss << data_line;
      /*message_length += data_line.length();
      if (message_length > 1300) {
        main_app.add_remote_message_to_queue(server_message_t("receive-ipaddressrangebans-player", oss.str(), sender_user));
        oss.str(string{});
        oss.clear();
        message_length = 0U;
      }*/
    ...
    // if (message_length > 0U) {
    const string last_data_line{ oss.str() };
    main_app.add_remote_message_to_queue(server_message_t("receive-ipaddressrangebans-player", last_data_line, sender_user));
    // }
  });

  // request-namebans-player
  main_app.add_remote_message_handler("request-namebans-player", [](const string &user, const time_t, const string &, bool, const string &sender_ip) {
    const auto &banned_names =
      main_app.get_game_servers()[0].get_banned_names_vector();
    const size_t start_index{ 100U <= banned_names.size() ? banned_names.size() - 100U : 0U };
    const auto &sender_user = get_user_for_specified_username_and_ip_address(user, sender_ip);
    sender_user->is_logged_in = true;
    // print_colored_text(app_handles.hwnd_re_messages_data, format("^5Received request from ^7{} ^5to send them the ^1last {} banned name ^5entries.", sender_user->user_name, banned_names.size() < 100U ? banned_names.size() : 100U).c_str());
    // size_t message_length{};
    ostringstream oss;
    for (size_t i{ start_index }; i < banned_names.size(); ++i) {
      ...
      /*message_length += data_line.length();
      if (message_length > 1300) {
        main_app.add_remote_message_to_queue(server_message_t("receive-namebans-player", oss.str(), sender_user));
        oss.str(string{});
        oss.clear();
        message_length = 0U;
      }*/
    ...
    // if (message_length > 0U) {
    const string last_data_line{ oss.str() };
    main_app.add_remote_message_to_queue(server_message_t("receive-namebans-player", last_data_line, sender_user));
    // }
  });

  // request-citybans-player
  main_app.add_remote_message_handler("request-citybans-player", [](const string &user, const time_t, const string &, bool, const string &sender_ip) {
    const auto &banned_cities =
      main_app.get_game_servers()[0].get_banned_cities_set();
    const auto &sender_user = get_user_for_specified_username_and_ip_address(user, sender_ip);
    sender_user->is_logged_in = true;
    // print_colored_text(app_handles.hwnd_re_messages_data, format("^5Received request from ^7{} ^5to send them ^1all of the banned city ^5entries.", sender_user->user_name).c_str());
    // size_t message_length{};
    ostringstream oss;
    for (const auto &banned_city : banned_cities) {
      ...
      /*message_length += data_line.length();
      if (message_length > 1300) {
        main_app.add_remote_message_to_queue(server_message_t("receive-citybans-player", oss.str(), sender_user));
        oss.str(string{});
        oss.clear();
        message_length = 0U;
      }*/
    ...
    // if (message_length > 0U) {
    const string last_data_line{ oss.str() };
    main_app.add_remote_message_to_queue(server_message_t("receive-citybans-player", last_data_line, sender_user));
    // }
  });

  // request-countrybans-player
  main_app.add_remote_message_handler("request-countrybans-player", [](const string &user, const time_t, const string &, bool, const string &sender_ip) {
    const auto &banned_countries =
      main_app.get_game_servers()[0].get_banned_countries_set();
    const auto &sender_user = get_user_for_specified_username_and_ip_address(user, sender_ip);
    sender_user->is_logged_in = true;
    // print_colored_text(app_handles.hwnd_re_messages_data, format("^5Received request from ^7{} ^5to send them all ^1all of the banned country ^5entries.", sender_user->user_name).c_str());
    // size_t message_length{};
    ostringstream oss;
    for (const auto &banned_country : banned_countries) {
      ...
      /*message_length += data_line.length();
      if (message_length > 1300) {
        main_app.add_remote_message_to_queue(server_message_t("receive-countrybans-player", oss.str(), sender_user));
        oss.str(string{});
        oss.clear();
        message_length = 0U;
      }*/
    ...
    // if (message_length > 0U) {
    const string last_data_line{ oss.str() };
    main_app.add_remote_message_to_queue(server_message_t("receive-countrybans-player", last_data_line, sender_user));
    // }
  });
  ...
  main_app.add_message_handler("upload-bans", [](const string &, const time_t, const string &data, bool) {
    ...
    if (files_to_compress.contains(file_name_key)) {
      ...
      if (upload_file_to_ftp_server(main_app.get_tiny_rcon_server_ip_address().c_str(), main_app.get_tiny_rcon_ftp_server_username().c_str(), main_app.get_tiny_rcon_ftp_server_password().c_str(), files_to_compress.at(file_name_key).c_str(), data.c_str())) {
        /*const string message{ format("^2Sent ^7{}^2's ^1{}.txt file ^2to Tiny^6Rcon ^5server ^2for updating.", main_app.get_username(), file_name_key) };
        print_colored_text(app_handles.hwnd_re_messages_data, message.c_str());*/
      }
    }
  });
  ...
  main_app.add_message_handler("receive-bans-compressed", [](const string &, const time_t, const string &data, bool) {
    ...
    if (download_file_from_ftp_server(main_app.get_tiny_rcon_server_ip_address().c_str(), main_app.get_tiny_rcon_ftp_server_username().c_str(), main_app.get_tiny_rcon_ftp_server_password().c_str(), updated_bans_7z_file_path.c_str(), data.c_str())) {

      const string extract_to_path{ format("{}data\\", main_app.get_current_working_directory()) };

      auto [success, error_msg] = extract_7z_file_to_specified_path(updated_bans_7z_file_path.c_str(), extract_to_path.c_str());
      DeleteFileA(updated_bans_7z_file_path.c_str());

      if (success) {

        parse_tempbans_data_file(file_names_to_file_paths.at("tempbans").c_str(), main_app.get_current_game_server().get_temp_banned_ip_addresses_vector(), main_app.get_current_game_server().get_temp_banned_ip_addresses_map());

        parse_banned_ip_addresses_file(file_names_to_file_paths.at("bans").c_str(), main_app.get_current_game_server().get_banned_ip_addresses_vector(), main_app.get_current_game_server().get_banned_ip_addresses_map());

        parse_banned_ip_address_ranges_file(file_names_to_file_paths.at("ip_range_bans").c_str(), main_app.get_current_game_server().get_banned_ip_address_ranges_vector(), main_app.get_current_game_server().get_banned_ip_address_ranges_map());

        parse_banned_cities_file(file_names_to_file_paths.at("banned_cities").c_str(), main_app.get_current_game_server().get_banned_cities_set());

        parse_banned_countries_file(file_names_to_file_paths.at("banned_countries").c_str(), main_app.get_current_game_server().get_banned_countries_set());

        parse_banned_names_file(file_names_to_file_paths.at("banned_names").c_str(), main_app.get_current_game_server().get_banned_names_vector(), main_app.get_current_game_server().get_banned_names_map());
  ...
  main_app.add_message_handler("receive-bans", [](const string &, const time_t, const string &data, bool) {
    ...
    if (files_to_compress.contains(file_name_key)) {
      ...
      if (download_file_from_ftp_server(main_app.get_tiny_rcon_server_ip_address().c_str(), main_app.get_tiny_rcon_ftp_server_username().c_str(), main_app.get_tiny_rcon_ftp_server_password().c_str(), files_to_compress.at(file_name_key).c_str(), data.c_str())) {
        if (file_name_key == "tempbans") {
          ...
          parse_banned_cities_file(main_app.get_banned_cities_file_path(), main_app.get_current_game_server().get_banned_cities_set());
          set_operation_completed_flag_for_admins(ban_entry_type::banned_cities);
  ...
  main_app.add_remote_message_handler("request-protectedipaddresses-player", [](const string &user, const time_t, const string &, bool, const string &sender_ip) {
    const auto &protected_ip_addresses = main_app.get_current_game_server().get_protected_ip_addresses();
    ostringstream oss;
    ...
    oss << flush;
    const auto &sender_user = get_user_for_specified_username_and_ip_address(user, sender_ip);
    main_app.add_remote_message_to_queue(server_message_t("receive-protectedipaddresses-player", oss.str(), sender_user, true));
  });

  main_app.add_remote_message_handler("request-protectedipaddressranges-player", [](const string &user, const time_t, const string &, bool, const string &sender_ip) {
    ...
    oss << flush;
    const auto &sender_user = get_user_for_specified_username_and_ip_address(user, sender_ip);
    main_app.add_remote_message_to_queue(server_message_t("receive-protectedipaddressranges-player", oss.str(), sender_user, true));
  });

  main_app.add_remote_message_handler("request-protectedcities-player", [](const string &user, const time_t, const string &, bool, const string &sender_ip) {
    const auto &protected_ip_cities = main_app.get_current_game_server().get_protected_cities();
    ostringstream oss;
    ...
    oss << flush;
    const auto &sender_user = get_user_for_specified_username_and_ip_address(user, sender_ip);
    main_app.add_remote_message_to_queue(server_message_t("receive-protectedcities-player", oss.str(), sender_user, true));
  });

  main_app.add_remote_message_handler("request-protectedcountries-player", [](const string &user, const time_t, const string &, bool, const string &sender_ip) {
    const auto &protected_ip_countries = main_app.get_current_game_server().get_protected_countries();
    ostringstream oss;
    ...
    oss << flush;
    const auto &sender_user = get_user_for_specified_username_and_ip_address(user, sender_ip);
    main_app.add_remote_message_to_queue(server_message_t("receive-protectedcountries-player", oss.str(), sender_user, true));
  });

  //****************************************************************************************************************


  main_app.add_message_handler("request-protectedipaddresses", [](const string &, const time_t, const string &, bool) {
    const string message{ "^3Requesting ^1protected IP addresses^3, ^1IP address ranges^3, ^1cities ^3and ^1countries ^3from ^5Tiny^6Rcon ^5server." };
    print_colored_text(app_handles.hwnd_re_messages_data, message.c_str());
  });

  main_app.add_message_handler("receive-protectedipaddresses", [](const string &, const time_t, const string &data, bool) {
  ...
  main_app.add_message_handler("request-protectedipaddressranges", [](const string &, const time_t, const string &, bool) {
  });

  main_app.add_message_handler("receive-protectedipaddressranges", [](const string &, const time_t, const string &data, bool) {
    auto parts = stl::helper::str_split(data, "\\", nullptr, split_on_whole_needle_t::yes, ignore_empty_string_t::yes);
  ...
  main_app.add_message_handler("request-protectedcountries", [](const string &, const time_t, const string &, bool) {
  });

  main_app.add_message_handler("receive-protectedcountries", [](const string &, const time_t, const string &data, bool) {
    auto parts = stl::helper::str_split(data, "\\", nullptr, split_on_whole_needle_t::yes, ignore_empty_string_t::yes);
    ...
    save_protected_entries_file(main_app.get_protected_countries_file_path(), protected_countries);
    const string message{ "^2Received ^1protected IP addresses^2, ^1IP address ranges^2, ^1cities ^2and ^1countries ^2from ^5Tiny^6Rcon ^5server." };
    print_colored_text(app_handles.hwnd_re_messages_data, message.c_str());
  });
  ...
  main_app.add_message_handler("add-warning", [](const string &user, const time_t, const string &data, bool) {
    ...
    if (parts.size() >= 5) {
      ...
      convert_guid_key_to_country_name(
        ...
        warned_player);
      convert_guid_key_to_country_name(main_app.get_connection_manager().get_geoip_data(), warned_player.ip_address, warned_player);
      const string msg{ format("^7{} ^3has ^1warned ^3player ^7{} ^3[^5IP address ^1{} ^3| ^5geoinfo: ^1{}, {}\n^5Date/time of warning: ^1{} ^3| ^5Reason of warning: ^1{} ^3| ^5Warned by: ^7{}\n", user, warned_player.player_name, warned_player.ip_address, warned_player.country_name, warned_player.city, warned_player.banned_date_time, warned_player.reason, user) };
  ...
  main_app.add_message_handler("inform-message", [](const string &, const time_t, const string &data, bool) {
    ...
    if (parts.size() >= 3) {
      ...
      bool is_print_to_messages_window{ true };
      if (!parts[2].empty() && parts[0] == "accept") {
        is_print_to_messages_window = parts[2].find("VPN/proxy") == string::npos && parts[2].find("VPN|proxy") == string::npos;

        rcon_say(parts[2], is_print_to_messages_window);
        is_date_and_time_info_displayed = true;
      }
      if (is_print_to_messages_window) {
        for (size_t i{ 3 }; i < parts.size(); ++i) {
          if (!parts[i].empty()) {
            print_colored_text(app_handles.hwnd_re_messages_data, parts[i].c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, !is_date_and_time_info_displayed ? is_log_datetime::yes : is_log_datetime::no, true, true, true);
  ...
  main_app.add_message_handler("protect-ipaddress", [](const string &, const time_t, const string &data, bool) {
    ...
    if (parts.size() >= 3) {
      ...
      if (!main_app.get_current_game_server().get_protected_ip_addresses().contains(parts[1])) {
        ...
        convert_guid_key_to_country_name(main_app.get_connection_manager().get_geoip_data(), parts[1], protected_player);
  ...
  main_app.add_message_handler("protect-ipaddressrange", [](const string &, const time_t, const string &data, bool) {
    ...
    if (parts.size() >= 3) {
      ...
      if (!main_app.get_current_game_server().get_protected_ip_address_ranges().contains(parts[1])) {
        ...
        const string message{ format("^7{} ^2added IP address range ^1{} ^2to the list of protected ^1IP address ranges^2.\n^5Reason: ^1{}", parts[0], parts[1], parts[2]) };
        print_colored_text(app_handles.hwnd_re_messages_data, message.c_str());
  ...
  main_app.add_message_handler("protect-city", [](const string &, const time_t, const string &data, bool) {
    ...
    if (parts.size() >= 3) {
      ...
      if (!main_app.get_current_game_server().get_protected_cities().contains(parts[1])) {
        ...
        const string message{ format("^7{} ^2added city ^1{} ^2to the list of protected ^1cities^2.\n^5Reason: ^1{}", parts[0], parts[1], parts[2]) };
        print_colored_text(app_handles.hwnd_re_messages_data, message.c_str());
  ...
  main_app.add_message_handler("protect-country", [](const string &, const time_t, const string &data, bool) {
    ...
    if (parts.size() >= 3) {
      ...
      if (!main_app.get_current_game_server().get_protected_countries().contains(parts[1])) {
        ...
        const string message{ format("^7{} ^2added country ^1{} ^2to the list of protected ^1countries^2.\n^5Reason: ^1{}", parts[0], parts[1], parts[2]) };
        print_colored_text(app_handles.hwnd_re_messages_data, message.c_str());
  ...
  main_app.add_message_handler("unprotect-ipaddressrange", [](const string &, const time_t, const string &data, bool) {
    auto parts = stl::helper::str_split(data, "\\", nullptr, split_on_whole_needle_t::yes, ignore_empty_string_t::no);
    if (parts.size() >= 3) {
      for (auto &part : parts) {
        stl::helper::trim_in_place(part);
      ...
      if (main_app.get_current_game_server().get_protected_ip_address_ranges().contains(parts[1])) {
        ...
        const string message{ format("^7{} ^3has removed protected IP address range ^1{} ^3from the list of protected ^1IP address ranges^3.\n^5Reason: ^1{}", parts[0], parts[1], parts[2]) };
        print_colored_text(app_handles.hwnd_re_messages_data, message.c_str());
  ...
  main_app.add_message_handler("unprotect-city", [](const string &, const time_t, const string &data, bool) {
    ...
    if (parts.size() >= 3) {
      ...
      if (main_app.get_current_game_server().get_protected_cities().contains(parts[1])) {
        ...
        const string message{ format("^7{} ^3has removed protected city ^1{} ^3from the list of protected ^1cities^3.\n^5Reason: ^1{}", parts[0], parts[1], parts[2]) };
        print_colored_text(app_handles.hwnd_re_messages_data, message.c_str());
  ...
  main_app.add_message_handler("unprotect-country", [](const string &, const time_t, const string &data, bool) {
    ...
    if (parts.size() >= 3) {
      ...
      if (main_app.get_current_game_server().get_protected_countries().contains(parts[1])) {
        ...
        const string message{ format("^7{} ^3has removed protected country ^1{} ^3from the list of protected ^1countries^3.\n^5Reason: ^1{}", parts[0], parts[1], parts[2]) };
        print_colored_text(app_handles.hwnd_re_messages_data, message.c_str());
  ...
  main_app.add_remote_message_handler("add-report", [](const string &, const time_t, const string &data, bool, const string &sender_ip) {
    auto parts = stl::helper::str_split(data, "\\", nullptr, split_on_whole_needle_t::yes, ignore_empty_string_t::no);
    for (auto &part : parts) {
      stl::helper::trim_in_place(part);
    }
    ...
    if (parts.size() >= 5) {
      ...
      if (is_valid_decimal_whole_number(parts[2], pid) && pid >= 0 && pid < 64) {
        ...
        const string date_and_time_of_report{ get_date_and_time_for_time_t("{DD}.{MM}.{Y} {hh}:{mm}", current_ts) };
        ...
        auto &current_user = get_user_for_specified_username_and_ip_address(parts[0], sender_ip);
        ++current_user->no_of_reports;
        main_app.get_connection_manager_for_messages().process_and_send_message("inc-number-of-reports", data, true, main_app.get_tiny_rcon_server_ip_address(), main_app.get_tiny_rcon_server_port(), false);
        string public_msg{ format("^7{} ^7has ^1reported ^7player {}. ^7Reason: ^1{}", parts[0], pd.player_name, pd.reason) };
        rcon_say(public_msg);
        const string private_msg{ format("^7{} ^5has ^1reported ^5player ^7{} ^5[^3PID: ^1{} ^5| ^3Score ^1{} ^5| ^3Ping: ^1{}\n ^3IP: ^1{} ^5| ^3geoinfo: ^1{}, {} ^5| ^3Date/time of report: ^1{} ^5| ^3Reason: ^1{}^5]\n", parts[0], pd.player_name, pd.pid, pd.score, pd.ping, pd.ip_address, pd.country_name, pd.city, pd.banned_date_time, pd.reason) };
        main_app.get_connection_manager_for_messages().process_and_send_message("public-message", format("{}\\{}", current_user->user_name, private_msg), true, main_app.get_tiny_rcon_server_ip_address(), main_app.get_tiny_rcon_server_port(), false);
        for (const auto &u : main_app.get_users()) {
          unsigned long ip_key{};
          if (u.get() != current_user.get() && check_ip_address_validity(u->ip_address, ip_key)) {
            main_app.get_connection_manager_for_remote_messages().process_and_send_message("add-report", data, true, u);
          }
        }
      }
    }
  ...
  main_app.add_remote_message_handler("remove-report", [](const string &username, const time_t, const string &data, bool, const string &) {
    auto parts = stl::helper::str_split(data, "\\", nullptr, split_on_whole_needle_t::yes, ignore_empty_string_t::no);
    for (auto &part : parts) {
      stl::helper::trim_in_place(part);
    }
    if (parts.size() >= 6) {
      string report_key{ parts[0] };
      string message;
      auto [status, temp_banned_player_data]{ remove_reported_player(report_key, message, false) };
      if (status) {
        ...
        temp_banned_player_data.reason = std::move(parts[4]);
        temp_banned_player_data.banned_by_user_name = parts.size() >= 6 ? parts[5] : username;
        const string removed_by{ parts.size() >= 7 ? parts[6] : username };
        convert_guid_key_to_country_name(
          main_app.get_connection_manager().get_geoip_data(),
          temp_banned_player_data.ip_address,
          temp_banned_player_data);
        auto &user_data = main_app.get_user_for_name(username);
        // main_app.get_connection_manager_for_messages().process_and_send_message("inc-number-of-reports", data, true, main_app.get_tiny_rcon_server_ip_address(), main_app.get_tiny_rcon_server_port(), false);
        const string msg{ format("^7{} ^5has removed previously reported player: ^3player name: ^7{} ^5| ^3IP address ^1{}\n ^3geoinfo: ^1{}, {} ^5| ^3Date/time of report: ^1{}\n ^3Reason of report: ^1{} ^5| ^3Reported by: ^7{}\n", removed_by, temp_banned_player_data.player_name, temp_banned_player_data.ip_address, temp_banned_player_data.country_name, temp_banned_player_data.city, temp_banned_player_data.banned_date_time, temp_banned_player_data.reason, temp_banned_player_data.banned_by_user_name) };
        print_colored_text(app_handles.hwnd_re_messages_data, msg.c_str());
        for (const auto &u : main_app.get_users()) {
          unsigned long ip_key{};
          if (u.get() != user_data.get() && check_ip_address_validity(u->ip_address, ip_key)) {
            main_app.get_connection_manager_for_remote_messages().process_and_send_message("remove-report", data, true, u);
          }
        }
      }
    }
  ...
  main_app.add_remote_message_handler("request-reports-player", [](const string &, const time_t, const string &data, bool, const string &sender_ip) {
    auto parts = stl::helper::str_split(data, "\\", nullptr, split_on_whole_needle_t::yes, ignore_empty_string_t::no);
    for (auto &part : parts) {
      stl::helper::trim_in_place(part);
    }
    ...
    if (parts.size() >= 3) {
      unsigned long guid{};
      player pd{};
      if (check_ip_address_validity(sender_ip, guid)) {
        strcpy_s(pd.player_name, std::size(pd.player_name), parts[0].c_str());
        pd.ip_address = sender_ip;
        auto &user = get_user_for_specified_username_and_ip_address(parts[0], sender_ip);
        user->is_logged_in = true;
        // const auto &reported_players = main_app.get_reported_players();
        string output;
        {
          lock_guard lk{ reports_mutex };
          int number{ 10 };
          const size_t no_of_reports_to_send = !is_valid_decimal_whole_number(parts[2], number) ? std::min<size_t>(10u, main_app.get_reported_players().size()) : std::min<size_t>(number, main_app.get_reported_players().size());
          ostringstream oss;
          const auto &reported_players = main_app.get_reported_players();
          for (size_t i{}; i < std::min<size_t>(no_of_reports_to_send, reported_players.size()); ++i) {
            oss << reported_players[i].player_name << '\\' << reported_players[i].ip_address << '\\' << reported_players[i].banned_start_time
                << '\\' << reported_players[i].reason << '\\' << reported_players[i].banned_by_user_name << '\n';
          }
          output = oss.str();
        }

        main_app.add_remote_message_to_queue(server_message_t("receive-reports-player", output, user));
      }
    }
  });
  ...
  main_app.add_message_handler("add-tempban", [](const string &, const time_t, const string &data, bool) {
    ...
    if (parts.size() >= 6 && !main_app.get_current_game_server().get_temp_banned_ip_addresses_map().contains(parts[0])) {
      ...
      print_colored_text(app_handles.hwnd_re_messages_data, msg.c_str());
      add_temporarily_banned_ip_address(temp_banned_player_data, main_app.get_current_game_server().get_temp_banned_ip_addresses_vector(), main_app.get_current_game_server().get_temp_banned_ip_addresses_map());
    }
  });
  ...
  main_app.add_message_handler("add-nameban", [](const string &, const time_t, const string &data, bool) {
    ...
    if (parts.size() >= 5 && !main_app.get_current_game_server().get_banned_names_map().contains(parts[2])) {
      ...
      print_colored_text(app_handles.hwnd_re_messages_data, msg.c_str());
      add_permanently_banned_player_name(pd, main_app.get_current_game_server().get_banned_names_vector(), main_app.get_current_game_server().get_banned_names_map());
    }
  });
  ...
  main_app.add_message_handler("remove-nameban", [](const string &, const time_t, const string &data, bool) {
    ...
    if (parts.size() >= 5 && main_app.get_current_game_server().get_banned_names_map().contains(parts[2])) {
      ...
      print_colored_text(app_handles.hwnd_re_messages_data, msg.c_str());
      remove_permanently_banned_player_name(pd, main_app.get_current_game_server().get_banned_names_vector(), main_app.get_current_game_server().get_banned_names_map());
    }
  });
  ...
  main_app.add_message_handler("add-ipban", [](const string &, const time_t, const string &data, bool) {
    ...
    if (parts.size() >= 5 && !main_app.get_current_game_server().get_banned_ip_addresses_map().contains(parts[0])) {
      ...
      const string msg{ format("^7{} ^5has banned the ^1IP address ^5of player:\n^3Name: ^7{} ^5| ^3IP address: ^1{} ^5| ^3geoinfo: ^1{}, {} ^5| ^3Date/time of ban: ^1{}\n^3Reason of ban: ^1{} ^5| ^3Banned by: ^7{}\n", pd.banned_by_user_name, pd.player_name, pd.ip_address, pd.country_name, pd.city, pd.banned_date_time, pd.reason, pd.banned_by_user_name) };
      print_colored_text(app_handles.hwnd_re_messages_data, msg.c_str());
      add_permanently_banned_ip_address(pd, main_app.get_current_game_server().get_banned_ip_addresses_vector(), main_app.get_current_game_server().get_banned_ip_addresses_map());
    }
  });
  ...
  main_app.add_message_handler("add-muted-ip", [](const string &, const time_t, const string &data, bool) {
    ...
    if (parts.size() >= 5) {
      ...
      const string msg{ format("^7{} ^5has muted the ^1IP address ^5of player:\n^3Name: ^7{} ^5| ^3IP address: ^1{} ^5| ^3geoinfo: ^1{}, {} ^5| ^3Date of getting muted: ^1{}\n^3Reason: ^1{} ^5| ^3Muted by: ^7{}\n", pd.banned_by_user_name, pd.player_name, pd.ip_address, pd.country_name, pd.city, pd.banned_date_time, pd.reason, pd.banned_by_user_name) };
      print_colored_text(app_handles.hwnd_re_messages_data, msg.c_str());
      save_muted_players_data_to_file(main_app.get_muted_players_file_path(), main_app.get_muted_players_map());
    }
  });
  ...
  main_app.add_message_handler("remove-muted-ip", [](const string &, const time_t, const string &data, bool) {
    ...
    if (parts.size() >= 5) {
      ...
      const string msg{ format("^7{} ^5has unmuted ^1IP address {} ^5of player:\n^3Name: ^7{} ^5| ^3geoinfo: ^1{}, {} ^5| ^3Date of getting muted: ^1{}\n^3Reason of mute: ^1{} ^5| ^3Muted by: ^7{}\n", removed_by, pd.ip_address, pd.player_name, pd.country_name, pd.city, pd.banned_date_time, pd.reason, pd.banned_by_user_name) };
      print_colored_text(app_handles.hwnd_re_messages_data, msg.c_str());
      string ip_address{ pd.ip_address };
      string message_about_removal;
  ...
  main_app.add_message_handler("add-iprangeban", [](const string &, const time_t, const string &data, bool) {
    ...
    if (parts.size() >= 5 && !main_app.get_current_game_server().get_banned_ip_address_ranges_map().contains(parts[0])) {
      player pd{};
      pd.ip_address = parts[0];
      strcpy_s(pd.guid_key, std::size(pd.guid_key), parts[1].c_str());
      strcpy_s(pd.player_name, std::size(pd.player_name), parts[2].c_str());
      ...
      print_colored_text(app_handles.hwnd_re_messages_data, msg.c_str());
      add_permanently_banned_ip_address_range(pd, main_app.get_current_game_server().get_banned_ip_address_ranges_vector(), main_app.get_current_game_server().get_banned_ip_address_ranges_map());
  ...
  main_app.add_message_handler("remove-iprangeban", [](const string &, const time_t, const string &data, bool) {
    ...
    if (main_app.get_current_game_server().get_banned_ip_address_ranges_map().contains(parts[0])) {
      ...
      string ip_address{ pd.ip_address };
      string message_about_removal;
      remove_permanently_banned_ip_address_range(pd, main_app.get_current_game_server().get_banned_ip_address_ranges_vector(), main_app.get_current_game_server().get_banned_ip_address_ranges_map());
    }
  });
  ...
  main_app.add_remote_message_handler("request-login", [](const string &, const time_t, const string &data, bool, const string &sender_ip) {
    auto parts = stl::helper::str_split(data, "\\", nullptr, split_on_whole_needle_t::yes, ignore_empty_string_t::no);
    for (auto &part : parts) stl::helper::trim_in_place(part);
    if (parts.size() >= 3) {
      unsigned long guid{};
      ...
      if (check_ip_address_validity(sender_ip, guid)) {
        ...
        pd.ip_address = sender_ip;
        convert_guid_key_to_country_name(main_app.get_connection_manager().get_geoip_data(), pd.ip_address, pd);
        auto &current_user = main_app.get_remote_user_for_name(parts[0], sender_ip);
        current_user->geo_information = format("{}, {}", pd.country_name, pd.city);
        current_user->country_code = pd.country_code;
        current_user->ip_address = sender_ip;
        current_user->no_of_logins++;
        current_user->is_admin = true;
        current_user->is_logged_in = true;
        current_user->last_login_time_stamp = get_current_time_stamp();
        save_tiny_rcon_users_data_to_json_file(main_app.get_remote_users_data_file_path());
        main_app.add_remote_message_to_queue(server_message_t("confirm-login", format("{}\\{}\\{}", current_user->user_name, current_user->ip_address, current_user->no_of_logins), current_user, true));
        string message{ format("^1Private admin ^7{} ^2has logged in to ^1private ^5Tiny^6Rcon ^5server.\n^2Number of logins: ^1{}\n", current_user->user_name, current_user->no_of_logins) };
        if (parts.size() >= 4) {
          message += format("^2Player name: ^7{}\n", parts[3]);
          current_user->player_name = parts[3];
        }
        ...
        {
          lock_guard lk{ welcome_messages_mutex_for_admins };
          main_app.get_welcome_messages_to_admins().emplace(current_user);
        }

        main_app.get_connection_manager_for_messages().process_and_send_message("request-welcome-message", format("{}\\{}\\{}", main_app.get_username(), main_app.get_user_ip_address(), get_current_time_stamp()), true, main_app.get_tiny_rcon_server_ip_address(), main_app.get_tiny_rcon_server_port(), false);
        const string player_stats_information{
          [&]() {
            if (stl::helper::str_contains(current_user->user_name, "hiken", 0u, true))
              return get_player_stats_information_message("Hiken No Ace^3.");
            if (stl::helper::str_contains(current_user->user_name, "aksaz", 0u, true))
              return get_player_stats_information_message("-=TR=-Aksaz/DemirHan");
            if (stl::helper::str_contains(current_user->user_name, "salim", 0u, true))
              return get_player_stats_information_message("-=TR=-Salim/DemirHan");
            if (stl::helper::str_contains(current_user->user_name, "fury", 0u, true))
              return get_player_stats_information_message("^1CPT FURY");
            if (stl::helper::str_contains(current_user->user_name, "staff_magic", 0u, true))
              return get_player_stats_information_message("^1staff_magic(a)");
            return get_player_stats_information_message(current_user->user_name);
          }()
        };

        if (!player_stats_information.empty()) {
          print_colored_text(app_handles.hwnd_re_messages_data, player_stats_information.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes, true, true, false);
        }
      }
    }
  });

  main_app.add_remote_message_handler("request-login-player", [](const string &, const time_t, const string &data, bool, const string &sender_ip) {
    auto parts = stl::helper::str_split(data, "\\", nullptr, split_on_whole_needle_t::yes, ignore_empty_string_t::no);
    for (auto &part : parts) stl::helper::trim_in_place(part);
    if (parts.size() >= 3) {
      unsigned long guid{};
      ...
      if (check_ip_address_validity(sender_ip, guid)) {
        ...
        pd.ip_address = sender_ip;
        convert_guid_key_to_country_name(main_app.get_connection_manager().get_geoip_data(), pd.ip_address, pd);
        auto &current_player = main_app.get_remote_player_for_name(parts[0], sender_ip);
        current_player->geo_information = format("{}, {}", pd.country_name, pd.city);
        current_player->country_code = pd.country_code;
        current_player->ip_address = sender_ip;
        current_player->no_of_logins++;
        current_player->is_admin = false;
        current_player->is_logged_in = true;
        current_player->last_login_time_stamp = get_current_time_stamp();
        save_tiny_rcon_players_data_to_json_file(main_app.get_remote_players_data_file_path());

        main_app.add_remote_message_to_queue(server_message_t("confirm-login-player", format("{}\\{}\\{}", current_player->user_name, current_player->ip_address, current_player->no_of_logins), current_player, true));
        string message{ format("^3Player ^7{} ^3[IP: ^1{} ^3| geoinfo: ^1{}^3] ^2has logged in to ^5Tiny^6Rcon ^5server.\n^2Number of logins: ^1{}\n", current_player->user_name, current_player->ip_address, current_player->geo_information, current_player->no_of_logins) };
        if (parts.size() >= 4) {
          message += format("^2Player name: ^7{}\n", parts[3]);
          current_player->player_name = parts[3];
        }
        if (parts.size() >= 5) {
          message += format("^2Game's version number: ^1{}\n", parts[4]);
        }
        ...
        for (const auto &u : main_app.get_users()) {
          unsigned long ip_key{};
          if (u.get() != current_player.get() && u->is_logged_in && check_ip_address_validity(u->ip_address, ip_key)) {
            main_app.get_connection_manager_for_remote_messages().process_and_send_message("inform-login-player", message, true, u);
          }
        }
        ...
        {
          lock_guard lk{ welcome_messages_mutex_for_players };
          main_app.get_welcome_messages_to_players().emplace(current_player);
        }
        main_app.get_connection_manager_for_messages().process_and_send_message("request-welcome-message", format("{}\\{}\\{}", main_app.get_username(), main_app.get_user_ip_address(), get_current_time_stamp()), true, main_app.get_tiny_rcon_server_ip_address(), main_app.get_tiny_rcon_server_port(), false);
        if (!current_player->player_name.empty()) {
          const string player_stats_information{ get_player_stats_information_message(current_player->player_name) };
          if (!player_stats_information.empty()) {
            print_colored_text(app_handles.hwnd_re_messages_data, player_stats_information.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes, true, true, true);
          }
        }
      }
    }
  ...
  main_app.add_remote_message_handler("request-logout", [](const string &, const time_t, const string &data, bool, const string &sender_ip) {
    auto parts = stl::helper::str_split(data, "\\", nullptr, split_on_whole_needle_t::yes, ignore_empty_string_t::no);
    for (auto &part : parts) stl::helper::trim_in_place(part);
    if (parts.size() >= 2) {
      unsigned long guid{};
      ...
      if (check_ip_address_validity(sender_ip, guid)) {
        ...
        pd.ip_address = sender_ip;
        convert_guid_key_to_country_name(main_app.get_connection_manager().get_geoip_data(), pd.ip_address, pd);
        const auto &current_user = main_app.get_remote_user_for_name(parts[0], sender_ip);
        current_user->geo_information = format("{}, {}", pd.country_name, pd.city);
        current_user->country_code = pd.country_code;
        current_user->is_logged_in = false;
        current_user->last_logout_time_stamp = get_current_time_stamp();
        save_tiny_rcon_users_data_to_json_file(main_app.get_remote_users_data_file_path());
        main_app.get_connection_manager_for_remote_messages().process_and_send_message("confirm-logout", format("{}\\{}\\{}", current_user->user_name, current_user->ip_address, current_user->no_of_logins), true, current_user);
        const string message2{ format("^1Private admin ^7{} ^2has logged out of ^5Tiny^6Rcon ^5server.\n", current_user->user_name) };
        print_colored_text(app_handles.hwnd_re_messages_data, message2.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes, true, true, false);
      }
    }
  });

  main_app.add_remote_message_handler("request-logout-player", [](const string &, const time_t, const string &data, bool, const string &sender_ip) {
    auto parts = stl::helper::str_split(data, "\\", nullptr, split_on_whole_needle_t::yes, ignore_empty_string_t::no);
    for (auto &part : parts) stl::helper::trim_in_place(part);
    if (parts.size() >= 2) {
      unsigned long guid{};
      ...
      if (check_ip_address_validity(sender_ip, guid)) {
        ...
        current_player->geo_information = format("{}, {}", pd.country_name, pd.city);
        current_player->country_code = pd.country_code;
        current_player->is_logged_in = false;
        current_player->last_logout_time_stamp = get_current_time_stamp();
        save_tiny_rcon_players_data_to_json_file(main_app.get_remote_players_data_file_path());
        main_app.get_connection_manager_for_remote_messages().process_and_send_message("confirm-logout-player", format("{}\\{}\\{}", current_player->user_name, current_player->ip_address, current_player->no_of_logins), true, current_player);
        const string message2{ format("^2Player ^7{} ^3[IP: ^1{} ^3| geoinfo: ^1{}^3] ^2has logged out of ^5Tiny^6Rcon ^5server.\n", current_player->user_name, current_player->ip_address, current_player->geo_information) };
        print_colored_text(app_handles.hwnd_re_messages_data, message2.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes, true, true, false);
        unsigned long ip_key{};
        for (const auto &u : main_app.get_users()) {
          if (u.get() != current_player.get() && check_ip_address_validity(u->ip_address, ip_key)) {
            main_app.get_connection_manager_for_remote_messages().process_and_send_message("inform-logout-player", message2, true, u);
          }
        }
        for (const auto &u : main_app.get_remote_players()) {
          if (u.get() != current_player.get() && check_ip_address_validity(u->ip_address, ip_key)) {
  ...
  main_app.add_remote_message_handler("request-admindata", [](const string &user, const time_t, const string &, bool, const string &sender_ip) {
    unsigned long guid{};
    player pd{};
    if (check_ip_address_validity(sender_ip, guid)) {
      strcpy_s(pd.player_name, std::size(pd.player_name), user.c_str());
      pd.ip_address = sender_ip;
      convert_guid_key_to_country_name(main_app.get_connection_manager().get_geoip_data(), pd.ip_address, pd);
      auto &user_data = main_app.get_remote_user_for_name(user, sender_ip);
      user_data->is_logged_in = true;
      const auto &users = main_app.get_remote_users();
      if (!users.empty()) {
        for (size_t i{}; i < users.size(); ++i) {
          const string admin_data{ format(R"({}\{}\{}\{}\{}\{}\{}\{}\{}\{}\{}\{}\{}\{}\{}\{}\{}\{}\{})", users[i]->user_name, (users[i]->is_admin ? "true" : "false"), (users[i]->is_logged_in ? "true" : "false"), (users[i]->is_online ? "true" : "false"), users[i]->ip_address, users[i]->geo_information, users[i]->last_login_time_stamp, users[i]->last_logout_time_stamp, users[i]->no_of_logins, users[i]->no_of_warnings, users[i]->no_of_kicks, users[i]->no_of_tempbans, users[i]->no_of_guidbans, users[i]->no_of_ipbans, users[i]->no_of_iprangebans, users[i]->no_of_citybans, users[i]->no_of_countrybans, users[i]->no_of_namebans, users[i]->no_of_reports) };
          main_app.add_remote_message_to_queue(server_message_t("receive-admindata", admin_data, user_data));
        }
      }
    }
  });

  main_app.add_remote_message_handler("request-admindata-player", [](const string &user, const time_t, const string &, bool, const string &sender_ip) {
    unsigned long guid{};
    player pd{};
    if (check_ip_address_validity(sender_ip, guid)) {
      strcpy_s(pd.player_name, std::size(pd.player_name), user.c_str());
      pd.ip_address = sender_ip;
      // convert_guid_key_to_country_name(main_app.get_connection_manager().get_geoip_data(), pd.ip_address, pd);
      auto &user_data = main_app.get_remote_player_for_name(user, sender_ip);
      user_data->is_logged_in = true;
      const auto &users = main_app.get_users();
      if (!users.empty()) {
        for (size_t i{}; i < users.size(); ++i) {
          const string admin_data{ format(R"({}\{}\{}\{}\{}\{}\{}\{}\{}\{}\{}\{}\{}\{}\{}\{}\{}\{}\{})", users[i]->user_name, (users[i]->is_admin ? "true" : "false"), (users[i]->is_logged_in ? "true" : "false"), (users[i]->is_online ? "true" : "false"), "hidden", users[i]->geo_information, users[i]->last_login_time_stamp, users[i]->last_logout_time_stamp, users[i]->no_of_logins, users[i]->no_of_warnings, users[i]->no_of_kicks, users[i]->no_of_tempbans, users[i]->no_of_guidbans, users[i]->no_of_ipbans, users[i]->no_of_iprangebans, users[i]->no_of_citybans, users[i]->no_of_countrybans, users[i]->no_of_namebans, users[i]->no_of_reports) };
          main_app.add_remote_message_to_queue(server_message_t("receive-admindata-player", admin_data, user_data));
        }
      }
    }
  });

  // main_app.add_remote_message_handler("request-reports", [](const string &user, const time_t, const string &, bool, const string &sender_ip) {
  //   auto parts = stl::helper::str_split(data, "\\", nullptr, split_on_whole_needle_t::yes, ignore_empty_string_t::no);
  //   for (auto &part : parts) {
  //     stl::helper::trim_in_place(part);
  //   }
  ...
  main_app.add_remote_message_handler("rcon-heartbeat", [](const string &, const time_t, const string &data, bool, const string &sender_ip) {
    auto parts = stl::helper::str_split(data, "\\", nullptr, split_on_whole_needle_t::yes, ignore_empty_string_t::no);
    for (auto &part : parts) stl::helper::trim_in_place(part);
    const auto current_ts = get_current_time_stamp();
    if (parts.size() >= 2) {
      if (unsigned long guid{}; check_ip_address_validity(sender_ip, guid)) {
        player pd{};
        const size_t no_of_chars_to_copy{ std::min<size_t>(parts[0].length(), std::size(pd.player_name) - 1) };
        strncpy_s(pd.player_name, std::size(pd.player_name), parts[0].c_str(), no_of_chars_to_copy);
        pd.player_name[no_of_chars_to_copy] = '\0';
        pd.ip_address = sender_ip;
        convert_guid_key_to_country_name(main_app.get_connection_manager().get_geoip_data(), pd.ip_address, pd);
        auto &current_user = main_app.get_remote_user_for_name(parts[0], sender_ip);
        current_user->geo_information = format("{}, {}", pd.country_name, pd.city);
        current_user->country_code = pd.country_code;
        current_user->is_logged_in = true;
        current_user->last_login_time_stamp = current_ts;
      }
    }
  });

  main_app.add_remote_message_handler("rcon-heartbeat-player", [](const string &, const time_t, const string &data, bool, const string &sender_ip) {
    auto parts = stl::helper::str_split(data, "\\", nullptr, split_on_whole_needle_t::yes, ignore_empty_string_t::no);
    for (auto &part : parts) stl::helper::trim_in_place(part);
    const auto current_ts = get_current_time_stamp();
    if (parts.size() >= 2) {
      if (unsigned long guid{}; check_ip_address_validity(sender_ip, guid)) {
        ...
        pd.ip_address = sender_ip;
        convert_guid_key_to_country_name(main_app.get_connection_manager().get_geoip_data(), pd.ip_address, pd);
        auto &current_user = main_app.get_remote_player_for_name(parts[0], sender_ip);
        current_user->geo_information = format("{}, {}", pd.country_name, pd.city);
        current_user->country_code = pd.country_code;
        current_user->is_logged_in = true;
        current_user->last_login_time_stamp = current_ts;
  ...
  main_app.add_remote_message_handler("user-command", [](const string &sender_name, const time_t, const string &data, bool, const string &sender_ip) {
    char user_buffer[512];
    print_colored_text(app_handles.hwnd_re_messages_data, format("^5Received remote user command ^1'{}' ^5from ^7{} ^5({})", data, sender_name, sender_ip).c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes, true, true, false);

    unsigned long guid{};
    player pd{};
    if (check_ip_address_validity(sender_ip, guid)) {
      strcpy_s(pd.player_name, std::size(pd.player_name), sender_name.c_str());
      pd.ip_address = sender_ip;
      convert_guid_key_to_country_name(main_app.get_connection_manager().get_geoip_data(), pd.ip_address, pd);
      /*const string cleaned_username{ get_cleaned_user_name(sender_name) };
      const string cleaned_countryname{ get_cleaned_user_name(pd.country_name) };
      if (main_app.get_allowed_users().contains(cleaned_username) && (main_app.get_allowed_users().at(cleaned_username).first == cleaned_countryname)) {*/

      string user_command{ data };
      if (const size_t sep_pos{ user_command.find('\\') }; sep_pos != string::npos) {
        user_command.erase(begin(user_command) + sep_pos, end(user_command));
      }
      trim_in_place(user_command);

      (void)snprintf(user_buffer, std::size(user_buffer), "%s", user_command.c_str());
      Edit_SetText(app_handles.hwnd_e_user_input, user_buffer);
      get_user_input();

      auto &current_user = main_app.get_remote_user_for_name(sender_name, sender_ip);
      current_user->geo_information = format("{}, {}", pd.country_name, pd.city);
      current_user->country_code = pd.country_code;
      current_user->is_logged_in = true;

      if (user_command.find("!m ") != string::npos || user_command.find("!map ") != string::npos) {
        current_user->no_of_map_changes++;
      } else if (user_command.find("map_restart") != string::npos || user_command.find("fast_restart") != string::npos) {
        current_user->no_of_map_restarts++;
      } else if (user_command.find("!w ") != string::npos || user_command.find("!warn ") != string::npos) {
        current_user->no_of_warnings++;
      } else if (user_command.find("!k ") != string::npos || user_command.find("!kick ") != string::npos) {
        current_user->no_of_kicks++;
      } else if (user_command.find("!tb ") != string::npos || user_command.find("!tempban ") != string::npos) {
        current_user->no_of_tempbans++;
      } else if (user_command.find("!gb ") != string::npos || user_command.find("!globalban ") != string::npos || user_command.find("!banip") != string::npos || user_command.find("!addip") != string::npos) {
        current_user->no_of_ipbans++;
      } else if (user_command.find("!b ") != string::npos || user_command.find("!ban ") != string::npos) {
        current_user->no_of_guidbans++;
      } else if (user_command.find("!br ") != string::npos || user_command.find("!banrange ") != string::npos) {
        current_user->no_of_iprangebans++;
      } else if (user_command.find("!bancity ") != string::npos) {
        current_user->no_of_citybans++;
      } else if (user_command.find("!bancountry ") != string::npos) {
        current_user->no_of_countrybans++;
      } else if (user_command.find("!bn ") != string::npos || user_command.find("!banname ") != string::npos) {
        current_user->no_of_namebans++;
      } else if (user_command.find("!protectip ") != string::npos) {
        current_user->no_of_protected_ip_addresses++;
      } else if (user_command.find("!protectiprange ") != string::npos) {
        current_user->no_of_protected_ip_address_ranges++;
      } else if (user_command.find("!protectcity ") != string::npos) {
        current_user->no_of_protected_cities++;
      } else if (user_command.find("!protectcountry ") != string::npos) {
        current_user->no_of_protected_countries++;
      } else if (user_command.find("!report") != string::npos) {
        current_user->no_of_reports++;
      }
  ...
  main_app.add_remote_message_handler("rcon-command", [](const string &sender_name, const time_t, const string &data, bool, const string &sender_ip) {
    unsigned long guid{};
    player pd{};
    if (check_ip_address_validity(sender_ip, guid)) {
      strcpy_s(pd.player_name, std::size(pd.player_name), sender_name.c_str());
      ...
      string rcon_command{ data };
      if (const size_t sep_pos{ rcon_command.find('\\') }; sep_pos != string::npos) {
        rcon_command.erase(begin(rcon_command) + sep_pos, end(rcon_command));
      }
      trim_in_place(rcon_command);
      stl::helper::to_lower_case_in_place(rcon_command);
      if (rcon_command.starts_with("rcon ")) {
        rcon_command.erase(0, len("rcon "));
        trim_in_place(rcon_command);
      }
      ...
      if (!rcon_status_commands.contains(rcon_command)) {
        print_colored_text(app_handles.hwnd_re_messages_data, format("^5Received remote rcon command ^1'{}' ^5from ^7{} ^5({})", rcon_command, sender_name, sender_ip).c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes, true, true, false);
      }

      if (rcon_status_commands.contains(rcon_command) || rcon_command.find("rcon_password") != string::npos || rcon_command.find("sv_privatepassword") != string::npos) return;

      Edit_SetText(app_handles.hwnd_e_user_input, rcon_command.c_str());
      get_user_input();

      if (rcon_command.find("clientkick ") != string::npos || rcon_command.find("onlykick ") != string::npos || rcon_command.find("kick ") != string::npos) {
        current_user->no_of_kicks++;
      } else if (rcon_command.find("tempbanclient ") != string::npos || rcon_command.find("tempbanuser ") != string::npos) {
        current_user->no_of_tempbans++;
      } else if (rcon_command.find("banclient ") != string::npos || rcon_command.find("banuser ") != string::npos) {
        current_user->no_of_guidbans++;
      }
  ...
  // main_app.add_remote_message_handler("rcon-command-player", [](const string &sender_name, const time_t, const string &data, bool, const string &sender_ip) {
  //   unsigned long guid{};
  //   player pd{};
  //   if (check_ip_address_validity(sender_ip, guid)) {
  //     strcpy_s(pd.player_name, std::size(pd.player_name), sender_name.c_str());
  ...
  //      print_colored_text(app_handles.hwnd_re_messages_data, format("^5Received remote rcon command ^1'{}' ^5from ^7{} ^5({})", rcon_command, sender_name, sender_ip).c_str());
  ...
  const string program_title{ "Private TinyRcon Server | "s + main_app.get_current_game_server().get_server_name() + " | "s + "version: "s + program_version };
  ...
  std::jthread task_thread{
    [&](stop_token st) {
      ...
      parse_tinyrcon_server_users_data(main_app.get_remote_users_data_file_path());
      parse_tinyrcon_server_players_data(main_app.get_remote_players_data_file_path());
      ...
      parse_protected_entries_file(main_app.get_protected_ip_addresses_file_path(), main_app.get_current_game_server().get_protected_ip_addresses());
      parse_protected_entries_file(main_app.get_protected_ip_address_ranges_file_path(), main_app.get_current_game_server().get_protected_ip_address_ranges());
      parse_protected_entries_file(main_app.get_protected_cities_file_path(), main_app.get_current_game_server().get_protected_cities());
          ...
          while (!st.stop_requested() && !is_terminate_program.load() && !main_app.get_remote_replies_queue().empty()) {
            ...
            for (auto &remote_user : main_app.get_remote_users()) {
              if (remote_user->is_logged_in || (current_ts - remote_user->last_login_time_stamp) <= 31) {
                main_app.get_connection_manager_for_remote_messages().process_and_send_message("rcon-reply", main_app.get_remote_replies_queue().front(), true, remote_user);
              }
            }
          ...
          while (!st.stop_requested() && !main_app.get_rcon_status_responses_for_players_queue().empty()) {
            ...
            for (auto &player : main_app.get_remote_players()) {
              if (player->is_logged_in || (current_ts - player->last_login_time_stamp) <= 31) {
                main_app.get_connection_manager_for_remote_messages().process_and_send_message("rcon-reply-player", main_app.get_rcon_status_responses_for_players_queue().front(), true, player);
              }
            }

            main_app.get_rcon_status_responses_for_players_queue().pop();
          }

        } catch (std::exception &ex) {
          const string error_message{ format("^3A specific exception was caught in the 'remote_messaging_thread' thread!\n^1Exception: {}", ex.what()) };
          print_colored_text(app_handles.hwnd_re_messages_data, error_message.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes, true, false, false);
        } catch (...) {
          char buffer[512];
          strerror_s(buffer, GetLastError());
          const string error_message{ format("^3A generic error was caught in the 'remote_messaging_thread' thread!\n^1Exception: {}", buffer) };
          print_colored_text(app_handles.hwnd_re_messages_data, error_message.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes, true, false, false);
        }
      ...
      []() {
        ...
        while (!is_terminate_program.load()) {
          try {
            ...
            const string error_message{ format("^3A specific exception was caught in check_ip_v4_addresses queue's thread!\n^1Exception: {}", ex.what()) };
            print_colored_text(app_handles.hwnd_re_messages_data, error_message.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes, true, false, false);
          } catch (...) {
            char buffer[512];
            strerror_s(buffer, GetLastError());
            const string error_message{ format("^3A generic error was caught in check_ip_v4_addresses queue's thread!\n^1Exception: {}", buffer) };
            print_colored_text(app_handles.hwnd_re_messages_data, error_message.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes, true, false, false);
          }
    ...
    while (GetMessage(&win32_msg, nullptr, 0, 0) > 0) {
      ...
      if (is_main_window_constructed && !is_tinyrcon_initialized) {
        ...
        std::thread th{
          [&]() {
            ...
            try {
              ...
              const string error_message{ format("^3A specific exception was caught in refresh_game_servers_data(HWND) function's thread of execution!\n^1Exception: {}", ex.what()) };
              print_colored_text(app_handles.hwnd_re_messages_data, error_message.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes, true, false, false);
            } catch (...) {
              char buffer[512];
              strerror_s(buffer, GetLastError());
              const string error_message{ format("^3A generic error was caught in refresh_game_servers_data(HWND) function's thread of execution\n^1Exception: {}", buffer) };
              print_colored_text(app_handles.hwnd_re_messages_data, error_message.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes, true, false, false);
            }
          }
        };
        ...
        if (main_app.get_is_connection_settings_valid()) {
          ...
          main_app.add_message_to_queue(message_t("request-login", format(R"({}\{}\{}\{}\{})", me->user_name, me->ip_address, get_current_time_stamp(), main_app.get_player_name(), main_app.get_game_version_number()), true));
          ...
          main_app.add_message_to_queue(message_t{ "request-protectedipaddresses", "Requesting protected IP addresses from TinyRcon server.", true });
          main_app.add_message_to_queue(message_t{ "request-protectedipaddressranges", "Requesting protected IP address ranges from TinyRcon server.", true });
          main_app.add_message_to_queue(message_t{ "request-protectedcities", "Requesting protected cities from TinyRcon server.", true });
          main_app.add_message_to_queue(message_t{ "request-protectedcountries", "Requesting protected countries from TinyRcon server.", true });
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
  ...
  case WM_CONTEXTMENU: {
    if (reinterpret_cast<HWND>(wParam) == app_handles.hwnd_re_messages_data) {
      ...
      if (check_if_selected_cell_indices_are_valid_for_players_grid(selected_player_row, 0)) {
        ...
        if (pid != -1) {
          ...
          if (main_app.get_user_ip_address() != get_player_ip_address_for_pid(pid)) {
            if (!is_player_being_spectated.load()) {
              (void)snprintf(spectate_player_command, std::size(spectate_player_command), "Spectate %s (PID: %d)?", selected_player.player_name, pid);
            } else {
          ...
          (void)snprintf(report_player_command, std::size(report_player_command), "Report %s with reason (PID: %d)?", selected_player.player_name, pid);
      ...
      if (main_app.get_is_connection_settings_valid() && check_if_selected_cell_indices_are_valid_for_players_grid(selected_player_row, 0)) {
        ...
        if (pid != -1) {
          selected_player = get_player_data_for_pid(pid);
          (void)snprintf(info_player_command, std::size(info_player_command), "Display information about %s (PID: %d)?", selected_player.player_name, pid);
          ...
          (void)snprintf(warn_player_command, std::size(warn_player_command), "Warn %s (PID: %d)?", selected_player.player_name, pid);
          ...
          (void)snprintf(kick_player_command, std::size(kick_player_command), "Kick %s (PID: %d)?", selected_player.player_name, pid);
          ...
          (void)snprintf(tempban_player_command, std::size(tempban_player_command), "Temporarily ban %s (PID: %d)?", selected_player.player_name, pid);
          ...
          if (strcmp(selected_player.guid_key, "0") != 0) {
            (void)snprintf(guidban_player_command, std::size(guidban_player_command), "Ban %s's GUID key: %s (PID: %d)?", selected_player.player_name, selected_player.guid_key, pid);
          ...
          (void)snprintf(ipban_player_command, std::size(ipban_player_command), "Ban %s's IP address: %s (PID: %d)?", selected_player.player_name, selected_player.ip_address.c_str(), pid);
          ...
          (void)snprintf(iprangeban_player_command, std::size(iprangeban_player_command), "Ban %s's IP address range: %s (PID: %d)?", selected_player.player_name, ip_address_range.c_str(), pid);
          ...
          if (/*main_app.get_is_automatic_city_kick_enabled() && */ !main_app.get_current_game_server().get_banned_cities_set().contains(selected_player.city)) {
            (void)snprintf(city_ban_player_command, std::size(city_ban_player_command), "Ban %s's city %s (PID: %d)?", selected_player.player_name, selected_player.city, pid);
          ...
          if (/*main_app.get_is_automatic_country_kick_enabled() && */ !main_app.get_current_game_server().get_banned_countries_set().contains(selected_player.country_name)) {
            (void)snprintf(country_ban_player_command, std::size(country_ban_player_command), "Ban %s's country: %s (PID: %d)?", selected_player.player_name, selected_player.country_name, pid);
          ...
          const bool is_player_name_valid{ !player_name.empty() && !main_app.get_current_game_server().get_banned_names_map().contains(player_name) };
  ...
  case WM_TIMER: {
    ...
    if (counter % 5 == 0) {
      ...
      for (auto &u : main_app.get_remote_users()) {
        if (unsigned long guid_key{}; check_ip_address_validity(u->ip_address, guid_key)) {
          u->is_logged_in = false;
          main_app.get_connection_manager_for_remote_messages().process_and_send_message("rcon-heartbeat", format("{}\\{}\\{}", u->user_name, current_ts, u->ip_address), true, u);
        }
      }

      for (auto &u : main_app.get_remote_players()) {
        if (unsigned long guid_key{}; check_ip_address_validity(u->ip_address, guid_key)) {
          u->is_logged_in = false;
          main_app.get_connection_manager_for_remote_messages().process_and_send_message("rcon-heartbeat-player", format("{}\\{}\\{}", u->user_name, current_ts, u->ip_address), true, u);
        }
      }
    }
    ...
    if (counter % 10 == 0) {
      main_app.get_connection_manager_for_messages().process_and_send_message(
        "heartbeat", format("{}\\{}", main_app.get_username(), get_current_time_stamp()), true, main_app.get_tiny_rcon_server_ip_address(), main_app.get_tiny_rcon_server_port(), false);
    }
    ...
    if (counter % 600 == 0) {
      std::thread tasks{
        []() {
          lock_guard lk{ save_players_stats_data_mutex };
    ...
    if (is_display_muted_players_data_event.load()) {
      std::thread display_large_data_set{
        display_muted_players_information, number_of_entries_to_display.load(), false
      };
      ...
      std::thread display_large_data_set{
        display_temporarily_banned_ip_addresses, number_of_entries_to_display.load(), false
      };
      display_large_data_set.detach();
      ...
      std::thread display_large_data_set{ display_permanently_banned_ip_addresses, number_of_entries_to_display.load(), false };
      display_large_data_set.detach();
      is_display_permanently_banned_players_data_event.store(false);
    } else if (is_display_banned_ip_address_ranges_data_event.load()) {
      std::thread display_large_data_set{ display_banned_ip_address_ranges, number_of_entries_to_display.load(), false };
      display_large_data_set.detach();
      is_display_banned_ip_address_ranges_data_event.store(false);
    } else if (is_display_banned_player_names_data_event.load()) {
      std::thread display_large_data_set{ display_banned_player_names, "^1Banned player names", number_of_entries_to_display.load(), false };
      display_large_data_set.detach();
      is_display_banned_player_names_data_event.store(false);
    } else if (is_display_banned_cities_data_event.load()) {
      std::thread display_large_data_set{ []() { display_banned_cities(main_app.get_current_game_server().get_banned_cities_set()); } };
      display_large_data_set.detach();
      is_display_banned_cities_data_event.store(false);
    } else if (is_display_banned_countries_data_event.load()) {
      std::thread display_large_data_set{ []() { display_banned_countries(main_app.get_current_game_server().get_banned_countries_set()); } };
      display_large_data_set.detach();
      is_display_banned_countries_data_event.store(false);
    } else if (is_display_public_admins_data_event.load()) {
      std::thread display_large_data_set{
        []() {
          display_admins_data(main_app.get_users(), "^2Public administrators:");
          display_admins_data(main_app.get_remote_users(), "^1Private administrators:");
        }
      };
      display_large_data_set.detach();
      is_display_public_admins_data_event.store(false);
    } else if (is_display_private_admins_data_event.load()) {
      std::thread display_large_data_set{
        []() { display_admins_data(main_app.get_remote_users(), "^1Private administrators:"); }
      };
      display_large_data_set.detach();
      is_display_private_admins_data_event.store(false);
      ...
      std::thread display_large_data_set{ []() {
        ...
        display_protected_entries("Protected IP addresses:", main_app.get_current_game_server().get_protected_ip_addresses(), protected_ip_to_player_name);
      } };
      display_large_data_set.detach();
      ...
      std::thread display_large_data_set{ []() {
        ...
        const auto &players_data = main_app.get_current_game_server().get_players_data();
        for (const auto &protected_ip_address_range : main_app.get_current_game_server().get_protected_ip_address_ranges()) {
          ostringstream oss;
          for (size_t i{}; i < main_app.get_current_game_server().get_number_of_players(); ++i) {
            const string player_narrow_ip_address_range { get_narrow_ip_address_range_for_specified_ip_address(players_data[i].ip_address) };
            const string player_wide_ip_address_range { get_wide_ip_address_range_for_specified_ip_address(players_data[i].ip_address) };
            if (protected_ip_address_range == player_narrow_ip_address_range || protected_ip_address_range == player_wide_ip_address_range) {
              oss << "^7" << players_data[i].player_name << " ^7| ";
            }
          }         
          string online_players_names{ oss.str() };
          if (online_players_names.length() >= 5)
            online_players_names.erase(online_players_names.length() - 5, 5);          
          protected_ip_address_range_to_player_name.emplace(protected_ip_address_range, std::move(online_players_names));          
        }
          display_protected_entries("Protected IP address ranges:", main_app.get_current_game_server().get_protected_ip_address_ranges(), protected_ip_address_range_to_player_name); } };
      display_large_data_set.detach();
      ...
      std::thread display_large_data_set{ []() {
        ...
        display_protected_entries("Protected cities:", main_app.get_current_game_server().get_protected_cities(), protected_ip_to_player_city);
      } };
      display_large_data_set.detach();
      ...
      std::thread display_large_data_set{ []() {
        ...
        display_protected_entries("Protected countries:", main_app.get_current_game_server().get_protected_countries(), protected_ip_to_player_country);
      } };
      display_large_data_set.detach();
  ...
  case WM_COMMAND: {
    ...
    case ID_MAP_RESTART: {
      ...
      if (!current_rcon_map_name.empty()) {
        ...
        if (show_user_confirmation_dialog(message_buffer, "Confirm your action")) {
          SetWindowTextA(app_handles.hwnd_e_user_input, "!rc map_restart");
          get_user_input();
        }
      }
    } break;
    ...
    case ID_FAST_RESTART: {
      ...
      if (!current_rcon_map_name.empty()) {
        ...
        if (show_user_confirmation_dialog(message_buffer, "Confirm your action")) {
          SetWindowTextA(app_handles.hwnd_e_user_input, "!rc fast_restart");
          get_user_input();
        }
      }
    } break;
    ...
    case ID_SHOWSERVERSBUTTON: {
      ...
      string window_title_message{ format("Rcon game server: {}:{} {}", main_app.get_game_servers()[0].get_server_ip_address(), main_app.get_game_servers()[0].get_server_port(), current_rcon_game_rcon_server_name) };
      append_to_title(app_handles.hwnd_main_window, std::move(window_title_message));
      std::thread th{
        [&]() { IsGUIThread(TRUE); view_game_servers(app_handles.hwnd_servers_grid); }
      };
      th.detach();
