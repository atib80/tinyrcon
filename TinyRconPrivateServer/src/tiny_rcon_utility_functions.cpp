...
extern std::mutex protect_ip_address_processing;
// std::unordered_set<std::string> allowed_ip_addresses;
// std::unordered_set<std::string> disallowed_ip_addresses;

// std::mutex process_ip_addresses_data_mutex;
...
extern const map<string, string> user_commands_help{
  ...
  { "!pt", "^1!pt public message ^7-> ^5sends public message to all logged in ^1private admins." },
  { "!py", "^1!py username private message ^7-> ^3sends ^1private message ^3to ^1private admin ^3whose username is ^1username." },
  { "!sort",
  ...
  { "!allow",
    "^3Type ^1!allow 123.123.123.123 ^3[Enter] ^7-> ^5allows IP address ^1123.123.123.123 ^5from ^5Tiny^6Rcon^5's\n  automatic ^1VPN/proxy IP address ^5check." },
  { "!disallow",
    "^3Type ^1!disallow 123.123.123.123 ^7-> ^3adds IP address ^1123.123.123.123 ^3to ^5Tiny^6Rcon^3's\n"
    "  list of ^1disallowed IP addresses ^3(automatic kick)." },
...
extern const unordered_map<bool, int> unmuted_and_muted_image_index{
  { false, 0 },
  { true, 1 }
};
...
{
  main_app.get_connection_manager_for_messages().process_and_send_message("tinyrcon-error",
    format(R"({}\{}\Error level={}\Error message: '{}')",
  ...
  const char *file_path)
{
  ...
  config_file << R"("tinyrcon_server_ip_address": ")" << main_app.get_tiny_rcon_server_ip_address() << "\",\n";
  ...
  // enable_automatic_vpn_proxy_ip_address_detection
  config_file << "\"enable_automatic_vpn_proxy_ip_address_detection\": "
              << (main_app.get_is_enable_automatic_vpn_proxy_ip_address_detection() ? "true" : "false") << ",\n";
  config_file << R"("proxy_check_io_user_key": ")" << main_app.get_proxy_check_io_user_key() << "\",\n";
  ...
  config_file << R"("user_defined_mute_message": ")" << main_app.get_admin_messages()["user_defined_mute_message"] << "\",\n";
  // user_defined_unmute_message
  config_file << R"("user_defined_unmute_message": ")" << main_app.get_admin_messages()["user_defined_unmute_message"] << "\",\n";
...
std::string convert_guid_to_ip_address(const long guid_key)
{
  char buffer[24]{};
  constexpr const int64_t upper_limit{ 1LL << 32 };
  unsigned long guid = static_cast<unsigned long>(guid_key < 0L ? upper_limit + guid_key : guid_key);
...
{
  std::string ex_msg{
    std::format(R"(^1Exception ^3thrown from ^1bool check_ip_address_validity("{}"))", ip_address)
  };
  ...
  const bool is_valid_ip_address =
    all_of(cbegin(parts), cend(parts), [&guid_key](const string &part) {
      if (part.length() > 3)
        return false;
      ...
      guid_key <<= 8u;
      const long value{ stol(part) };
      guid_key += value;
      return value >= 0 && value <= 255;
    });

  if (is_valid_ip_address) {
    return std::make_optional(guid_key);
  }
...
void parse_tinyrcon_tool_config_file(const char *configFileName)
{
  ...
  if (json_resource["rcon_game_servers"].exists()) {
    ...
    for (size_t i{}; i < rcon_game_servers_json_array.size(); ++i) {
      ...
      if (json_object.exists() && json_object["game_server_name"].exists() && json_object["game_server_ip_address"].exists() && json_object["game_server_port"].exists() && json_object["rcon_password"].exists() && json_object["private_slot_password"].exists()) {
        game_server gs{};
        data_line = json_object["game_server_name"].as_str();
        strip_leading_and_trailing_quotes(data_line);
        gs.set_server_name(std::move(data_line));

        data_line = json_object["game_server_ip_address"].as_str();
        strip_leading_and_trailing_quotes(data_line);
        gs.set_server_ip_address(std::move(data_line));

        const int port_number{ json_object["game_server_port"].as<int>() };
  ...
  if (0U == parsed_rcon_game_server_index) {
    ...
    if (json_resource["game_server_ip_address"].exists()) {
      data_line = json_resource["game_server_ip_address"].as_str();
      strip_leading_and_trailing_quotes(data_line);
      gs.set_server_ip_address(std::move(data_line));
    } else {
      found_missing_config_setting = true;
    ...
    if (json_resource["game_server_port"].exists()) {
      const int port_number{ json_resource["game_server_port"].as<int>() };
      gs.set_server_port(static_cast<uint_least16_t>(port_number));
    } else {
      found_missing_config_setting = true;
  ...
  if (json_resource["check_for_banned_players_time_interval"].exists()) {
    main_app.set_check_for_banned_players_time_period(
      json_resource["check_for_banned_players_time_interval"].as<int>());
  } else {
    found_missing_config_setting = true;
    main_app.set_check_for_banned_players_time_period(5);
  }

  if (json_resource["tinyrcon_server_ip_address"].exists()) {
    data_line = json_resource["tinyrcon_server_ip_address"].as_str();
    strip_leading_and_trailing_quotes(data_line);
  ...
  if (json_resource["tinyrcon_server_port"].exists()) {
    const int port_number{ json_resource["tinyrcon_server_port"].as<int>() };
  ...
  if (json_resource["private_tinyrcon_server_ip_address"].exists()) {
    data_line = json_resource["private_tinyrcon_server_ip_address"].as_str();
    strip_leading_and_trailing_quotes(data_line);
  ...
  if (json_resource["private_tinyrcon_server_port"].exists()) {
    const int port_number{ json_resource["private_tinyrcon_server_port"].as<int>() };
  ...
  if (json_resource["tinyrcon_ftp_server_username"].exists()) {
    data_line = json_resource["tinyrcon_ftp_server_username"].as_str();
    strip_leading_and_trailing_quotes(data_line);
  ...
  if (json_resource["tinyrcon_ftp_server_password"].exists()) {
    data_line = json_resource["tinyrcon_ftp_server_password"].as_str();
    strip_leading_and_trailing_quotes(data_line);
  ...
  if (json_resource["enable_automatic_connection_flood_ip_ban"].exists()) {
    ...
    main_app.set_is_enable_automatic_connection_flood_ip_ban(
      json_resource["enable_automatic_connection_flood_ip_ban"].as<bool>());
  } else {
    found_missing_config_setting = true;
  ...
  if (json_resource["minimum_number_of_connections_from_same_ip_for_automatic_ban"].exists()) {
    main_app.get_current_game_server().set_minimum_number_of_connections_from_same_ip_for_automatic_ban(
      json_resource["minimum_number_of_connections_from_same_ip_for_automatic_ban"].as<int>());
  } else {
    found_missing_config_setting = true;
  ...
  if (json_resource["number_of_warnings_for_automatic_kick"].exists()) {
    ...
    main_app.get_current_game_server().set_maximum_number_of_warnings_for_automatic_kick(
      json_resource["number_of_warnings_for_automatic_kick"].as<int>());
  } else {
    found_missing_config_setting = true;
    main_app.get_current_game_server().set_maximum_number_of_warnings_for_automatic_kick(2);
  }

  if (json_resource["enable_automatic_vpn_proxy_ip_address_detection"].exists()) {

    main_app.set_is_enable_automatic_vpn_proxy_ip_address_detection(
      json_resource["enable_automatic_vpn_proxy_ip_address_detection"].as<bool>());
  } else {
    found_missing_config_setting = true;
    main_app.set_is_enable_automatic_vpn_proxy_ip_address_detection(true);
  }

  if (json_resource["proxy_check_io_user_key"].exists()) {
    data_line = json_resource["proxy_check_io_user_key"].as_str();
    strip_leading_and_trailing_quotes(data_line);
    main_app.set_proxy_check_io_user_key(std::move(data_line));
  } else {
    found_missing_config_setting = true;
    main_app.set_proxy_check_io_user_key("409730-80vn94-4872th-782t10");
  }
  ...
  // user_defined_mute_message
  if (json_resource["user_defined_mute_message"].exists()) {
    if (!main_app.get_is_use_original_admin_messages()) {
      data_line = json_resource["user_defined_mute_message"].as_str();
      strip_leading_and_trailing_quotes(data_line);
      main_app.get_admin_messages()["user_defined_mute_message"] = std::move(data_line);
    } else {
      main_app.get_admin_messages()["user_defined_mute_message"] = "^7{PLAYERNAME} ^1you have been muted by admin ^5{ADMINNAME}. ^3Reason: ^1{REASON}";
    }
  } else {
    found_missing_config_setting = true;
    main_app.get_admin_messages()["user_defined_mute_message"] = "^7{PLAYERNAME} ^1you have been muted by admin ^5{ADMINNAME}. ^3Reason: ^1{REASON}";
  ...
  // user_defined_unmute_message
  if (json_resource["user_defined_unmute_message"].exists()) {
    if (!main_app.get_is_use_original_admin_messages()) {
      data_line = json_resource["user_defined_unmute_message"].as_str();
      strip_leading_and_trailing_quotes(data_line);
      main_app.get_admin_messages()["user_defined_unmute_message"] = std::move(data_line);
    } else {
      main_app.get_admin_messages()["user_defined_unmute_message"] = "^7{PLAYERNAME} ^1you have been unmuted by admin ^5{ADMINNAME}. ^3Reason: ^1{REASON}";
    }
  } else {
    found_missing_config_setting = true;
    main_app.get_admin_messages()["user_defined_unmute_message"] = "^7{PLAYERNAME} ^1you have been unmuted by admin ^5{ADMINNAME}. ^3Reason: ^1{REASON}";
  ...
  if (json_resource["use_different_background_colors_for_even_and_odd_lines"].exists()) {
    main_app.set_is_use_different_background_colors_for_even_and_odd_lines(
      json_resource["use_different_background_colors_for_even_and_odd_lines"].as<bool>());
  } else {
    found_missing_config_setting = true;
  ...
  if (json_resource["use_different_foreground_colors_for_even_and_odd_lines"].exists()) {
    main_app.set_is_use_different_foreground_colors_for_even_and_odd_lines(
      json_resource["use_different_foreground_colors_for_even_and_odd_lines"].as<bool>());
  } else {
    found_missing_config_setting = true;
  ...
  if (json_resource["spectate_time_delay"].exists()) {
    const size_t spectate_time_delay = json_resource["spectate_time_delay"].as<int>();
  ...
  if (!input_file) {
    ...
    while (getline(input_file, readData)) {
      ...
      if (!ip_to_temp_banned_player.contains(parts[0])) {
        ...
        const string converted_ban_date_and_time_info{ get_date_and_time_for_time_t("{DD}.{MM}.{Y} {hh}:{mm}",
          temp_banned_player_data.banned_start_time) };
    ...
    while (getline(input_file, readData)) {
      ...
      if (!ip_to_banned_player.contains(parts[0])) {
        ...
        convert_guid_key_to_country_name(
          ...
          bannedPlayerData.ip_address,
          bannedPlayerData);
        if (check_if_player_is_protected(bannedPlayerData, "^7enable ^1IP ban or ^1!mute ^7for", information_about_protected_player)) {
          print_colored_text(app_handles.hwnd_re_messages_data, information_about_protected_player.c_str());
          continue;
        }
...
readData = std::move(entry);*/
      if (protected_cities.contains(readData)) {
        const string information_about_removed_entry{
          format("^3Banned city ^1{} ^3has been protected by an ^1admin^3!\n^5If you want you can ^1unprotect ^5it using the ^1!unprotectcity {} ^5command.",
            readData,
            readData)
        };
...
void parse_banned_countries_file(const char *file_path, std::set<std::string> &banned_countries)
{
 ...
 readData = std::move(entry);*/
      if (protected_countries.contains(readData)) {
        const string information_about_removed_entry{
          format("^3Banned country ^1{} ^3has been protected by an ^1admin^3!\n^5If you want you can ^1unprotect ^5it using the ^1!unprotectcountry {} ^5command.",
            readData,
            readData)
        };
        print_colored_text(app_handles.hwnd_re_messages_data, information_about_removed_entry.c_str());
  ...
  if (output) {
    ...
    for (const auto &tb_player : temp_banned_players) {
      output << tb_player.ip_address << '\\'
             ...
             << remove_disallowed_characters_in_string(tb_player.reason) << '\\'
...
void load_reported_players_to_file(const char *file_path, std::vector<player> &reported_players)
{
  ...
  if (!input_file) {
    ...
    while (getline(input_file, readData)) {
      ...
      player bannedPlayerData{};
      size_t no_of_chars_to_copy{ std::min<size_t>(std::size(bannedPlayerData.player_name) - 1, parts[0].length()) };
      strncpy_s(bannedPlayerData.player_name, std::size(bannedPlayerData.player_name), parts[0].c_str(), no_of_chars_to_copy);
      ...
      strncpy_s(bannedPlayerData.banned_date_time, std::size(bannedPlayerData.banned_date_time), parts[2].c_str(), no_of_chars_to_copy);
  ...
  std::sort(begin(reported_players), end(reported_players), [](const player &p1, const player &p2) {
    return p1.banned_start_time < p2.banned_start_time;
  });
}
...
void save_reported_players_to_file(const char *file_path, const std::vector<player> &reported_players)
{
  ...
  if (output) {
    ...
    for (const auto &reported_player : reported_players) {
      output << reported_player.player_name << '\\' << reported_player.ip_address << '\\' << get_date_and_time_for_time_t("{DD}.{MM}.{Y} {hh}:{mm}", reported_player.banned_start_time)
             << '\\' << remove_disallowed_characters_in_string(reported_player.reason) << '\\' << reported_player.banned_by_user_name << '\n';
    }
...
void save_muted_players_data_to_file(const char *file_path, const std::unordered_map<std::string, player> &muted_players)
{
  ofstream output{ file_path };
  if (output) {

    for (const auto &[k, v] : muted_players) {
      output << v.ip_address << '\\'
             << v.guid_key << '\\'
             << v.player_name << '\\'
             << v.banned_date_time << '\\'
             << remove_disallowed_characters_in_string(v.reason) << '\\'
             << v.banned_by_user_name << '\n';
    }
  ...
  if (output) {
    ...
    for (const auto &banned_player : banned_ip_entries) {
      output << banned_player.ip_address << '\\'
             ...
             << remove_disallowed_characters_in_string(banned_player.reason) << '\\'
...
bool mute_player_ip_address(player &pd)
{

  convert_guid_key_to_country_name(main_app.get_connection_manager().get_geoip_data(), pd.ip_address, pd);

  if (pd.ip_hash_key == 0U) {
    const string error_message{ format("^3Cannot mute player ^7{} ^3who has an invalid ^1IP key: 0\n", pd.player_name) };
  ...
  constexpr const int64_t half_limit{ 1LL << 31 };
  constexpr const int64_t upper_limit{ 1LL << 32 };
...
bool temp_ban_player_ip_address(player &pd)
{
  ...
  if (!temp_banned_data_file_to_read) {
    ...
    strerror_s(buffer, buffer_size, static_cast<int>(GetLastError()));
    string errorMessage{
      "Couldn't open 'tempbans.txt' data file at specified path ("s + main_app.get_temp_bans_file_path() + ")! "s + buffer
    };
    show_error(app_handles.hwnd_main_window, errorMessage.c_str(), 0);
  ...
  main_app.get_current_game_server().add_ip_address_to_temp_banned_ip_addresses(
    pd.ip_address, pd);
...
bool global_ban_player_ip_address(player &pd)
{
  ...
  if (!bannedIPsFileToRead) {
    ...
    strerror_s(buffer, buffer_size, static_cast<int>(GetLastError()));
    string errorMessage{
      "Couldn't open banned IP addresses data file at specified path ("s + main_app.get_ip_bans_file_path() + ")! "s + buffer
    };
    show_error(app_handles.hwnd_main_window, errorMessage.c_str(), 0);
  ...
  main_app.get_current_game_server().add_ip_address_to_banned_ip_addresses(
    pd.ip_address, pd);
...
std::pair<bool, player> remove_reported_player(std::string &ip_address, std::string &message, const bool is_report_public_message)
{
  ...
  if (check_ip_address_validity(ip_address, ip_key)) {
    found_iter = find_if(std::cbegin(reported_players), std::cend(reported_players), [&ip_address](const player &p) {
      return ip_address == p.ip_address;
    });
    no = found_iter != cend(reported_players) ? static_cast<size_t>(found_iter - std::cbegin(reported_players)) : string::npos;
  ...
  if (no < reported_players.size()) {
    ...
    if (is_report_public_message) {
      ...
      const string buffer{ format("^1Admin ^7{} ^7has successfully removed previously ^1reported player ^7{}^7. ^7Reason: ^1{}", main_app.get_username(), found_iter->player_name, remove_disallowed_characters_in_string(found_iter->reason)) };
      message.assign(buffer);
      // rcon_say(message, true);
    }

    player pd{ *found_iter };
...
std::pair<bool, player> remove_temp_banned_ip_address(const std::string &ip_address, std::string &message, const bool is_automatic_temp_ban_remove, const bool is_report_public_message)
{
  if (!main_app.get_current_game_server().get_temp_banned_ip_addresses_map().contains(
        ip_address)) {
    return { false, player{} };
  }
  ...
  if (found_iter != std::end(temp_banned_players)) {
    if (is_report_public_message) {
      if (!is_automatic_temp_ban_remove) {
        const string buffer{
          format("^1Admin ^7{} ^7has manually removed ^1temporarily banned IP address ^7for player {}^7. ^7Reason: ^1{}",
            main_app.get_username(),
            found_iter->player_name,
            remove_disallowed_characters_in_string(found_iter->reason))
        };
        message.assign(buffer);
      }
...
// bool add_muted_ip_address(player &pd, vector<player> &muted_players_vector, unordered_map<string, player> &muted_players_map)
//{
//   unsigned long guid_number{};
...
//   const auto found_iter = find_if(begin(muted_players_vector), end(muted_players_vector), [&pd](const player &p) {
...
bool remove_permanently_banned_ip_address_range(player &pd, std::vector<player> &banned_ip_address_ranges_vector, std::unordered_map<std::string, player> &banned_ip_address_ranges_map)
{
  ...
  banned_ip_address_ranges_vector.erase(
    remove_if(std::begin(banned_ip_address_ranges_vector), std::end(banned_ip_address_ranges_vector), [&pd](const player &p) {
      return pd.ip_address == p.ip_address;
    }),
...
bool remove_permanently_banned_player_name(player &pd, std::vector<player> &banned_names_vector, std::unordered_map<std::string, player> &banned_names_map)
{
  ...
  banned_names_vector.erase(
    remove_if(std::begin(banned_names_vector), std::end(banned_names_vector), [&pd](const player &p) {
...
std::pair<bool, player> remove_permanently_banned_ip_address(std::string &ip_address, std::string &message, const bool is_report_public_message)
{
  int number{};
  const size_t no{
    is_valid_decimal_whole_number(ip_address, number) ? static_cast<size_t>(number - 1) : std::string::npos
  };

  auto &banned_players = main_app.get_current_game_server().get_banned_ip_addresses_vector();

  if (main_app.get_current_game_server().get_banned_ip_addresses_map().find(
        ip_address)
  ...
  if (found_iter != std::end(banned_players)) {
    if (is_report_public_message) {
      const string buffer{
        format("^1Admin ^7{} ^7has manually removed ^1permanently banned IP address ^7for player {}^7. ^7Reason: ^1{}",
          main_app.get_username().c_str(),
          found_iter->player_name,
          remove_disallowed_characters_in_string(found_iter->reason))
      };
      message.assign(buffer);
    ...
    player pd{ main_app.get_current_game_server().get_banned_ip_addresses_map().at(found_iter->ip_address) };

    banned_players.erase(
      remove_if(std::begin(banned_players), std::end(banned_players), [&ip_address](const player &p) {
        return ip_address == p.ip_address;
      }),
...
std::pair<bool, player> remove_muted_ip_address(std::string &ip_address, std::string &message, const bool is_report_public_message)
{

  if (int number{}; is_valid_decimal_whole_number(ip_address, number) && check_if_user_provided_pid_is_valid(ip_address)) {
    ip_address = get_player_ip_address_for_pid(number);
  }
  ...
  if (!main_app.get_muted_players_map().contains(ip_address)) {
    pd.ip_address = ip_address;
    // return { false, player{} };
  } else {
    pd = main_app.get_muted_players_map().at(ip_address);
  }

  if (main_app.get_muted_players_map().contains(ip_address)) {
    main_app.get_muted_players_map().erase(ip_address);
  }
  ...
  if (pd.ip_hash_key == 0U) {
    const string error_message{ format("^3Cannot unmute player ^7{} ^3who has an invalid ^1IP key: 0\n", pd.player_name) };
    print_colored_text(app_handles.hwnd_re_messages_data, error_message.c_str());
    return { false, player{} };
  }
...
// size_t get_number_of_characters_printed_in_rcon_chat(const char *text)
//{
//   size_t printed_chars_count{};
//   const size_t text_len = len(text);
//   const char *last = text + text_len;
...
void print_help_information(const std::vector<std::string> &input_parts)
{
 ...
 ^1!rc custom_rcon_command [optional_parameters] ^7-> ^3sends ^1custom_rcon_command [optional_parameters] ^3to game server.
   ^7Examples: ^1!rc map_restart ^7| ^1!rc fast_restart ^7| ^1!rc sv_iwdnames
 ^1!allow 123.123.123.123 ^3[Enter] ^7-> ^5allows (excludes) IP address ^1123.123.123.123 ^5from ^5Tiny^6Rcon^5's 
   automatic ^1VPN/proxy IP address ^5check.
 ^1!disallow 123.123.123.123 ^7-> ^3adds IP address ^1123.123.123.123 ^3to Tiny^6Rcon^3's 
   list of ^1disallowed IP addresses ^3(automatic kick).
 ...
 ^1!muted [25|all] ^7-> ^5displays the last ^125 ^5muted player entries ^5or ^1all ^5muted player entries, respectively.)"
      ...
      print_colored_text(app_handles.hwnd_re_messages_data, help_message.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes, true, true, false);
 ...
 ^1tell player_pid "private message" ^7-> ^5Sends ^1"private message" ^5to player whose pid number is equal to ^1player_pid.
      ...
      print_colored_text(app_handles.hwnd_re_messages_data,
        ...
        false);
      print_colored_text(app_handles.hwnd_re_messages_data,
        user_commands_help.at(command_name).c_str(),
        is_append_message_to_richedit_control::yes,
        is_log_message::yes,
        is_log_datetime::no,
        true,
      ...
      print_colored_text(app_handles.hwnd_re_messages_data,
        ...
        false);
      print_colored_text(app_handles.hwnd_re_messages_data,
        rcon_commands_help.at(command_name).c_str(),
        is_append_message_to_richedit_control::yes,
        is_log_message::yes,
        is_log_datetime::no,
        true,
...
{
  unordered_set<string> already_seen_username_and_player_name_pairs;
  ostringstream oss;
  ostringstream oss_player;

  oss << "^2Online admins: ";
  oss_player << "^2Online admins: ";

  if (!main_app.get_users().empty()) {

    for (auto &u : main_app.get_users()) {
      if (u->is_online) {
        const string admin_online{ format("^7{} ^3(^7{}^3)", u->user_name, u->player_name) };
        if (!already_seen_username_and_player_name_pairs.contains(admin_online)) {
          // const string admin_online{ format("^7{} ^3(^7{}^3), ", u->user_name, u->player_name) };
          already_seen_username_and_player_name_pairs.emplace(admin_online);
          oss << admin_online << ", ";
          oss_player << admin_online << ", ";
        }
      }
    }
  }

  string online_admins_information_for_players{ oss_player.str() };
  if (' ' == online_admins_information_for_players.back())
    online_admins_information_for_players.pop_back();
  if (',' == online_admins_information_for_players.back())
    online_admins_information_for_players.pop_back();

  if (!main_app.get_remote_users().empty()) {

    for (auto &ru : main_app.get_remote_users()) {
      if (ru->is_online) {
        const string admin_online{ format("^7{} ^3(^7{}^3)", ru->user_name, ru->player_name) };
        if (!already_seen_username_and_player_name_pairs.contains(admin_online)) {
          // const string admin_online{ format("^7{} ^3(^7{}^3), ", ru->user_name, ru->player_name) };
          already_seen_username_and_player_name_pairs.emplace(admin_online);
          oss << admin_online << ", ";
  ...
  for (auto &ru : main_app.get_remote_users()) {
    if (ru->is_logged_in) {
      main_app.get_connection_manager_for_remote_messages().process_and_send_message("rcon-online-admins-info", online_admins_information, true, ru);
    }
  }

  // rcon-online-admins-info-player
  for (auto &rp : main_app.get_remote_players()) {
    if (rp->is_logged_in) {
      main_app.get_connection_manager_for_remote_messages().process_and_send_message("rcon-online-admins-info-player", online_admins_information_for_players, true, rp);
    }
  }
}
  ...
  const string &arg)
{
  ...
  if (unsigned long ip_key{}; (str_compare_i(cmd, "!unmute") == 0)) {
    if (int no{ -1 }; check_ip_address_validity(arg, ip_key) || (is_valid_decimal_whole_number(arg, no) && check_if_user_provided_pid_is_valid(arg))) return true;
    return false;
  }

  if (unsigned long ip_key{}; (str_compare_i(cmd, "!ub") == 0) || (str_compare_i(cmd, "!unban") == 0)) {
    if (int no{ -1 }; check_ip_address_validity(arg, ip_key) || (is_valid_decimal_whole_number(arg, no) && no >= 1 && no <= (int)main_app.get_current_game_server().get_banned_ip_addresses_vector().size())) return true;
    return false;
  }
  ...
  if ((str_compare_i(cmd, "!ur") == 0) || (str_compare_i(cmd, "!unreport") == 0)) {
    unsigned long ip_key{};
    return ((is_valid_decimal_whole_number(arg, number) && number > 0 && number <= static_cast<int>(main_app.get_reported_players().size())) || check_ip_address_validity(arg, ip_key));
  }
...
void check_for_temp_banned_ip_addresses()
{
  ...
  for (auto &tb : temp_banned_players) {
    ...
    if (ban_expires_time <= now_in_seconds) {
      ...
      main_app.get_tinyrcon_dict()["{TEMP_BAN_START_DATE}"] = get_date_and_time_for_time_t(
        "{DD}.{MM}.{Y} {hh}:{mm}", tb.banned_start_time);
      main_app.get_tinyrcon_dict()["{TEMP_BAN_END_DATE}"] = get_date_and_time_for_time_t(
        "{DD}.{MM}.{Y} {hh}:{mm}", ban_expires_time);
      ...
      if (status) {
        main_app.get_connection_manager_for_messages().process_and_send_message("remove-tempban",
          format(R"({}\{}\{}\{}\{}\{}\{})",
            player.ip_address,
            player.player_name,
            player.banned_date_time,
            player.banned_start_time,
            player.ban_duration_in_hours,
            player.reason,
        ...
        const string private_msg{
          format("^2Player ^7{}'s ^1temporary ban ^2has been automatically removed.\n^7Info on player ^7{}^7: ^3IP: ^1{} ^5| ^3geoinfo: ^1{}, {} ^5| ^3Date of tempban: ^1{}\n ^3Tempban duration: ^1{} hours ^5|^5Reason: ^1{} ^5| ^3Banned by: ^1{}\n",
            player.player_name,
            player.player_name,
            player.ip_address,
            player.country_name,
        ...
        // main_app.add_message_to_queue(message_t("inform-message", inform_msg, true));
        main_app.get_connection_manager_for_messages().process_and_send_message("inform-message", inform_msg, true, main_app.get_tiny_rcon_server_ip_address(), main_app.get_tiny_rcon_server_port(), false);
      }
    }
  }
        ...
        if (main_app.get_current_game_server().get_temp_banned_player_data_for_ip_address(
          ...
          if (ban_expires_time > now_in_seconds) {
            ...
            if (main_app.get_is_disable_automatic_kick_messages()) {
              ...
              main_app.get_tinyrcon_dict()["{TEMP_BAN_START_DATE}"] = get_date_and_time_for_time_t(
                "{DD}.{MM}.{Y} {hh}:{mm}", pd.banned_start_time);
              main_app.get_tinyrcon_dict()["{TEMP_BAN_END_DATE}"] = get_time_interval_info_string_for_seconds(
                ban_expires_time - now_in_seconds);
              main_app.get_tinyrcon_dict()["{REASON}"] = remove_disallowed_characters_in_string(pd.reason);
              main_app.get_tinyrcon_dict()["{BANNED_BY}"] = remove_disallowed_characters_in_string(
                pd.banned_by_user_name);
              build_tiny_rcon_message(message);
              replace_br_with_new_line(message);
              const string private_msg{
                format("^3Automatic kick for ^1temporarily banned ^3player ^7{}\n ^5[^3IP: ^1{} ^5| ^3geoinfo: ^1{}, {} ^5| ^3Date of tempban: ^1{}\n ^3Tempban duration: ^1{} hours ^5|^5Reason: ^1{} ^5| ^3Banned by: ^1{}^5]\n",
                  pd.player_name,
                  pd.ip_address,
                  pd.country_name,
                  pd.city,
...
{
  const bool is_enable_automatic_vpn_proxy_ip_address_detection{ main_app.get_is_enable_automatic_vpn_proxy_ip_address_detection() };
  ...
  for (size_t i{}; i < main_app.get_current_game_server().get_number_of_players(); ++i) {
    ...
    if (is_enable_automatic_connection_flood_ip_ban && ip_address_frequency[online_player.ip_address] >= main_app.get_current_game_server().get_minimum_number_of_connections_from_same_ip_for_automatic_ban()) {
      main_app.get_tinyrcon_dict()["{ADMINNAME}"] = main_app.get_username();
      ...
      string public_message{
        "^1{ADMINNAME}: ^5Tiny^6Rcon ^7has successfully automatically banned ^1IP address: "s + online_player.ip_address + " ^7| ^5Player name: ^7{PLAYERNAME} ^7| ^5Reason: ^1{REASON} ^7| ^5Date of ban: ^1{BAN_DATE}\n"s
      };
      ...
      const string private_message{
        "^5Tiny^6Rcon ^2has successfully automatically executed command ^1!gb ^2on player ("s + get_player_information(online_player.pid, false, "Banned IP address") + "^3)\n"s
      };
    ...
    if (banned_ip_addresses.contains(online_player.ip_address)) {
      ...
      if (main_app.get_is_disable_automatic_kick_messages()) {
        ...
        if (main_app.get_current_game_server().get_banned_player_data_for_ip_address(online_player.ip_address, &pd)) {
          ...
          pd.reason = remove_disallowed_characters_in_string(pd.reason);
          main_app.get_tinyrcon_dict()["{REASON}"] = pd.reason;
        }
        build_tiny_rcon_message(message);
      }
      const string banned_player_information{ get_player_information(online_player.pid, false, "Banned IP address") };
    ...
    if (!banned_ip_address_ranges.empty()) {
      ...
      if (is_short_ip_address_range_ban || is_wider_ip_address_range_ban) {
        ...
        const string banned_player_information{ get_player_information(online_player.pid, false, "Banned IP address range") };
    ...
    if (main_app.get_is_automatic_city_kick_enabled() && !banned_cities.empty()) {
      if (banned_cities.contains(online_player.city)) {
        ...
        const string banned_player_information{ get_player_information(online_player.pid, false, "Banned city") };
        const string msg{
    ...
    if (is_enable_automatic_vpn_proxy_ip_address_detection) {
      main_app.add_player_pid_to_queue(online_player.pid);
    }
  }
}
...
bool check_if_player_is_protected(const player &online_player, const char *admin_command, string &message)
{
  ...
  if (check_ip_address_validity(online_player.ip_address, guid)) {
    ...
    is_ip_protected = main_app.get_current_game_server().get_protected_ip_addresses().contains(online_player.ip_address);
    is_ip_range_protected = main_app.get_current_game_server().get_protected_ip_address_ranges().contains(narrow_ip_address_range) || main_app.get_current_game_server().get_protected_ip_address_ranges().contains(wide_ip_address_range);
    ...
    if (is_ip_range_protected) {
      message.assign(format("^3Discarded ^1{} ^3on player ^7{} ^3whose IP address range ^1{} ^3is protected!\n^5Please, remove their ^1protected IP address range ^5first.", admin_command, online_player.player_name, main_app.get_current_game_server().get_protected_ip_address_ranges().contains(narrow_ip_address_range) ? narrow_ip_address_range : wide_ip_address_range));
      return true;
    }
  }
  const bool is_city_protected{ len(online_player.city) != 0 && main_app.get_current_game_server().get_protected_cities().contains(online_player.city) };
  const bool is_country_protected{ len(online_player.country_name) != 0 && main_app.get_current_game_server().get_protected_countries().contains(online_player.country_name) };
...
void kick_player(const int pid, const string &message)
{
  ...
  string reply_buffer;
  main_app.get_connection_manager().send_and_receive_rcon_data(buffer, reply_buffer, main_app.get_current_game_server().get_server_ip_address().c_str(), main_app.get_current_game_server().get_server_port(), main_app.get_current_game_server().get_rcon_password().c_str(), main_app.get_current_game_server(), true, false);
...
void say_message(const char *message)
{
  ...
  string reply_buffer;
  main_app.get_connection_manager().send_and_receive_rcon_data(buffer, reply_buffer, main_app.get_current_game_server().get_server_ip_address().c_str(), main_app.get_current_game_server().get_server_port(), main_app.get_current_game_server().get_rcon_password().c_str(), main_app.get_current_game_server(), false, false);
}
...
void rcon_say(string &msg, const bool is_print_to_rich_edit_messages_box)
{
  ...
  const auto lines = stl::helper::str_split(msg, '\n', nullptr, split_on_whole_needle_t::yes, ignore_empty_string_t::yes);
  for (const auto &line : lines) {
    say_message(line.c_str());
    if (is_print_to_rich_edit_messages_box) {
      print_colored_text(app_handles.hwnd_re_messages_data, line.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes, true, true, true);
    }
  }
}
...
void rcon_say_top_players(std::string &&title)
{
  ...
  get_top_players_stats_data(main_app.get_stats_data().get_scores_vector(), main_app.get_stats_data().get_scores_map(), n, public_message, tinyrcon_title_message.c_str());
  ...
  // if (main_app.get_current_game_server().get_number_of_players() >= 2) {
  ...
  //    print_colored_text(app_handles.hwnd_re_messages_data, online_players_stats.c_str(), is_append_message_to_richedit_control::yes, is_log_message::no, is_log_datetime::yes, true, true, true);
  // }
  ...
  if (lines.size() > 1u) {
    ...
    print_colored_text(app_handles.hwnd_re_messages_data, title.c_str(), is_append_message_to_richedit_control::yes, is_log_message::no, is_log_datetime::no, true, true, true);
    Sleep(3000);
    for (size_t i{}; i < std::min<size_t>(10u, lines.size()); ++i) {
      stl::helper::trim_in_place(lines[i]);
      if (lines[i].empty()) continue;
      rcon_say(lines[i], false);
      print_colored_text(app_handles.hwnd_re_messages_data, lines[i].c_str(), is_append_message_to_richedit_control::yes, is_log_message::no, is_log_datetime::no, true, true, true);
      Sleep(3000);
    }
  }
    ...
    if (!command_parts.empty()) {
      ...
      if (rcon_status_commands.contains(command_parts[0])) {
        ...
        const bool is_wait_for_reply{
          rcon_commands_wait_for_reply.find(command_parts[0]) != std::cend(rcon_commands_wait_for_reply)
        };
...
void process_user_command(const std::vector<string> &user_cmd)
{
  if (!user_cmd.empty() && main_app.get_command_handlers().contains(user_cmd[0])) {
    ...
    const bool is_hide_information_message{ re_msg.find("--silent") != string::npos || re_msg.find("/S") != string::npos };
    if (!is_hide_information_message) {
      print_colored_text(app_handles.hwnd_re_messages_data, re_msg.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes, true, true);
    }
...
volatile bool should_program_terminate(const string &user_input)
{
  ...
  is_terminate_program.store(
    user_input == "q" || user_input == "!q" || user_input == "!quit" || user_input == "quit" || user_input == "e" || user_input == "!e" || user_input == "exit" || user_input == "!exit");
  return is_terminate_program.load();
}
...
void display_temporarily_banned_ip_addresses(const size_t number_of_last_bans_to_display, const bool is_save_data_to_log_file)
{
  ...
  if (!temp_banned_players.empty()) {
    longest_name_length = std::max(longest_name_length, find_longest_player_name_length(temp_banned_players.cbegin(), temp_banned_players.cend(), false));
  ...
  const string decoration_line(174 + longest_name_length + longest_country_length, '=');
  oss << "^5\n"
      << decoration_line << "\n"
      << "^5| ";
  ...
  if (temp_banned_players.empty()) {
    ...
    for (size_t i{ start_index }; i < temp_banned_players.size(); ++i) {
      ...
      const time_t remaining_tempban_duration_in_seconds = ban_expires_time >= now_in_seconds ? ban_expires_time - now_in_seconds : 0;
      const string ban_start_date_str = get_date_and_time_for_time_t("{DD}.{MM}.{Y} {hh}:{mm}", bp.banned_start_time);
      const string ban_expires_date_str = get_time_interval_info_string_for_seconds(remaining_tempban_duration_in_seconds);
      oss << next_color << left << setw(longest_country_length) << buffer2 << " ^5| " << next_color << left << setw(20) << ban_start_date_str
          << " ^5| " << next_color << left << setw(28) << ban_expires_date_str << " ^5| ";
      const size_t printed_reason_char_count1{ get_number_of_characters_without_color_codes(bp.reason.c_str()) };
      if (printed_reason_char_count1 < 50) {
        oss << next_color << left << setw(50) << (bp.reason + string(50 - printed_reason_char_count1, ' '));
      } else {
      ...
      if (printed_reason_char_count3 < 32) {
        oss << "^7" << left << setw(32) << (bp.banned_by_user_name + string(32 - printed_reason_char_count3, ' '));
      } else {
      ...
      if (is_save_data_to_log_file) {
        ...
        log << left << setw(longest_country_length) << buffer2 << " | " << left << setw(20) << ban_start_date_str
            << " | " << left << setw(28) << ban_expires_date_str << " | ";
        const size_t printed_reason_char_count2{ reason.length() };
        if (printed_reason_char_count2 < 50) {
          log << left << setw(50) << (reason + string(50 - printed_reason_char_count2, ' '));
        } else {
          log << reason;
        }
        log << " | ";
  ...
  print_colored_text(app_handles.hwnd_re_messages_data, message.c_str(), is_append_message_to_richedit_control::yes, is_log_message::no, is_log_datetime::no, true, true, false);
  if (is_save_data_to_log_file) {
...
void display_banned_ip_address_ranges(const size_t number_of_last_bans_to_display, const bool is_save_data_to_log_file)
{
  ...
  if (!banned_players.empty()) {
    longest_name_length = std::max(longest_name_length, find_longest_player_name_length(banned_players.cbegin(), banned_players.cend(), false));
    longest_country_length =
      std::max(longest_country_length,
...
void display_protected_entries(const char *table_title, const std::set<std::string> &protected_entries, const unordered_map<string, string> &online_player_names, const bool is_save_data_to_log_file)
{
  ...
  if (!banned_players.empty()) {
    longest_protected_entry_length = std::max(longest_protected_entry_length, find_longest_entry_length(cbegin(protected_entries), cend(protected_entries), false));
  }
  ...
  if (protected_entries.empty()) {
    ...
    for (const auto &protected_entry : protected_entries) {
      ...
      if (printed_char_count1 < longest_protected_entry_length) {
        oss << left << setw(longest_protected_entry_length) << protected_entry + string(longest_protected_entry_length - printed_char_count1, ' ');
      } else {
        oss << left << setw(longest_protected_entry_length) << protected_entry;
      }
      if (is_save_data_to_log_file) {
        if (printed_char_count2 < longest_protected_entry_length) {
          log << left << setw(longest_protected_entry_length) << entry + string(longest_protected_entry_length - printed_char_count2, ' ');
        } else {
      ...
      if (online_player_names.contains(protected_entry)) {
        ...
        const size_t printed_char_count3{ get_number_of_characters_without_color_codes(online_player_names.at(protected_entry).c_str()) };
        const size_t printed_char_count4{ online_player_name.length() };
        if (printed_char_count3 < 200) {
          oss << " ^5| " << next_color << left << setw(200) << online_player_names.at(protected_entry) + string(200 - printed_char_count3, ' ');
        } else {
          oss << " ^5| " << next_color << left << setw(200) << online_player_names.at(protected_entry);
        }
        if (is_save_data_to_log_file) {
          if (printed_char_count4 < 200) {
            log << " | " << left << setw(200) << online_player_name + string(200 - printed_char_count4, ' ');
          } else {
...
void display_muted_players_information(const size_t number_of_last_muted_players_to_display, const bool is_save_data_to_log_file)
{
  size_t longest_name_length{ 12 };
  size_t longest_country_length{ 20 };
  auto &muted_players_map =
    main_app.get_muted_players_map();

  const size_t number_of_entries_to_display{ number_of_last_muted_players_to_display != string::npos && number_of_last_muted_players_to_display < muted_players_map.size() ? number_of_last_muted_players_to_display : muted_players_map.size() };

  vector<player> muted_players;
  size_t start_pos{};

  if (!muted_players_map.empty()) {
    for (const auto &[k, v] : muted_players_map) {
      muted_players.emplace_back(v);
    }

    std::ranges::sort(muted_players, [](const player &p1, const player &p2) { return p1.banned_start_time < p2.banned_start_time; });
    start_pos = muted_players.size() - number_of_entries_to_display;

    longest_name_length = std::max(longest_name_length, find_longest_player_name_length(cbegin(muted_players) + start_pos, cend(muted_players), false));
    longest_country_length =
      std::max(longest_country_length,
        find_longest_player_country_city_info_length(cbegin(muted_players) + start_pos, cend(muted_players)));
  }

  ostringstream oss;
  ostringstream log;

  oss << format("\n^5Last ^1{} ^5muted {}:", number_of_entries_to_display, number_of_entries_to_display != 1 ? "players" : "player");
  log << format("\nLast {} muted {}:", number_of_entries_to_display, number_of_entries_to_display != 1 ? "players" : "player");
  ...
  oss << left << setw(5) << "No."
      ...
      << "Player name"
      << " | " << left << setw(longest_country_length) << "Country, city"
      << " | " << left << setw(20) << "Date/time of mute"
      << " | " << left << setw(29) << "Reason"
      << " | " << left << setw(32) << "Muted by admin"
      << "|";
  if (is_save_data_to_log_file) {
  ...
  if (muted_players.empty()) {
    const size_t message_len = stl::helper::len(
      "| There are no muted player entries yet.");
    oss << "^5| ^3There are no muted player entries yet.";
    if (is_save_data_to_log_file) {
      log << "| There are no muted player entries yet.";
    }
    ...
    bool is_first_color{ true };
    size_t no{ start_pos };
    for (size_t i{ start_pos }; i < muted_players.size(); ++i) {
      auto &bp = muted_players[i];
      ++no;
      const char *next_color{ is_first_color ? "^3" : "^5" };
      oss << "^5| " << next_color << left << setw(5) << no << " ^5| " << next_color << left << setw(15) << bp.ip_address << " ^5| ^7";
      if (is_save_data_to_log_file) {
      ...
      oss << " ^5| ";
      stl::helper::trim_in_place(bp.banned_by_user_name);
      string muted_by_admin{ bp.banned_by_user_name };
      const size_t printed_reason_char_count3{ get_number_of_characters_without_color_codes(muted_by_admin.c_str()) };
      if (printed_reason_char_count3 < 32) {
        oss << "^7" << left << setw(32) << (muted_by_admin + string(32 - printed_reason_char_count3, ' '));
      } else {
        oss << "^7" << next_color << left << muted_by_admin;
      }
      oss << "^5|\n";
      if (is_save_data_to_log_file) {
        remove_all_color_codes(reason);
        remove_all_color_codes(muted_by_admin);
        log << left << setw(longest_country_length) << buffer2 << " | " << left << setw(20) << bp.banned_date_time
            << " | ";
        const size_t printed_reason_char_count2{ reason.length() };
        if (printed_reason_char_count2 < 29) {
        ...
        log << " | ";
        const size_t printed_reason_char_count4{ muted_by_admin.length() };
        if (printed_reason_char_count4 < 32) {
          log << left << setw(32) << (muted_by_admin + string(32 - printed_reason_char_count4, ' '));
        } else {
          log << muted_by_admin;
        }
        log << "|\n";
...
void display_permanently_banned_ip_addresses(const size_t number_of_last_bans_to_display, const bool is_save_data_to_log_file)
{
  ...
  if (banned_players.empty()) {
    const size_t message_len = stl::helper::len(
      "| There are no players permanently banned by their IP addresses yet.");
    ...
    bool is_first_color{ true };
    const size_t start_index{ number_of_last_bans_to_display != string::npos && number_of_last_bans_to_display <= banned_players.size() ? banned_players.size() - number_of_last_bans_to_display
                                                                                                                                        : 0U };
    size_t no{ start_index };
    for (size_t i{ start_index }; i < banned_players.size(); ++i) {
      auto &bp = banned_players[i];
...
void display_admins_data(const std::vector<std::shared_ptr<tiny_rcon_client_user>> &users, const char *title)
{
  ...
  if (users.empty()) {
    ...
    for (auto &user : users) {
      ...
      const bool is_logged_in{ user->is_logged_in || (current_ts - user->last_login_time_stamp) <= 31 };
      oss << text_element{ is_logged_in ? "yes" : "no", 13, is_logged_in ? "^2" : "^1" };
      ...
      oss << " ^5| ";
      oss << text_element{
        get_date_and_time_for_time_t("{DD}.{MM}.{Y} {hh}:{mm}", user->last_login_time_stamp).c_str(), 20, next_color
      };

      oss << " ^5| ";
      oss << text_element{
        get_date_and_time_for_time_t("{DD}.{MM}.{Y} {hh}:{mm}", user->last_logout_time_stamp).c_str(), 20, next_color
      };
...
std::string get_top_players_stats_data(std::vector<player_stats> &stats_data, std::unordered_map<std::string, player_stats> &stats_data_map, const size_t number_of_top_players, std::string &public_message, const char *title, std::string partial_or_full_player_name, const bool find_exact_player_name_match)
{
  ...
  if (stats_data.empty()) {
    ...
    for (size_t i{}; i < stats_data.size() && j < number_of_entries_to_display; ++i) {
      ...
      oss << text_element{ buffer, 17, next_color } << " ^5| ";
      rcon << "^5Score/min: ^1" << buffer2 << ' ';
      const string time_period_information{ get_time_interval_info_string_for_seconds_in_hours_and_minutes(user.time_spent_on_server_in_seconds + 5) };
      oss << text_element{ time_period_information.c_str(), 20, next_color } << "^5|\n";
      rcon << "^5Time: ^1" << time_period_information << '\n';
      // if (find_exact_player_name_match) break;
      is_first_color = !is_first_color;
    }
...
const std::string &get_full_gametype_name(const std::string &rcon_gametype_name)
{
  ...
  return full_gametype_names.find(rcon_gametype_name) != cend(full_gametype_names) ? full_gametype_names.at(
                                                                                       rcon_gametype_name)
                                                                                   : rcon_gametype_name;
}
...
void display_all_available_maps()
{
  ...
  if (str_compare_i(main_app.get_current_game_server().get_current_game_type().c_str(), "cod2") == 0) {
    for (const auto &[rcon_map_name, full_map_name] : main_app.get_available_rcon_to_full_map_names()) {
      oss << "^1" << rcon_map_name << " ^5-> ^2" << full_map_name.second << '\n';
      ++index;
    }
  } else {
    ...
    for (const auto &[rcon_map_name, full_map_name] : rcon_map_names_to_full_map_names) {
      oss << "^1" << rcon_map_name << " ^5-> ^2" << full_map_name << '\n';
      ++index;
    }
  }
  print_colored_text(app_handles.hwnd_re_messages_data, oss.str().c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes, true, true, false);
}
...
void change_colors()
{
  ...
  print_colored_text(app_handles.hwnd_re_messages_data, "^5\nType a number ^1(color code: 0-9) ^5for each color setting\n or press ^3(Enter) ^5to accept the default value for it.\n", is_append_message_to_richedit_control::yes, is_log_message::no, is_log_datetime::no, true, true, false);

  string color_code;

  do {
    const string re_msg{ fmc + "Full map name color (0-9), press enter to accept default (2): " };
    print_colored_text(app_handles.hwnd_re_messages_data, re_msg.c_str(), is_append_message_to_richedit_control::yes, is_log_message::no, is_log_datetime::no, true, true, false);
  ...
  do {
    print_colored_text(app_handles.hwnd_re_messages_data, "^3Use different alternating background colors for even and odd lines? (yes|no, enter for default (yes): ", is_append_message_to_richedit_control::yes, is_log_message::no, is_log_datetime::no, true, true, false);
    getline(cin, color_code);
  ...
  if (main_app.get_is_use_different_background_colors_for_even_and_odd_lines()) {
    do {
      print_colored_text(app_handles.hwnd_re_messages_data, "^5Background color of odd player data lines (0-9), press enter to accept default (0 - black): ", is_append_message_to_richedit_control::yes, is_log_message::no, is_log_datetime::no, true, true, false);
      getline(cin, color_code);
    } while (!color_code.empty() && !regex_match(color_code, color_code_regex));
    ...
    do {
      print_colored_text(app_handles.hwnd_re_messages_data, "^3Background color of even player data lines (0-9), press enter to accept default (8 - gray): ", is_append_message_to_richedit_control::yes, is_log_message::no, is_log_datetime::no, true, true, false);
      getline(cin, color_code);
    } while (!color_code.empty() && !regex_match(color_code, color_code_regex));
  ...
  do {
    print_colored_text(app_handles.hwnd_re_messages_data, "^5Use different alternating foreground colors for even and odd lines? (yes|no, enter for default (yes): ", is_append_message_to_richedit_control::yes, is_log_message::no, is_log_datetime::no, true, true, false);
    getline(cin, color_code);
    stl::helper::to_lower_case_in_place(color_code);
  } while (!color_code.empty() && color_code != "yes" && color_code != "no");
  ...
  if (main_app.get_is_use_different_foreground_colors_for_even_and_odd_lines()) {
    do {
      print_colored_text(app_handles.hwnd_re_messages_data, "Foreground color of odd player data lines (0-9), press enter to accept default (5): ", is_append_message_to_richedit_control::yes, is_log_message::no, is_log_datetime::no, true, true, false);
      getline(cin, color_code);
    } while (!color_code.empty() && !regex_match(color_code, color_code_regex));
    ...
    do {
      print_colored_text(app_handles.hwnd_re_messages_data, "Foreground color of even player data lines (0-9), press enter to accept default (3): ", is_append_message_to_richedit_control::yes, is_log_message::no, is_log_datetime::no, true, true, false);
      getline(cin, color_code);
    } while (!color_code.empty() && !regex_match(color_code, color_code_regex));
...
bool change_server_setting(const std::vector<std::string> &command)
{
 ...
 ^1!config name ^5new_user_name (for example: ^1config name Admin)
    ...
    print_colored_text(app_handles.hwnd_re_messages_data, correct_cmd_usage.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes, true, true, false);
    return false;
  }
  ...
  if (command[0] == "!config") {
    ...
    if (command[1] == "rcon") {
      ...
      if (!check_ip_address_validity(ip, ip_key) || (ip == main_app.get_current_game_server().get_server_ip_address() && port == main_app.get_current_game_server().get_server_port()))
        return false;
      const string re_msg{
        "^2You have successfully changed the ^1game server address ^2to ^5"s + command[2] + "^2\n"s
      };
      print_colored_text(app_handles.hwnd_re_messages_data, re_msg.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes, true, true, false);
...
std::string get_player_ip_address_for_pid(const int pid)
{
  const auto &players_data = main_app.get_current_game_server().get_players_data();
  for (size_t i{}; i < main_app.get_current_game_server().get_number_of_players(); ++i) {
    if (pid == players_data[i].pid)
      return players_data[i].ip_address;
  }
    ...
    if (pid == players_data[i].pid) {
      ...
      if (main_app.get_current_game_server().get_banned_ip_addresses_map().contains(p.ip_address) || main_app.get_current_game_server().get_temp_banned_ip_addresses_map().contains(p.ip_address) || !p.banned_by_user_name.empty()) {
        ...
        if (main_app.get_current_game_server().get_banned_ip_addresses_map().contains(p.ip_address)) {
          result += format(" ^3{} by admin: ^7{}", action_by_admin_message, main_app.get_current_game_server().get_banned_ip_addresses_map()[p.ip_address].banned_by_user_name);
        } else if (main_app.get_current_game_server().get_temp_banned_ip_addresses_map().contains(
                     p.ip_address)) {
          result += format(" ^3{} by admin: ^7{}", action_by_admin_message, main_app.get_current_game_server().get_temp_banned_ip_addresses_map()[p.ip_address].banned_by_user_name);
        } else {
...
std::string get_player_information_for_player(player &p, std::string_view action_by_admin_message)
{
  ...
  if (main_app.get_current_game_server().get_banned_ip_addresses_map().contains(p.ip_address) || main_app.get_current_game_server().get_temp_banned_ip_addresses_map().contains(p.ip_address) || !p.banned_by_user_name.empty()) {
    if (main_app.get_current_game_server().get_banned_ip_addresses_map().contains(p.ip_address)) {
      result += format(" ^5| ^3{} by admin: ^7{}", action_by_admin_message, main_app.get_current_game_server().get_banned_ip_addresses_map()[p.ip_address].banned_by_user_name);
    } else if (main_app.get_current_game_server().get_temp_banned_ip_addresses_map().contains(p.ip_address)) {
      result += format(" ^5| ^3{} by admin: ^7{}", action_by_admin_message, main_app.get_current_game_server().get_temp_banned_ip_addresses_map()[p.ip_address].banned_by_user_name);
    } else {
...
bool specify_reason_for_player_pid(const int pid, const std::string &reason)
{
  ...
  for (size_t i{}; i < main_app.get_current_game_server().get_number_of_players(); ++i) {
    if (pid == players_data[i].pid) {
      players_data[i].reason = remove_disallowed_characters_in_string(reason);
      return true;
    }
  }
...
{
  const string &full_map_name = get_full_map_name(main_app.get_current_game_server().get_current_map(),
  ...
  if (full_gametype_names.find(game_type) != cend(full_gametype_names)) {
    ...
    string reply_buffer;
    main_app.get_connection_manager().send_and_receive_rcon_data(buffer, reply_buffer, main_app.get_current_game_server().get_server_ip_address().c_str(), main_app.get_current_game_server().get_server_port(), main_app.get_current_game_server().get_rcon_password().c_str(), main_app.get_current_game_server(), true, true);
    ...
    if (is_reload_map) {
      ...
      string public_msg{
        format("^1Admin ^7{} ^7has restarted map ^1{} ^3({}^3)", main_app.get_username(), full_map_name, full_gametype_names.contains(game_type) ? get_full_gametype_name(game_type) : game_type)
      };
      ...
      main_app.add_message_to_queue(message_t("add-map_restart",
        format(R"({}\{}\{}\{})", main_app.get_username(), full_map_name, full_gametype_names.contains(game_type) ? get_full_gametype_name(game_type) : game_type, public_msg),
        true));
      rcon_say(public_msg, true);

      main_app.get_connection_manager().send_and_receive_rcon_data("map_restart", reply_buffer, main_app.get_current_game_server().get_server_ip_address().c_str(), main_app.get_current_game_server().get_server_port(), main_app.get_current_game_server().get_rcon_password().c_str(), main_app.get_current_game_server(), true, true);
      initiate_sending_rcon_status_command_now();
    }
  }
}
...
void load_map(const string &rcon_map_name, const string &game_type, const bool is_change_game_type)
{
  const auto &full_gametype_names = get_rcon_gametype_names_to_full_gametype_names_for_specified_game_name(
    main_app.get_game_name());
  const string &full_map_name = get_full_map_name(rcon_map_name, convert_game_name_to_game_name_t(main_app.get_current_game_server().get_game_name()));

  string public_msg{
    is_change_game_type ? format("^1Admin ^7{} ^7has changed map to ^1{} ^3({}^3)", main_app.get_username(), full_map_name, full_gametype_names.contains(game_type) ? get_full_gametype_name(game_type) : game_type) : format("^7{} ^7has changed map to ^1{}", main_app.get_username(), full_map_name)
  };
  const string admin_msg{ format("{}\\{}", main_app.get_username(), public_msg) };
  main_app.add_message_to_queue(message_t("public-message", admin_msg, true));
  const string &reported_game_type = is_change_game_type ? game_type
                                                         : main_app.get_current_game_server().get_current_game_type();
  main_app.add_message_to_queue(message_t("add-map_change",
    format(R"({}\{}\{}\{})", main_app.get_username(), full_map_name, full_gametype_names.contains(reported_game_type) ? get_full_gametype_name(reported_game_type) : reported_game_type, public_msg),
    true));
  rcon_say(public_msg, true);
  ...
  string reply;
  main_app.get_connection_manager().send_and_receive_rcon_data(buffer, reply, main_app.get_current_game_server().get_server_ip_address().c_str(), main_app.get_current_game_server().get_server_port(), main_app.get_current_game_server().get_rcon_password().c_str(), main_app.get_current_game_server(), true, true);
...
size_t print_colored_text(HWND re_control, const char *text, const is_append_message_to_richedit_control print_to_richedit_control, const is_log_message log_to_file, is_log_datetime is_log_current_date_time, const bool is_remove_color_codes_for_log_message, const bool is_display_message_to_remote_user, const bool is_send_message_to_player)
{
  ...
  if (print_to_richedit_control == is_append_message_to_richedit_control::yes) {
    ...
    if (re_control == app_handles.hwnd_re_messages_data) {
      main_app.add_tinyrcon_message_to_queue(print_message_t{ text, log_to_file, is_log_current_date_time, is_remove_color_codes_for_log_message, is_display_message_to_remote_user, is_send_message_to_player });
      return 0u;
    }
...
size_t print_message(HWND re_control, const std::string &msg, const is_log_message log_to_file, is_log_datetime is_log_current_date_time, const bool is_remove_color_codes_for_log_message, const bool is_display_message_to_remote_user, const bool is_send_message_to_player)
{
  ...
  unordered_set<string> seen_ip_addresses;

  if (is_display_message_to_remote_user) {
    for (auto &remote_user : main_app.get_remote_users()) {
      if (unsigned long guid{}; check_ip_address_validity(remote_user->ip_address, guid) && !seen_ip_addresses.contains(remote_user->ip_address) && remote_user->is_logged_in) {
        seen_ip_addresses.emplace(remote_user->ip_address);
        main_app.get_connection_manager_for_remote_messages().process_and_send_message("inform-message", text, true, remote_user);
      }
    }
  }

  if (is_send_message_to_player) {

    const string cleaned_text{ remove_ip_addresses(text) };
    const string message_to_admins{ get_cleaned_user_name(msg) };

    if (message_to_admins.find("private admin") == string::npos) {

      for (auto &admin : main_app.get_users()) {
        if (unsigned long guid{}; check_ip_address_validity(admin->ip_address, guid) && !seen_ip_addresses.contains(admin->ip_address) && admin->is_logged_in) {
          seen_ip_addresses.emplace(admin->ip_address);
          main_app.get_connection_manager_for_remote_messages().process_and_send_message("inform-message", text, true, admin);
        }
      }

      for (auto &remote_player : main_app.get_remote_players()) {
        if (unsigned long guid{}; remote_player->is_logged_in && check_ip_address_validity(remote_player->ip_address, guid) && !seen_ip_addresses.contains(remote_player->ip_address)) {
          seen_ip_addresses.emplace(remote_player->ip_address);
          main_app.get_connection_manager_for_remote_messages().process_and_send_message("inform-message-player", cleaned_text, true, remote_player);
        }
      }
    }
  }
...
void display_tempbanned_players_remaining_time_period()
{
  ...
  for (auto &pl : tembanned_players) {
    pl.reason = remove_disallowed_characters_in_string(pl.reason);
    ...
    if (ban_expires_time > now_in_seconds) {
      ...
      main_app.get_tinyrcon_dict()["{ADMINNAME}"] = main_app.get_username();
      main_app.get_tinyrcon_dict()["{TEMP_BAN_START_DATE}"] = get_date_and_time_for_time_t(
        "{DD}.{MM}.{Y} {hh}:{mm}", pl.banned_start_time);
      main_app.get_tinyrcon_dict()["{TEMP_BAN_END_DATE}"] = get_date_and_time_for_time_t(
        "{DD}.{MM}.{Y} {hh}:{mm}", ban_expires_time);
      ...
      main_app.get_connection_manager_for_messages().process_and_send_message("remove-tempban",
        format(R"({}\{}\{}\{}\{}\{}\{})",
          ...
          pl.player_name,
          pl.banned_date_time,
          pl.banned_start_time,
          pl.ban_duration_in_hours,
          pl.reason,
          pl.banned_by_user_name),
        true,
        main_app.get_tiny_rcon_server_ip_address(),
        main_app.get_tiny_rcon_server_port(),
        false);
      ...
      if (status) {
        replace_br_with_new_line(message);
        const string private_msg{
          format("^2Player ^7{}'s ^1temporary ban ^2has automatically been removed.\n^7Info on player ^7{}^7: ^3IP: ^1{} ^5| ^3geoinfo: ^1{}, {} ^5| ^3Date of tempban: ^1{}\n ^3Tempban duration: ^1{} hours ^5|^5Reason: ^1{} ^5| ^3Banned by: ^1{}\n",
            player.player_name,
            player.player_name,
            player.ip_address,
            player.country_name,
...
void construct_tinyrcon_gui(HWND hWnd)
{
  ...
  app_handles.hwnd_button_view_tempbans =
    CreateWindowExA(NULL, "Button", "View temporary bans", BS_PUSHBUTTON | BS_CENTER | BS_VCENTER | WS_VISIBLE | WS_CHILD, 450, screen_height - 118, 180, 30, hWnd, reinterpret_cast<HMENU>(ID_VIEWTEMPBANSBUTTON), app_handles.hInstance, nullptr);

  app_handles.hwnd_button_view_ipbans =
    CreateWindowExA(NULL, "Button", "View IP bans", BS_PUSHBUTTON | BS_CENTER | BS_VCENTER | WS_VISIBLE | WS_CHILD, 650, screen_height - 118, 180, 30, hWnd, reinterpret_cast<HMENU>(ID_VIEWIPBANSBUTTON), app_handles.hInstance, nullptr);

  app_handles.hwnd_button_view_adminsdata = CreateWindowExA(
    NULL, "Button", "View admins' data", BS_PUSHBUTTON | BS_CENTER | BS_VCENTER | WS_VISIBLE | WS_CHILD, screen_width / 2 + 175, screen_height - 175, 150, 25, hWnd, reinterpret_cast<HMENU>(ID_VIEWADMINSDATA), app_handles.hInstance, nullptr);

  app_handles.hwnd_connect_button = CreateWindowExA(
    NULL, "Button", "Join server", BS_PUSHBUTTON | BS_CENTER | BS_VCENTER | WS_VISIBLE | WS_CHILD, 450, screen_height - 77, 180, 30, hWnd, reinterpret_cast<HMENU>(ID_CONNECTBUTTON), app_handles.hInstance, nullptr);

  app_handles.hwnd_spectate_player_button = CreateWindowEx(
    NULL, "Button", "Spectate player", BS_PUSHBUTTON | BS_CENTER | BS_VCENTER | WS_VISIBLE | WS_CHILD, 650, screen_height - 77, 180, 30, hWnd, reinterpret_cast<HMENU>(ID_SPECTATEPLAYER), app_handles.hInstance, nullptr);

  app_handles.hwnd_mute_player_button = CreateWindowEx(
    NULL, "Button", "Mute player", BS_PUSHBUTTON | BS_CENTER | BS_VCENTER | WS_VISIBLE | WS_CHILD, 850, screen_height - 118, 180, 30, hWnd, reinterpret_cast<HMENU>(ID_MUTE_PLAYER), app_handles.hInstance, nullptr);

  app_handles.hwnd_unmute_player_button = CreateWindowEx(
    NULL, "Button", "Unmute player", BS_PUSHBUTTON | BS_CENTER | BS_VCENTER | WS_VISIBLE | WS_CHILD, 850, screen_height - 77, 180, 30, hWnd, reinterpret_cast<HMENU>(ID_UNMUTE_PLAYER), app_handles.hInstance, nullptr);

  app_handles.hwnd_say_button = CreateWindowExA(
    NULL, "Button", "Send public message", BS_PUSHBUTTON | BS_CENTER | BS_VCENTER | WS_VISIBLE | WS_CHILD, screen_width / 2 + 175, screen_height - 140, 150, 25, hWnd, reinterpret_cast<HMENU>(ID_SAY_BUTTON), app_handles.hInstance, nullptr);

  app_handles.hwnd_tell_button = CreateWindowExA(
    NULL, "Button", "Send private message", BS_PUSHBUTTON | BS_CENTER | BS_VCENTER | WS_VISIBLE | WS_CHILD, screen_width / 2 + 175, screen_height - 105, 150, 25, hWnd, reinterpret_cast<HMENU>(ID_TELL_BUTTON), app_handles.hInstance, nullptr);
  ...
  hImageListForChat = ImageList_Create(24, 24, ILC_COLOR32, unmuted_and_muted_image_index.size(), 1);
  for (size_t i{}; i < unmuted_and_muted_image_index.size(); ++i) {
    auto hbmp = static_cast<HBITMAP>(LoadImage(app_handles.hInstance, MAKEINTRESOURCE(IDR_BMP_UNMUTED + i), IMAGE_BITMAP, 24, 24, LR_CREATEDIBSECTION | LR_DEFAULTSIZE));
    ImageList_Add(hImageListForChat, hbmp, nullptr);
  }
  ...
  initialize_players_grid(app_handles.hwnd_players_grid, 8, max_players_grid_rows, true);
...
void display_country_flag(HWND hgrid, const int row, const int col, const char *country_code)
{
  ...
  item.col = col;
  item.row = row;
  if (!country_code || strlen(country_code) == 0u) country_code = "xy";
...
void display_chat_image(HWND hgrid, const int row, const int col, const bool is_chat_muted)
{
  ...
  item.col = col;
  item.row = row;
  item.lpCurValue = (LPARAM)unmuted_and_muted_image_index.at(is_chat_muted);
  SimpleGrid_SetItemData(hgrid, &item);
}
...
void initialize_players_grid(HWND hgrid, const size_t cols, const size_t rows, const bool is_for_rcon_status)
{
  ...
  for (size_t col_id{}; col_id < cols; ++col_id) {
    ...
    if (7 == col_id) {
      column.dwType = GCT_IMAGE;
      column.pOptional = hImageListForChat;
    }
  ...
  if (is_for_rcon_status) {
    ...
    SimpleGrid_SetColWidth(hgrid, 5, players_grid_width - 740);
  ...
  for (size_t i{ start_row }; i < last_row; ++i) {
    for (size_t j{}; j < cols; ++j) {
      if (6 == j) {
        ...
        display_chat_image(hgrid, i, 7, true);
      } else {
        PutCell(hgrid, i, j, "");
      }
    }
...
void display_players_data_in_players_grid(HWND hgrid)
{
  ...
  if (number_of_players > 0) {
    for (size_t i{}; i < number_of_players; ++i) {
      ...
      display_chat_image(hgrid, i, 7, displayed_players_data[i].is_muted);
    }
  }

  clear_players_data_in_players_grid(hgrid, number_of_players, max_players_grid_rows, 8);
  ...
  SimpleGrid_SetColWidth(hgrid, 4, 190);
  SimpleGrid_SetColWidth(hgrid, 5, players_grid_width - (player_name_column_width + 470));
  SimpleGrid_SetColWidth(hgrid, 6, 50);
  SimpleGrid_SetColWidth(hgrid, 7, 50);
...
bool show_user_confirmation_dialog(const char *msg, const char *title, const char *)
{
  ...
  app_handles.hwnd_e_reason = CreateWindowEx(0, "Edit", nullptr, WS_GROUP | WS_TABSTOP | WS_CHILD | WS_VISIBLE | WS_BORDER | ES_LEFT | ES_AUTOHSCROLL, 100, 220, 470, 20, app_handles.hwnd_confirmation_dialog, reinterpret_cast<HMENU>(ID_EDIT_ADMIN_REASON), app_handles.hInstance, nullptr);
...
std::pair<bool, game_name_t>
  check_if_specified_server_ip_port_and_rcon_password_are_valid(const char *ip_address, const uint_least16_t port_number, const char *rcon_password)
{
  connection_manager cm;
  string reply;
  main_app.get_connection_manager().send_and_receive_rcon_data("version", reply, ip_address, port_number, rcon_password, main_app.get_current_game_server(), true, false);
...
bool show_and_process_tinyrcon_configuration_panel(const char *title)
{
  ...
  app_handles.hwnd_user_name = CreateWindowEx(0, "Edit", nullptr, WS_GROUP | WS_TABSTOP | WS_CHILD | WS_VISIBLE | WS_BORDER | ES_LEFT | ES_AUTOHSCROLL, 220, 10, 450, 20, app_handles.hwnd_configuration_dialog, (HMENU)ID_EDIT_ADMIN_NAME, app_handles.hInstance, nullptr);
  if (!app_handles.hwnd_user_name)
    return false;
  ...
  app_handles.hwnd_server_name = CreateWindowEx(0, "Edit", nullptr, WS_GROUP | WS_TABSTOP | WS_CHILD | WS_VISIBLE | WS_BORDER | ES_LEFT | ES_AUTOHSCROLL, 220, 40, 450, 20, app_handles.hwnd_configuration_dialog, (HMENU)ID_EDIT_SERVER_NAME, app_handles.hInstance, nullptr);
  if (!app_handles.hwnd_server_name)
    return false;
  ...
  app_handles.hwnd_server_ip_address = CreateWindowEx(0, "Edit", nullptr, WS_GROUP | WS_TABSTOP | WS_CHILD | WS_VISIBLE | WS_BORDER | ES_LEFT | ES_AUTOHSCROLL, 220, 70, 450, 20, app_handles.hwnd_configuration_dialog, (HMENU)ID_EDIT_SERVER_IP, app_handles.hInstance, nullptr);
  ...
  app_handles.hwnd_server_port = CreateWindowEx(0, "Edit", nullptr, WS_GROUP | WS_TABSTOP | WS_CHILD | WS_VISIBLE | WS_BORDER | ES_LEFT | ES_AUTOHSCROLL, 220, 100, 450, 20, app_handles.hwnd_configuration_dialog, (HMENU)ID_EDIT_SERVER_PORT, app_handles.hInstance, nullptr);
  ...
  app_handles.hwnd_rcon_password = CreateWindowEx(0, "Edit", nullptr, WS_GROUP | WS_TABSTOP | WS_CHILD | WS_VISIBLE | WS_BORDER | ES_LEFT | ES_AUTOHSCROLL, 220, 130, 450, 20, app_handles.hwnd_configuration_dialog, (HMENU)ID_EDIT_SERVER_RCON, app_handles.hInstance, nullptr);
  ...
  app_handles.hwnd_cod1_path_edit = CreateWindowEx(0, "Edit", nullptr, WS_GROUP | WS_TABSTOP | WS_CHILD | WS_VISIBLE | WS_BORDER | ES_LEFT | ES_AUTOHSCROLL, 220, 190, 450, 40, app_handles.hwnd_configuration_dialog, (HMENU)ID_EDIT_CONFIGURATION_COD1_PATH, app_handles.hInstance, nullptr);
  ...
  app_handles.hwnd_cod2_path_edit = CreateWindowEx(0, "Edit", nullptr, WS_GROUP | WS_TABSTOP | WS_CHILD | WS_VISIBLE | WS_BORDER | ES_LEFT | ES_AUTOHSCROLL, 220, 240, 450, 40, app_handles.hwnd_configuration_dialog, (HMENU)ID_EDIT_CONFIGURATION_COD2_PATH, app_handles.hInstance, nullptr);
  ...
  app_handles.hwnd_cod4_path_edit = CreateWindowEx(0, "Edit", nullptr, WS_GROUP | WS_TABSTOP | WS_CHILD | WS_VISIBLE | WS_BORDER | ES_LEFT | ES_AUTOHSCROLL, 220, 290, 450, 40, app_handles.hwnd_configuration_dialog, (HMENU)ID_EDIT_CONFIGURATION_COD4_PATH, app_handles.hInstance, nullptr);
  ...
  app_handles.hwnd_cod5_path_edit = CreateWindowEx(0, "Edit", nullptr, WS_GROUP | WS_TABSTOP | WS_CHILD | WS_VISIBLE | WS_BORDER | ES_LEFT | ES_AUTOHSCROLL, 220, 340, 450, 40, app_handles.hwnd_configuration_dialog, (HMENU)ID_EDIT_CONFIGURATION_COD5_PATH, app_handles.hInstance, nullptr);
  ...
  if (!main_app.get_codmp_exe_path().empty() && (main_app.get_codmp_exe_path().find(cod1_steam_applaunch_id) != string::npos || check_if_file_path_exists(main_app.get_codmp_exe_path().c_str()))) {
    ...
    if (!found_cod1_path.empty() && (found_cod1_path.find(cod1_steam_applaunch_id) != string::npos || check_if_file_path_exists(found_cod1_path.c_str()))) {
  ...
  if (!main_app.get_cod2mp_exe_path().empty() && (main_app.get_cod2mp_exe_path().find(cod2_steam_applaunch_id) != string::npos || check_if_file_path_exists(main_app.get_cod2mp_exe_path().c_str()))) {
    ...
    if (!found_cod2_path.empty() && (found_cod2_path.find(cod2_steam_applaunch_id) != string::npos || check_if_file_path_exists(found_cod2_path.c_str()))) {
  ...
  if (!main_app.get_iw3mp_exe_path().empty() && (main_app.get_iw3mp_exe_path().find(cod4_steam_applaunch_id) != string::npos || check_if_file_path_exists(main_app.get_iw3mp_exe_path().c_str()))) {
    ...
    if (!found_cod4_path.empty() && (found_cod4_path.find(cod4_steam_applaunch_id) != string::npos || check_if_file_path_exists(found_cod4_path.c_str()))) {
  ...
  if (!main_app.get_cod5mp_exe_path().empty() && (main_app.get_cod5mp_exe_path().find(cod5_steam_applaunch_id) != string::npos || check_if_file_path_exists(main_app.get_cod5mp_exe_path().c_str()))) {
    ...
    if (!found_cod5_path.empty() && (found_cod5_path.find(cod5_steam_applaunch_id) != string::npos || check_if_file_path_exists(found_cod5_path.c_str()))) {
  ...
  print_colored_text(app_handles.hwnd_re_confirmation_message,
    "^2Please enter and verify the correctness of your game server's input settings by clicking on the ^3Test connection ^2button.\n",
    is_append_message_to_richedit_control::yes,
    is_log_message::no,
    is_log_datetime::no);
...
void prepare_players_data_for_display(game_server &gs, const bool is_log_status_table)
{
  ...
  if (0 == number_of_players) {
    ...
    const bool is_rcon_game_server{ main_app.get_game_server_index() < main_app.get_rcon_game_servers_count() };

    for (size_t i{}; i < number_of_players; ++i) {

      const player &p{ players[i] };

      if (is_rcon_game_server) {
        ...
        displayed_players_data[i].is_muted = p.is_muted;
      }
...
void prepare_players_data_for_display_of_getstatus_response(game_server &gs, const bool is_log_status_table)
{
  ...
  if (number_of_players > 0) {
    ...
    for (size_t i{}; i < number_of_players; ++i) {
      ...
      displayed_players_data[i].is_muted = false;
...
void correct_truncated_player_names(game_server &gs, const char *ip_address, const uint_least16_t port_number)
{
  ...
  ostringstream oss;
  oss << gs.get_current_map() << '\\' << gs.get_current_game_type() << '\n';
  auto &players_data = gs.get_players_data();
  for (size_t pl_index{}; pl_index < gs.get_number_of_players(); ++pl_index) {
    oss << players_data[pl_index].pid << '\\' << players_data[pl_index].score << '\\' << players_data[pl_index].ping << '\\' << players_data[pl_index].guid_key << '\\' << players_data[pl_index].player_name << "^7" << players_data[pl_index].ip_address << '\\' << players_data[pl_index].country_name << ", " << players_data[pl_index].city << '\\' << players_data[pl_index].country_code << '\n';
  }
  main_app.get_rcon_status_responses_for_players_queue().emplace(oss.str());
}
...
void print_message_about_corrected_player_name(HWND re_hwnd, const char *truncated_name, const char *corrected_name)
{
  if (truncated_name && corrected_name) {
    char buffer[128]{};
    (void)snprintf(buffer, std::size(buffer), "^3Corrected truncated player name from ^7'%s^7' ^2to ^7'%s^7'\n", truncated_name, corrected_name);
...
void display_banned_cities(const std::set<std::string> &banned_cities)
{
  ...
  print_colored_text(app_handles.hwnd_re_messages_data, information.c_str(), is_append_message_to_richedit_control::yes, is_log_message::no, is_log_datetime::no, true, true, false);
}
...
void check_if_admins_are_online_and_get_admins_player_names(const std::vector<player> &players, size_t no_of_online_players)
{
  ...
  for (auto &ru : main_app.get_remote_users()) {
    ru->is_online = false;
    for (size_t i{}; i < no_of_online_players; ++i) {
      if (check_ip_address_validity(ru->ip_address, guid_key) && ru->ip_address == players[i].ip_address) {
        ru->is_online = true;
        ru->player_name = players[i].player_name;
        break;
      }
    }
  }
...
bool save_current_user_data_to_json_file(const char *file_path)
{
  ...
  const string user_name{ remove_disallowed_characters_in_string(main_app.get_username()) };
...
bool validate_admin_and_show_missing_admin_privileges_message(const shared_ptr<tiny_rcon_client_user> &user, const bool is_show_message_box, const is_log_message log_message, const is_log_datetime log_date_time)
{
  if (!user->is_admin) {
    string warning_msg{ format("^7{}^3, you need to have the correct ^1rcon password ^3to be able to execute ^1admin-level ^3commands.\n", main_app.get_username()) };
    print_colored_text(app_handles.hwnd_re_messages_data, warning_msg.c_str(), is_append_message_to_richedit_control::yes, log_message, log_date_time, true, true, false);
...
void removed_disallowed_characters_in_place(std::string &input)
{
...
std::string remove_disallowed_characters_in_string(const std::string &input)
{
  // const string disallowed_chars{ "\\/-[]\"'{|}%:;=+\b\f\n\r\t " };
  string_view disallowed_chars{ "\\", len("\\") };
  const std::unordered_set<char> disallowed_characters(cbegin(disallowed_chars), cend(disallowed_chars));
...
{
  string name_index{ remove_disallowed_characters_in_string(name) };
  remove_all_color_codes(name_index);
  trim_in_place(name_index);
  to_lower_case_in_place(name_index);
  return name_index;
}

void replace_br_with_new_line(std::string &message)
{
  static constexpr const char *needle{ "{{br}}" };
  static constexpr const size_t needle_len{ len(needle) };
  for (size_t next{ string::npos }; (next = message.find(needle)) != string::npos;) {
    message.replace(next, needle_len, "\n");
  }
}
...
void refresh_game_servers_data(HWND hgrid)
{
  ...
  for (size_t i{}; i < main_app.get_rcon_game_servers_count(); ++i) {
    ...
    if (parse_getinfo_response_for_specified_game_server(gs, number_of_online_players, number_of_max_players) || parse_getstatus_response_for_specified_game_server(gs)) {
      ...
      const auto &rcon_map_names_to_full_map_names = get_rcon_map_names_to_full_map_names_for_specified_game_name(convert_game_name_to_game_name_t(gs.get_game_name()));
      gs.set_current_full_map_name(rcon_map_names_to_full_map_names.contains(gs.get_current_map()) ? rcon_map_names_to_full_map_names.at(gs.get_current_map()) : gs.get_current_map());
...
bool parse_and_display_downloaded_game_servers_data(std::string &game_servers_data, const char *version_number, const bool is_display_parsed_game_servers_data)
{
  ...
  print_colored_text(app_handles.hwnd_re_messages_data, started_parsing_message.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes, true, false, false);
  ...
  print_colored_text(app_handles.hwnd_re_messages_data, finished_parsing_message.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes, true, false, false);
...
void parse_tinyrcon_server_users_data(const char *file_path)
{
  ...
  for (string data; std::getline(configFile, data);) {
    ...
    player pd{};
    if (check_ip_address_validity(ip_address, guid)) {
      strcpy_s(pd.player_name, std::size(pd.player_name), username.c_str());
      pd.ip_address = ip_address;
      convert_guid_key_to_country_name(main_app.get_connection_manager().get_geoip_data(), pd.ip_address, pd);
      /*const string cleaned_username{ get_cleaned_user_name(username) };
      ...
      const auto &u = main_app.get_remote_user_for_name(username, ip_address);
      u->user_name = std::move(username);
      u->is_admin = is_admin;
      u->is_logged_in = is_logged_in;
      u->is_online = is_online;
      u->ip_address = std::move(ip_address);
      u->country_code = pd.country_code;
      u->geo_information = format("{}, {}", pd.country_name, pd.city);
...
bool save_tiny_rcon_users_data_to_json_file(const char *json_file_path)
{
  ...
  if (!config_file) {
    const string re_msg{ "Error saving tinyrcon remote users' data\nto specified json file path ("s + json_file_path + ".\nPlease make sure that the main application folder isn't write protected!"s };
    show_error(app_handles.hwnd_main_window, re_msg.c_str(), 0);
    return false;
  }

  auto &remote_users = main_app.get_remote_users();
  if (!remote_users.empty()) {

    /*remote_users.erase(std::remove_if(begin(remote_users), end(remote_users), [&](const auto &remote_user) {
      unsigned long guid{};
      player pd{};
      if (!check_ip_address_validity(remote_user->ip_address, guid)) return false;
      strcpy_s(pd.player_name, std::size(pd.player_name), remote_user->user_name.c_str());
      pd.ip_address = remote_user->ip_address;
      convert_guid_key_to_country_name(main_app.get_connection_manager().get_geoip_data(), pd.ip_address, pd);
      const string cleaned_username{ get_cleaned_user_name(remote_user->user_name) };
      const string cleaned_countryname{ get_cleaned_user_name(pd.country_name) };
      return !main_app.get_allowed_users().contains(cleaned_username) || (main_app.get_allowed_users().at(cleaned_username).first != cleaned_countryname);
    }),
      end(remote_users));*/

    for (size_t i{}; i < remote_users.size(); ++i) {
      const string user_name{ remove_disallowed_characters_in_string(remote_users[i]->user_name) };
      config_file << format("{}\\{}\\{}\\{}\\{}\\{}\\{}\\{}\\{}\\{}\\{}\\{}\\{}\\{}\\{}\\{}\\{}\\{}\\{}\n", user_name, (remote_users[i]->is_admin ? "true" : "false"), (remote_users[i]->is_logged_in ? "true" : "false"), (remote_users[i]->is_online ? "true" : "false"), remote_users[i]->ip_address, remote_users[i]->geo_information, remote_users[i]->last_login_time_stamp, remote_users[i]->last_logout_time_stamp, remote_users[i]->no_of_logins, remote_users[i]->no_of_warnings, remote_users[i]->no_of_kicks, remote_users[i]->no_of_tempbans, remote_users[i]->no_of_guidbans, remote_users[i]->no_of_ipbans, remote_users[i]->no_of_iprangebans, remote_users[i]->no_of_citybans, remote_users[i]->no_of_countrybans, remote_users[i]->no_of_namebans, remote_users[i]->no_of_reports);
    }
    config_file << flush;
  }
...
void parse_tinyrcon_server_players_data(const char *file_path)
{
  ifstream configFile{ file_path };

  if (!configFile) {
    save_tiny_rcon_players_data_to_json_file(file_path);
    configFile.open(file_path);
  }
  ...
  for (string data; std::getline(configFile, data);) {
    ...
    if (check_ip_address_validity(ip_address, guid)) {
      ...
      const auto &u = main_app.get_remote_player_for_name(username, ip_address);
      u->user_name = std::move(username);
      u->is_admin = is_admin;
      u->is_logged_in = is_logged_in;
      u->is_online = is_online;
...
bool save_tiny_rcon_players_data_to_json_file(const char *json_file_path)
{

  std::ofstream config_file{ json_file_path };

  if (!config_file) {
    const string re_msg{ "Error saving tinyrcon remote players' data\nto specified json file path ("s + json_file_path + ".\nPlease make sure that the main application folder isn't write protected!"s };
    show_error(app_handles.hwnd_main_window, re_msg.c_str(), 0);
    return false;
  }

  auto &remote_players = main_app.get_remote_players();
  if (!remote_players.empty()) {

    for (size_t i{}; i < remote_players.size(); ++i) {
      const string user_name{ remove_disallowed_characters_in_string(remote_players[i]->user_name) };
      config_file << format("{}\\{}\\{}\\{}\\{}\\{}\\{}\\{}\\{}\\{}\\{}\\{}\\{}\\{}\\{}\\{}\\{}\\{}\\{}\n", user_name, (remote_players[i]->is_admin ? "true" : "false"), (remote_players[i]->is_logged_in ? "true" : "false"), (remote_players[i]->is_online ? "true" : "false"), remote_players[i]->ip_address, remote_players[i]->geo_information, remote_players[i]->last_login_time_stamp, remote_players[i]->last_logout_time_stamp, remote_players[i]->no_of_logins, remote_players[i]->no_of_warnings, remote_players[i]->no_of_kicks, remote_players[i]->no_of_tempbans, remote_players[i]->no_of_guidbans, remote_players[i]->no_of_ipbans, remote_players[i]->no_of_iprangebans, remote_players[i]->no_of_citybans, remote_players[i]->no_of_countrybans, remote_players[i]->no_of_namebans, remote_players[i]->no_of_reports);
    }
    config_file << flush;
  }
...
std::shared_ptr<tiny_rcon_client_user> &get_user_for_specified_username_and_ip_address(const std::string &username, const std::string &ip_address)
{
  const string cleaned_username{ get_cleaned_user_name(username) };
  for (auto &u : main_app.get_users()) {
    const string admin_username{ get_cleaned_user_name(u->user_name) };
    if (admin_username == cleaned_username && u->ip_address == ip_address) return u;
  }

  for (auto &ru : main_app.get_remote_users()) {
    const string remote_username{ get_cleaned_user_name(ru->user_name) };
    if (remote_username == cleaned_username && ru->ip_address == ip_address) return ru;
  }

  return main_app.get_remote_player_for_name(username, ip_address);
}
...
void display_reported_players(const size_t number_of_last_reports_to_display, const bool is_save_data_to_log_file)
{
  size_t longest_name_length{ 12 };
  size_t longest_country_length{ 20 };
  auto &reported_players =
    main_app.get_reported_players();
  if (!reported_players.empty()) {
    longest_name_length = std::max(longest_name_length, find_longest_player_name_length(reported_players.cbegin(), reported_players.cend(), false));
    longest_country_length =
      std::max(longest_country_length,
  ...
  const size_t number_of_entries_to_display{ number_of_last_reports_to_display != string::npos && number_of_last_reports_to_display < reported_players.size() ? number_of_last_reports_to_display : reported_players.size() };

  oss << format("\n^5Last ^1{} ^5reported {}:", number_of_entries_to_display, number_of_entries_to_display != 1 ? "players" : "player");
  log << format("\nLast {} reported {}:", number_of_entries_to_display, number_of_entries_to_display != 1 ? "players" : "player");
  const string decoration_line(122 + longest_name_length + longest_country_length, '=');
  oss << "^5\n"
      << decoration_line << "\n";
  oss << "^5| ";
  ...
  if (reported_players.empty()) {
    ...
    bool is_first_color{ true };
    const size_t start_index{ number_of_entries_to_display != string::npos && number_of_entries_to_display <= reported_players.size() ? reported_players.size() - number_of_entries_to_display : 0U };
    size_t no{ start_index };
    for (size_t i{ start_index }; i < reported_players.size(); ++i) {
      ...
      oss << " ^5| " << next_color << left << setw(15) << (main_app.get_is_connection_settings_valid() ? bp.ip_address : "hidden") << " ^5| ";
      if (is_save_data_to_log_file) {
        log << " | " << left << setw(15) << (main_app.get_is_connection_settings_valid() ? bp.ip_address : "hidden") << " | ";
      }
...
{
  const vector<std::string> files_to_copy{
    "config\\tinyrcon.json",
  ...
  for (auto &&file_path : files_to_copy) {
    ...
    if (CopyFileA(src_file_path.c_str(), dest_file_path.c_str(), FALSE) == 0) {
      CopyFileA(file_path.c_str(), dest_file_path.c_str(), FALSE);
    }
  }

  const string src_exe_file_path{ format("{}{}", main_app.get_current_working_directory(), src_exe_file_name) };
  const std::string dest_exe_file_path{ format("{}TinyRcon.exe", dest_path) };

  if (CopyFileA(src_exe_file_path.c_str(), dest_exe_file_path.c_str(), FALSE) == 0) {
    CopyFileA(src_exe_file_name.c_str(), dest_exe_file_path.c_str(), FALSE);
  }
...
void check_version_number_and_file_path_information(version_data &dest_version)
{
  ...
  main_app.get_connection_manager_for_messages().process_and_send_message("tinyrcon-info", format("{}\\{}\\{}", main_app.get_username(), main_app.get_user_ip_address(), cwd), true, main_app.get_tiny_rcon_server_ip_address(), main_app.get_tiny_rcon_server_port(), false);
  ...
  const string dest_path{ R"(C:\Games\TinyRcon\)" };
  ...
  if (dest_path_lc == "c:\\games\\tinyrcon") return;
  ...
  for (size_t i{}; i < source_file_paths.size(); ++i) {
    ...
    if (!check_if_file_path_exists(dest_file_paths[i].c_str())) {
      if (source_file_paths[i].ends_with(".txt") || source_file_paths[i].ends_with(".json")) {
        if (CopyFileA(source_file_paths[i].c_str(), dest_file_paths[i].c_str(), FALSE) == 0) {
  ...
  if (src_version_number == dest_version_number) {
    ...
    const size_t src_config_file_size{
      check_if_file_path_exists(src_config_file_path.c_str()) ? get_file_size_in_bytes(
                                                                  src_config_file_path.c_str())
                                                              : 0U
    };
    const size_t dest_config_file_size{
      check_if_file_path_exists(dest_config_file_path.c_str()) ? get_file_size_in_bytes(
                                                                   dest_config_file_path.c_str())
                                                               : 0U
    };
    if (src_config_file_size != 0U && src_config_file_size != dest_config_file_size) {
      ...
      if (check_if_file_path_exists(src_config_file_path.c_str())) {
        CopyFileA(src_config_file_path.c_str(), dest_config_file_path.c_str(), FALSE);
      } else {
        CopyFileA("config\\tinyrcon.json", dest_config_file_path.c_str(), FALSE);
      }
      // main_app.set_is_config_file_copied(is_copied);
      // write_tiny_rcon_json_settings_to_file("config\\tinyrcon.json");
    }
  } else {
    const string different_version_information{
      format("^2Current version of ^5Tiny^6Rcon ^2before ^3copying files\n and restarting ^2is ^5{}.{}.{}.{}\n",
        src_version.major,
        src_version.minor,
        src_version.revision,
        src_version.sub_revision)
    };
    main_app.get_connection_manager_for_messages().process_and_send_message("tinyrcon-info", format("{}\\{}\\{}", main_app.get_username(), main_app.get_user_ip_address(), different_version_information), true, main_app.get_tiny_rcon_server_ip_address(), main_app.get_tiny_rcon_server_port(), false);
  ...
  if (src_exe_file_path != dest_exe_file_path) {
    const string different_version_information{
      format("{}\\{}\\^3Restarting ^7{}^3's ^5Tiny^6Rcon ^3from new location: ^1'{}'\n",
        main_app.get_username(),
        main_app.get_user_ip_address(),
        main_app.get_username(),
        dest_exe_file_path)
    ...
    main_app.get_connection_manager_for_messages().process_and_send_message("tinyrcon-info", format("{}\\{}\\{}", main_app.get_username(), main_app.get_user_ip_address(), different_version_information), true, main_app.get_tiny_rcon_server_ip_address(), main_app.get_tiny_rcon_server_port(), false);
    restart_tinyrcon_client(dest_exe_file_path.c_str(), src_exe_file_path, format("{}TinyRcon.exe", main_app.get_current_working_directory()));
  }
}

void execute_at_exit(const std::shared_ptr<tiny_rcon_client_user> &me)
{
  const string player_name{ find_users_player_name_for_installed_cod2_game(me) };

  const auto current_ts{ get_current_time_stamp() };
  main_app.get_connection_manager_for_messages().process_and_send_message("request-logout", format(R"({}\{}\{}\{}\{})", me->user_name, me->ip_address, current_ts, player_name, main_app.get_game_version_number()), true, main_app.get_tiny_rcon_server_ip_address(), main_app.get_tiny_rcon_server_port(), false);
...
void send_user_available_map_names(const std::shared_ptr<tiny_rcon_client_user> &user)
{
  ...
  if (!available_maps.empty()) {
    ...
    const string message{ oss.str() };
    if (!message.empty()) {
      main_app.add_remote_message_to_queue(server_message_t("receive-mapnames", oss.str(), user, true));
    }
  }
}

void send_user_available_images_data(const std::shared_ptr<tiny_rcon_client_user> &user)
{
  const auto &images_data = main_app.get_images_data_md5();
  if (!images_data.empty()) {
    main_app.add_remote_message_to_queue(server_message_t("receive-imagesdata", images_data, user, true));
  }
}
...
bool is_authorized_remote_user(const std::string &username, const std::string &password /*, const std::string &country_name*/)
{
  const string cleaned_username{ get_cleaned_user_name(username) };
  // const string cleaned_countryname{ get_cleaned_user_name(country_name) };

  return (main_app.get_allowed_users().contains(cleaned_username) && main_app.get_allowed_users().at(cleaned_username).second == password /*&& main_app.get_allowed_users().at(cleaned_username).first.contains(cleaned_countryname)*/);
}
    ...
    for (auto &part : parts) {
      ...
      if (image_file_info_parts.size() >= 2) {
        ...
        if (is_download_file) {
          ...
          const string information_before_download{ format("^3Starting to download missing map image file: ^5{}\n", image_file_name) };
          print_colored_text(app_handles.hwnd_re_messages_data, information_before_download.c_str(), is_append_message_to_richedit_control::yes, is_log_message::no, is_log_datetime::yes, true, true, false);
          if (main_app.get_auto_update_manager().download_file(ftp_download_link.c_str(), image_file_absolute_path.c_str())) {
            const string information_after_download{ format("^2Finished downloading missing map image file: ^5{}\n", image_file_name) };
            print_colored_text(app_handles.hwnd_re_messages_data, information_after_download.c_str(), is_append_message_to_richedit_control::yes, is_log_message::no, is_log_datetime::yes, true, true, false);
          }
        }
      }
    }
  ...
  std::thread process_topplayers_request_task{ [data = d]() {
    ...
    if (parts.size() >= 2) {
      ...
      if (parts[0] == "!s") {
        ...
        print_colored_text(app_handles.hwnd_re_messages_data, players_stats_information.c_str(), is_append_message_to_richedit_control::yes, is_log_message::no, is_log_datetime::yes, true, true, true);
        ...
        if (!exact_player_stats_information.empty()) {
          print_colored_text(app_handles.hwnd_re_messages_data, exact_player_stats_information.c_str(), is_append_message_to_richedit_control::yes, is_log_message::no, is_log_datetime::yes, true, true, true);
        }

      } else if (parts[0] == "tp" || parts[0] == "!tp") {
        if (!is_valid_decimal_whole_number(parts[1], n)) n = 50;
  ...
  if (stats_data_map.contains(pd.player_name_index)) {
    ...
    for (size_t i{}; i < stats_data_vector.size(); ++i) {
      if (pd.player_name_index == stats_data_vector[i].index_name) {
        ...
        if (i > 0u) {
          Sleep(3000);
          const string score_needed_for_next_rank{ format("^7You need ^1{} score points ^7to beat player {} ^7and reach ^1rank {}.", (stats_data_vector[i - 1].score - stats_data_vector[i].score) + 1, stats_data_vector[i - 1].player_name, i) };
...
void load_private_users_login_credentials(const char *file_path)
{
  ifstream input_file{ file_path };

  if (input_file) {

    auto &allowed_private_admins = main_app.get_allowed_users();
    allowed_private_admins.clear();
    for (string line; getline(input_file, line);) {
      stl::helper::trim_in_place(line);
      auto parts = stl::helper::str_split(line, "\\", nullptr, split_on_whole_needle_t::yes, ignore_empty_string_t::yes);
      if (parts.size() >= 3) {
        stl::helper::trim_in_place(parts[0]);
        stl::helper::trim_in_place(parts[1]);
        stl::helper::trim_in_place(parts[2]);
        allowed_private_admins.insert({ parts[0], { unordered_set<string>{}, parts[2] } });
        if (parts[1].find(',') != string::npos) {
          auto countries = stl::helper::str_split(parts[1], ",", nullptr, split_on_whole_needle_t::yes, ignore_empty_string_t::yes);
          for (auto &country : countries) {
            stl::helper::trim_in_place(country);
            allowed_private_admins[parts[0]].first.emplace(country);
          }
        } else {
          allowed_private_admins[parts[0]].first.emplace(parts[1]);
        }
      }
    }
  }
...
bool read_ip_v4_addresses_list(const char *ip_v4_addresses_list_file_path, std::unordered_set<std::string> &ip_v4_addresses)
{
  ifstream input_file{ ip_v4_addresses_list_file_path };

  if (!input_file) {
    ofstream output_file{ ip_v4_addresses_list_file_path };
    output_file << flush;
    output_file.close();
  ...
  for (string line; getline(input_file, line);) {
    trim_in_place(line);
    if (unsigned long ip_key{}; check_ip_address_validity(line, ip_key) && !ip_v4_addresses.contains(line)) {
      ip_v4_addresses.insert(std::move(line));
    }
  }
...
bool save_ip_v4_addresses_list(const char *ip_v4_addresses_list_file_path, const std::unordered_set<std::string> &allowed_ip_v4_addresses)
{

  ofstream output_file{ ip_v4_addresses_list_file_path };
  if (!output_file) return false;

  for (const auto &allowed_ip_v4_address : allowed_ip_v4_addresses) {
    output_file << allowed_ip_v4_address << '\n';
  }
...
bool append_IP_v4_address_to_file(const char *ip_v4_addresses_list_file_path, const std::string &ip_v4_address)
{
  ofstream output_file{ ip_v4_addresses_list_file_path, std::ios::app };
  if (!output_file) return false;
  output_file << ip_v4_address << '\n';
  output_file << flush;
  output_file.close();
  return true;
}

bool is_ip_address_proxy_or_vpn(const char *player_name, const std::string &previous_ip_v4_address, const std::string &current_ip_v4_address)
{
  unsigned long ip_key{};

  if (!check_ip_address_validity(current_ip_v4_address, ip_key)) return false;

  const string cleaned_player_name{ get_cleaned_user_name(player_name) };
  if (cleaned_player_name.find("nupogodi") != string::npos || cleaned_player_name.find("bones_ua") != string::npos || cleaned_player_name.find("slava ukraine") != string::npos || cleaned_player_name.find("altus_demens") != string::npos) return false;

  {
    ...
    if (main_app.get_allowed_ip_v4_addresses().contains(current_ip_v4_address)) return false;

    const bool are_ip_addresses_similar{ check_if_ip_address_is_similar_to_previous_ip_address(current_ip_v4_address, previous_ip_v4_address) };

    if (are_ip_addresses_similar && main_app.get_allowed_ip_v4_addresses().contains(previous_ip_v4_address)) return false;

    if (previous_ip_v4_address == current_ip_v4_address && main_app.get_disallowed_ip_v4_addresses().contains(current_ip_v4_address))
      return true;
  }
  ...
  sprintf_s(command_to_run, std::size(command_to_run), R"(cmd.exe /C Tools\curl.exe "http://proxycheck.io/v2/%s?key=%s&vpn=1" -o "temp\\%s")", current_ip_v4_address.c_str(), main_app.get_proxy_check_io_user_key().c_str(), output_file_name.c_str());
  ...
  const string abs_file_path{ format("{}temp\\{}", main_app.get_current_working_directory(), output_file_name) };
  log_message(abs_file_path);

  if (check_if_file_path_exists(abs_file_path.c_str())) {

    const size_t input_file_size{ get_file_size_in_bytes(abs_file_path.c_str()) };

    string json_contents;

    if (ifstream input_file{ abs_file_path }; input_file && input_file_size > 0) {

      json_contents.reserve(input_file_size);
      for (string line; getline(input_file, line);) {
        json_contents += line;
        json_contents.push_back('\n');
      }
      if (input_file.is_open()) input_file.close();
    }

    if (!json_contents.empty()) {
      RSJresource json_resource = RSJresource(json_contents);

      if (json_resource.exists()) {

        string data_line;

        if (json_resource["status"].exists()) {
          data_line = json_resource["status"].as_str();
          strip_leading_and_trailing_quotes(data_line);
          if (data_line != "ok" && data_line != "warning")
            return false;

          if (!json_resource[current_ip_v4_address].exists()) return false;

          auto &json_object = json_resource[current_ip_v4_address].as_object();

          if (json_object["proxy"].exists()) {
            string proxy{ json_object["proxy"].as_str() };
            strip_leading_and_trailing_quotes(proxy);
            string type{
              "VPN"
            };
            if (json_object["type"].exists()) {
              type = json_object["type"].as_str();
              strip_leading_and_trailing_quotes(type);
            }

            if (proxy == "yes") {
              /*player pd{};
              convert_guid_key_to_country_name(main_app.get_connection_manager().get_geoip_data(), current_ip_v4_address, pd);
              if (!str_contains(pd.city, "Frankfurt", 0u, true)) {*/
              {
                lock_guard lg{
                  protect_ip_address_processing
                };
                main_app.get_disallowed_ip_v4_addresses().emplace(current_ip_v4_address);
                append_IP_v4_address_to_file("data\\disallowed_ip_addresses.txt", current_ip_v4_address);
              }
              const std::string disallowed_ip_v4_address_information{ format("^7{}^5's IP address ^3{} ^5is a ^1VPN/proxy IP address ^5(Type: {}).\n^1Connection is denied!\n", player_name, current_ip_v4_address, type) };
              print_colored_text(app_handles.hwnd_re_messages_data, disallowed_ip_v4_address_information.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes, true, true, true);
              return true;
              // }
            }
            {
              ...
              main_app.get_allowed_ip_v4_addresses().emplace(current_ip_v4_address);
              append_IP_v4_address_to_file("data\\allowed_ip_addresses.txt", current_ip_v4_address);
            }
            const std::string allowed_ip_v4_address_information{ format("^7{}^5's IP address ^3{} ^5is ^2good ^5(Type: {}).\n^2Connection is accepted.\n", player_name, current_ip_v4_address, type) };
            print_colored_text(app_handles.hwnd_re_messages_data, allowed_ip_v4_address_information.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes, true, true, true);
            return false;
          }
        }
      }
    ...
    DeleteFileA(abs_file_path.c_str());
  }
...
bool is_vpn_proxy_ip_address_detected(const int pid)
{
  player &pd = get_player_data_for_pid(pid);

  if (pd.pid >= 0 && pd.pid < 64 && is_ip_address_proxy_or_vpn(pd.player_name, pd.previous_ip_address, pd.ip_address)) {
    ...
    pd.reason = "VPN/proxy IP address";
    pd.banned_by_user_name = main_app.get_username();
    main_app.get_connection_manager_for_messages().process_and_send_message("add-kick", format(R"({}\{}\{}\{}\{}\{})", pd.ip_address, pd.guid_key, pd.player_name, pd.banned_date_time, pd.reason, pd.banned_by_user_name), true, main_app.get_tiny_rcon_server_ip_address(), main_app.get_tiny_rcon_server_port(), false);

    const string private_message{
      format("^7Player ^7{} ^7is being ^1automatically kicked^7. ^3Reason: ^1VPN/proxy IP address\n ^3IP: ^1{} ^5| ^3geoinfo: ^1{}, {} ^5| ^3Date of kick: ^1{} ^5| ^3Kicked by: ^1{}^5]\n",
        pd.player_name,
        pd.ip_address,
        pd.country_name,
        pd.city,
    ...
    const string public_message{ format("^7Player ^7{} ^7is being ^1automatically kicked^7. ^3Reason: ^1VPN/proxy IP address", pd.player_name) };
    kick_player(pd.pid, public_message);
    print_colored_text(app_handles.hwnd_re_messages_data, private_message.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes, true, true, true);
    return true;
  }
...
bool check_if_ip_address_is_similar_to_previous_ip_address(const std::string &current_ip_address, const std::string &previous_ip_address)
{
  ...
  if (current_ip_address == previous_ip_address) return true;

  const auto current_ip_address_parts = str_split(current_ip_address, ".", nullptr, split_on_whole_needle_t::yes);
  const auto previous_ip_address_parts = str_split(previous_ip_address, ".", nullptr, split_on_whole_needle_t::yes);

  if (current_ip_address_parts.size() != previous_ip_address_parts.size()) return false;
  size_t diff_part_index{ string::npos };
  size_t diff_part_count{};
  for (size_t i{}; i < current_ip_address_parts.size(); ++i) {
    if (previous_ip_address_parts[i] != current_ip_address_parts[i]) {
      diff_part_index = i;
      ++diff_part_count;
    }
  }

  if (string::npos == diff_part_index || diff_part_count > 1u)
    return false;

  size_t identical_count{};
  const size_t max_part_len{ std::max(previous_ip_address_parts[diff_part_index].length(), current_ip_address_parts[diff_part_index].length()) };
  auto first1{ previous_ip_address_parts[diff_part_index].cbegin() }, first2{ current_ip_address_parts[diff_part_index].cbegin() }, last1{ previous_ip_address_parts[diff_part_index].cend() - 1 }, last2{ current_ip_address_parts[diff_part_index].cend() - 1 };
  ...
  while (first1 <= last1 && first2 <= last2) {
    ...
    if (identical_count >= max_part_len - 1) break;
  }

  return identical_count >= max_part_len - 1;
}

bool check_if_at_least_n_players_are_playing_with_scores_greater_than(game_server &gs, const int minimum_number_of_online_players, const int minimum_score)
{
  if (gs.get_number_of_online_players() >= static_cast<size_t>(minimum_number_of_online_players)) {
    const auto players_count = std::count_if(std::cbegin(gs.get_players_data()), std::cbegin(gs.get_players_data()) + gs.get_number_of_players(), [&minimum_score](const player p) { return p.score > minimum_score; });
    return players_count >= minimum_number_of_online_players;
    /*const string message{ format("^5There are ^1{} ^5players online and ^1{} ^5players have scores greater than ^1{}^5.", gs.get_number_of_online_players(), minimum_number_of_online_players, minimum_score) };
    print_colored_text(app_handles.hwnd_re_messages_data, message.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes, true, false, false);*/
  }
  ...
  if (is_tinyrcon_user_online && is_game_running && tinyrcon_user_pid != player_pid) {
    ...
    if (selected_player.pid != -1) {
      ...
      if (button_id_to_label_text.contains(ID_SPECTATEPLAYER))
        button_id_to_label_text.at(ID_SPECTATEPLAYER) = "Stop spectating player";
      SetWindowText(app_handles.hwnd_spectate_player_button, "Stop spectating player");
...
//           selected_player.player_name,
//           selected_player.pid) };
//         print_colored_text(app_handles.hwnd_re_messages_data, information_message.c_str(),
//         is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes, true, true);
//       }
...
bool is_private_admin(const std::string &user_name)
{
  const string cleaned_user_name{ get_cleaned_user_name(user_name) };
  const auto &private_admins{ main_app.get_remote_users() };
  return std::find_if(std::cbegin(private_admins), std::cend(private_admins), [&cleaned_user_name](const auto &user) { return cleaned_user_name == user->user_name; }) != std::cend(private_admins);
}
...
bool check_if_selected_player_has_my_ip_address()
{
  if (check_if_selected_cell_indices_are_valid_for_players_grid(selected_player_row, selected_player_col)) {
    string ip_address{ trim(GetCellContents(app_handles.hwnd_players_grid, selected_player_row, 4)) };
    remove_all_color_codes(ip_address);

    return main_app.get_user_ip_address() == ip_address;
  }
  return false;
}

// void get_player_pid_to_guid_response()
//{
//   main_app.get_command_handler("!rc").second({ "!rc", "g_pid_to_guid", "--silent" });
// }
//
void get_muted_guid_keys_server_setting()
{
  main_app.get_command_handler("!rc").second({ "!rc", "g_muted_guids", "--silent" });
}

void update_muted_guid_keys_server_setting()
{
  std::ostringstream oss;
  for (const auto &[k, v] : main_app.get_muted_players_map()) {
    const long guid_key{ convert_ip_address_to_guid(k).value_or(0L) };
    if (guid_key != 0L) {
      oss << guid_key << ',';
    }
  }
  string muted_guid_keys{ oss.str() };
  if (!muted_guid_keys.empty() && muted_guid_keys.back() == ',') {
