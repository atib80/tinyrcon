...
{
  string ex_msg{ format(R"(^1Exception ^3thrown from ^1size_t connection_manager::receive_non_rcon_reply_from_server member function:
remote_ip="{}",
remote_port={},
received_reply:
"{}",
  ...
  if (noOfAllReceivedBytes > 0) {
    ...
    if (str_starts_with(received_reply, "inforesponse", true)) {
      ...
      while (current < received_reply.c_str() + received_reply.length() && *current) {
        ...
        players_data[player_num].player_name_index = get_cleaned_user_name(players_data[player_num].player_name);
...
{
  (void)snprintf(buffer, buffer_size, "\xFF\xFF\xFF\xFFrcon %s %s\0", rcon_password, rcon_command_to_send);
}
  ...
  try {
    ...
    while (true) {
      ...
      if (noOfReceivedBytes > 0U) {
        ...
        if (received_reply.find(end_needle_to_search_for) != string::npos) break;
      }
    }

    if (noOfAllReceivedBytes > 0) {

      if (!str_contains(received_reply, "rcon_password", 0u, true) && !str_contains(received_reply, "sv_privatepassword", 0u, true)) {
        main_app.get_remote_replies_queue().emplace(received_reply);
      }
      ...
      if (received_reply.starts_with("Invalid password.") || received_reply.starts_with("Bad rcon")) {
        ...
        if (received_reply.find(rcon_status_response_needle1) != string::npos
          ...
          if (previous_map.empty() || current_map != previous_map) {
            ...
            main_app.get_stats_data().get_previous_player_score_and_timestamp().clear();
          ...
          if (!is_server_empty_or_error_receiving_udp_datagrams) {
            ...
            for (const string &player_line : lines) {
              ...
              if (gn == game_name_t::cod1 || gn == game_name_t::cod2) {
                while ((isdigit(player_line[j]) || '-' == player_line[j]) && j < i) ++j;
              } else {
                while ((isxdigit(player_line[j]) || '-' == player_line[j]) && j < i) ++j;
              }
              ...
              if (regex_search(player_line, bot_ip_regex_smatch, bot_ip_address_regex)) {
                ...
                if (string::npos != j) {
                  if ('.' == player_line[j]) {// 123.85.101.31115678
                    // j = player_line.rfind('.');
              ...
              if (players_data[pl_index].previous_ip_address.empty()) {
                players_data[pl_index].previous_ip_address = players_data[pl_index].ip_address = main_app.get_pid_to_ip_address().contains(player_pid) ? main_app.get_pid_to_ip_address().at(player_pid).first : ip_address;
              } else {
                players_data[pl_index].previous_ip_address = players_data[pl_index].ip_address;
            ...
            if (lines.size() != pl_index) {
              const string warning_message{ format(R"(^3Received ^1rcon status ^3contains more lines ^1({}) ^3than the actual number of parsed player entities ^1({})^3!)", lines.size(), pl_index) };
              print_colored_text(app_handles.hwnd_re_messages_data, warning_message.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes, true, false, false);
          ...
          if (check_if_at_least_n_players_are_playing_with_scores_greater_than(gs, 2, 0)) {
    ...
    const string ex_msg{
      format(R"(^1Custom exception ^3thrown from ^1size_t connection_manager::receive_rcon_reply_from_server ^3member function:
Exception message: {},
remote_ip="{}",
remote_port={},
gs,
...
is_process_reply={}
    ...
    const string ex_msg{ format(R"(^1Generic exception ^3thrown from ^1size_t connection_manager::receive_rcon_reply_from_server ^3member function.
remote_ip="{}",
remote_port={},
gs,
received_reply:
