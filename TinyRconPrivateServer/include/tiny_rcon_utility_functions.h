...
std::pair<bool, std::string> create_necessary_file_path(const std::string &file_path);
...
// template<typename Iter>
// size_t find_longest_player_name_length_for_chat(
//   Iter first,
//   const Iter last,
//   const bool count_color_codes)
//{
...
//   while (first != last) {
//     max_player_name_length =
//       std::max<size_t>(count_color_codes ? stl::helper::len(first->player_name) : get_number_of_characters_printed_in_rcon_chat(first->player_name), max_player_name_length);
//     ++first;
//   }
//
//   return max_player_name_length;
...
std::pair<bool, player> remove_muted_ip_address(std::string &ip_address, std::string &message, const bool is_report_public_message = true);

size_t print_colored_text(HWND re_control, const char *text, const is_append_message_to_richedit_control = is_append_message_to_richedit_control::yes, const is_log_message = is_log_message::yes, is_log_datetime = is_log_datetime::yes, const bool is_remove_color_codes_for_log_message = true, const bool is_display_message_to_remote_user = true, const bool is_send_message_to_player = true);
size_t print_message(HWND re_control, const std::string &text, const is_log_message log_to_file = is_log_message::yes, is_log_datetime is_log_current_date_time = is_log_datetime::yes, const bool is_remove_color_codes_for_log_message = true, const bool is_display_message_to_remote_user = true, const bool is_send_message_to_player = false);
...
void display_muted_players_information(const size_t number_of_last_muted_players_to_display = std::string::npos, const bool is_save_data_to_log_file = false);
...
class stats_data;
std::string get_top_players_stats_data(std::vector<player_stats> &stats_data, std::unordered_map<std::string, player_stats> &stats_data_map, const size_t number_of_top_players, std::string &public_message, const char *title, std::string partial_or_full_player_name = "", const bool find_exact_player_name_match = false);
...
bool validate_admin_and_show_missing_admin_privileges_message(const std::shared_ptr<tiny_rcon_client_user> &user, const bool is_show_message_box, const is_log_message log_message = is_log_message::no, const is_log_datetime log_date_time = is_log_datetime::no);
...
void parse_tinyrcon_server_players_data(const char *);
bool save_tiny_rcon_users_data_to_json_file(const char *);
bool save_tiny_rcon_players_data_to_json_file(const char *);
...
bool is_authorized_remote_user(const std::string &username, const std::string &password /*, const std::string &country*/);
...
bool check_if_at_least_n_players_are_playing_with_scores_greater_than(game_server &gs, const int minimum_number_of_online_players = 2, const int minimum_score = 0);
...
LRESULT CALLBACK monitor_game_key_press_events(
  _In_ int code,
  _In_ WPARAM wParam,
  _In_ LPARAM lParam);
