...
class tiny_rcon_client_application
{
  ...
  std::string remote_users_data_file_path{ "data\\remote_users.txt" };
  std::string remote_players_data_file_path{ "data\\remote_players.txt" };
  ...
  string proxy_check_io_user_key{ "409730-80vn94-4872th-782t10" };
  ...
  std::unordered_map<std::string, std::string> admin_messages{
    { "user_defined_mute_message", "^7{PLAYERNAME} ^1you have been muted by admin ^5{ADMINNAME}. ^3Reason: ^1{REASON}" },
    { "user_defined_unmute_message", "^7{PLAYERNAME} ^1you have been unmuted by admin ^5{ADMINNAME}. ^3Reason: ^1{REASON}" },
  ...
  std::unordered_set<std::string> allowed_ip_v4_addresses;
  std::unordered_set<std::string> disallowed_ip_v4_addresses;
  std::unordered_map<std::string, std::pair<std::unordered_set<std::string>, std::string>> allowed_users;
  ...
  connection_manager_for_remote_messages cm_for_remote_messages;
  std::queue<command_t> command_queue{};
  std::queue<message_t> message_queue{};
  std::queue<int> player_pid_queue{};
  std::queue<server_message_t> remote_messages_queue{};
  std::queue<std::string> remote_replies_queue{};
  std::queue<std::string> rcon_status_responses_for_players_queue{};
  std::queue<print_message_t> tinyrcon_messages_to_print;

  std::queue<std::shared_ptr<tiny_rcon_client_user>> welcome_messages_to_send_to_players{};
  std::queue<std::shared_ptr<tiny_rcon_client_user>> welcome_messages_to_send_to_admins{};

  std::vector<std::shared_ptr<tiny_rcon_client_user>> users;
  std::vector<std::shared_ptr<tiny_rcon_client_user>> remote_users;
  std::vector<std::shared_ptr<tiny_rcon_client_user>> remote_players;
  ...
  std::unordered_map<std::string, std::shared_ptr<tiny_rcon_client_user>> name_to_remote_user;
  std::unordered_map<std::string, std::shared_ptr<tiny_rcon_client_user>> name_to_remote_player;
  ...
  std::unordered_map<std::string, std::function<void(const std::string &, time_t, const std::string &, const bool, const string &)>> remote_message_handlers;
...
public:
  ...
  bool get_is_enable_automatic_vpn_proxy_ip_address_detection() const noexcept
  {
    return is_enable_automatic_vpn_proxy_ip_address_detection;
  }

  void set_is_enable_automatic_vpn_proxy_ip_address_detection(const bool new_value) noexcept
  {
    is_enable_automatic_vpn_proxy_ip_address_detection = new_value;
  }
  ...
  inline std::condition_variable &get_ip_v4_addresses_queue_cv()
  {
    return player_pid_queue_cv;
  }

  inline std::recursive_mutex &get_ip_v4_addresses_queue_mutex()
  {
    return player_pid_queue_mutex;
  }
  ...
  {
    remove_disallowed_characters_in_string(new_value);
  ...
  inline connection_manager_for_remote_messages &get_connection_manager_for_remote_messages() noexcept
  {
    return cm_for_remote_messages;
  }

  inline std::unordered_set<std::string> &get_allowed_ip_v4_addresses() noexcept
  {
    return allowed_ip_v4_addresses;
  }

  inline std::unordered_set<std::string> &get_disallowed_ip_v4_addresses() noexcept
  {
    return disallowed_ip_v4_addresses;
  }

  inline std::unordered_map<std::string, std::pair<std::unordered_set<std::string>, std::string>> &
    get_allowed_users() noexcept
  {
  ...
  void set_is_user_data_received_for_user(const std::string &name)
  {
    string cleaned_name{ get_cleaned_user_name(name) };
  ...
  std::vector<std::shared_ptr<tiny_rcon_client_user>> &get_remote_users()
  {
    return remote_users;
  }

  // remote_players
  std::vector<std::shared_ptr<tiny_rcon_client_user>> &get_remote_players()
  {
  ...
  std::shared_ptr<tiny_rcon_client_user> &get_remote_user_for_name(const std::string &name, const string &ip_address)
  {
    ...
    return name_to_remote_user.at(cleaned_name);
  }

  std::shared_ptr<tiny_rcon_client_user> &get_remote_player_for_name(const std::string &name, const string &ip_address)
  {
    string cleaned_name{ get_cleaned_user_name(name) };
    ...
    if (!name_to_remote_player.contains(cleaned_name)) {
      remote_players.emplace_back(std::make_shared<tiny_rcon_client_user>());
      remote_players.back()->user_name = name;
      remote_players.back()->ip_address = ip_address;
      name_to_remote_player.emplace(cleaned_name, remote_players.back());
    }

    return name_to_remote_player.at(cleaned_name);
  }
  ...
  void set_current_working_directory()
  {
    ...
    remote_users_data_file_path.assign(format("{}{}", current_working_directory, remote_users_data_file_path));
    remote_players_data_file_path.assign(format("{}{}", current_working_directory, remote_players_data_file_path));
    reported_players_file_path.assign(format("{}{}", current_working_directory, reported_players_file_path));
    // muted_players_file_path.assign(format("{}{}", current_working_directory, muted_players_file_path));
  ...
  const char *get_remote_users_data_file_path() const noexcept
  {
    return remote_users_data_file_path.c_str();
  }

  const char *get_remote_players_data_file_path() const noexcept
  {
    return remote_players_data_file_path.c_str();
  }
  ...
  const std::string &get_private_tiny_rcon_server_ip_address() const
  {
    return private_tiny_rcon_server_ip_address;
  }

  void set_private_tiny_rcon_server_ip_address(string new_tiny_rcon_server_ip_address)
  {
    private_tiny_rcon_server_ip_address = std::move(new_tiny_rcon_server_ip_address);
  }

  uint_least16_t get_private_tiny_rcon_server_port() const
  {
    return private_tiny_rcon_server_port;
  }

  void set_private_tiny_rcon_server_port(const int new_tiny_rcon_server_port)
  {
    private_tiny_rcon_server_port = static_cast<uint_least16_t>(new_tiny_rcon_server_port);
  }
  ...
  inline const std::string &get_proxy_check_io_user_key() const noexcept
  {
    return proxy_check_io_user_key;
  }

  inline void set_proxy_check_io_user_key(std::string new_value) noexcept
  {
    proxy_check_io_user_key = std::move(new_value);
  }
  ...
  bool is_player_pid_queue_empty()
  {
    std::lock_guard lg{ player_pid_queue_mutex };
    return player_pid_queue.empty();
  }

  inline int get_player_pid_from_queue()
  {
    std::lock_guard lg{ player_pid_queue_mutex };
    const int pid = player_pid_queue.front();
    player_pid_queue.pop();
    return pid;
  }

  inline void add_player_pid_to_queue(const int pid)
  {
    {
      std::lock_guard lg{ player_pid_queue_mutex };
      player_pid_queue.emplace(pid);
    }
    player_pid_queue_cv.notify_one();
  }
  ...
  bool is_remote_message_queue_empty()
  {
    std::lock_guard lg{ remote_message_queue_mutex };
    return remote_messages_queue.empty();
  }

  inline void add_remote_message_to_queue(server_message_t message)
  {
    std::lock_guard lg{ remote_message_queue_mutex };
    remote_messages_queue.emplace(std::move(message));
  }

  inline server_message_t get_remote_message_from_queue()
  {
    std::lock_guard lg{ remote_message_queue_mutex };
    auto next_message = std::move(remote_messages_queue.front());
    remote_messages_queue.pop();
    return next_message;
  }

  inline std::queue<std::string> &get_remote_replies_queue() noexcept
  {
    std::lock_guard lg{ remote_message_queue_mutex };
    return remote_replies_queue;
  }

  inline std::queue<std::string> &get_rcon_status_responses_for_players_queue() noexcept
  {
    return rcon_status_responses_for_players_queue;
  }

  inline std::queue<std::shared_ptr<tiny_rcon_client_user>> &get_welcome_messages_to_players() noexcept
  {
    return welcome_messages_to_send_to_players;
  }

  inline std::queue<std::shared_ptr<tiny_rcon_client_user>> &get_welcome_messages_to_admins() noexcept
  {
    return welcome_messages_to_send_to_admins;
  }
  ...
  const std::unordered_map<std::string, std::function<void(const std::string &, time_t, const std::string &, const bool, const string &)>> &get_remote_message_handlers() const
  {
    return remote_message_handlers;
  }

  void add_remote_message_handler(std::string remote_message_name, std::function<void(const std::string &, time_t, const std::string &, const bool, const string &)> remote_message_handler)
  {
    remote_message_handlers.emplace(std::move(remote_message_name), std::move(remote_message_handler));
  }

  const std::function<void(const std::string &, time_t, const std::string &, const bool, const string &)> &get_remote_message_handler(const std::string &remote_message_name) const
  {
    static std::function<void(const std::string &, time_t, const std::string &, const bool, const string &)> unknown_remote_message_handler{
