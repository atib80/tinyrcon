...
std::u8string get_type_name_wstr(T &&t)
{
  ...
  std::transform(std::cbegin(cstr), std::cend(cstr), std::begin(u8str), [](const auto ch) { return static_cast<char8_t>(ch); });
  return u8str;
}
#endif
...
std::wstring get_type_name_wstr(T &&t)
{
  ...
  std::transform(std::cbegin(cstr), std::cend(cstr), std::begin(wstr), [](const auto ch) { return static_cast<wchar_t>(ch); });
  return wstr;
}
...
std::u16string get_type_name_u16str(T &&t)
{
  ...
  std::transform(std::cbegin(cstr), std::cend(cstr), std::begin(u16str), [](const auto ch) { return static_cast<char16_t>(ch); });
  return u16str;
}
...
std::u32string get_type_name_u32str(T &&t)
{
  ...
  std::transform(std::cbegin(cstr), std::cend(cstr), std::begin(u32str), [](const auto ch) { return static_cast<char32_t>(ch); });
  return u32str;
}
...
template<
  ...
  typename U,
  typename = std::enable_if_t<(
    is_char_array_type_v<T> || is_char_pointer_type_v<T> || is_valid_string_type_v<T> || is_valid_string_view_type_v<T>)&&(is_char_array_type_v<U> || is_char_pointer_type_v<U> || is_valid_string_type_v<U> || is_valid_string_view_type_v<U> || is_valid_char_type_v<U>)&&std::is_same_v<get_char_type_t<T>,
    get_char_type_t<U>>>>
...
template<typename T,
  typename U,
  typename = std::enable_if_t<
    (is_valid_string_type_v<T> || is_valid_string_view_type_v<T> || is_char_pointer_type_v<T> || is_char_array_type_v<T>)&&(is_valid_string_type_v<U> || is_valid_string_view_type_v<U> || is_char_pointer_type_v<U> || is_char_array_type_v<U>)&&std::
      is_same_v<get_char_type_t<T>, get_char_type_t<U>>>>
...
template<
  ...
  typename V,
  typename = std::enable_if_t<(
    is_valid_string_type_v<T> || is_valid_string_view_type_v<T> || is_char_pointer_type_v<T> || is_char_array_type_v<T>)&&(is_valid_char_type_v<U> || is_valid_string_type_v<U> || is_valid_string_view_type_v<U> || is_char_pointer_type_v<U> || is_char_array_type_v<U>)&&(is_valid_char_type_v<V> || is_valid_string_type_v<V> || is_valid_string_view_type_v<V> || is_char_pointer_type_v<V> || is_char_array_type_v<V>)&&is_all_of_v<get_char_type_t<T>,
    get_char_type_t<U>,
    get_char_type_t<V>>>>
...
template<
  ...
  typename U,
  typename = std::enable_if_t<(
    is_valid_string_type_v<T> || is_valid_string_view_type_v<T> || is_char_pointer_type_v<T> || is_char_array_type_v<T>)&&(is_valid_char_type_v<U> || is_valid_string_type_v<U> || is_valid_string_view_type_v<U> || is_char_pointer_type_v<U> || is_char_array_type_v<U>)&&std::is_same_v<get_char_type_t<T>,
    get_char_type_t<U>>>>
...
template<
  ...
  typename = std::enable_if_t<
    (is_valid_string_type_v<T> || is_valid_string_view_type_v<T>)&&(
      is_valid_string_type_v<U> || is_valid_string_view_type_v<U>)&&(std::is_same_v<get_char_type_t<T>, get_char_type_t<U>>)>>
...
template<
  ...
  typename = std::enable_if_t<
    (is_char_array_type_v<T> || is_char_pointer_type_v<T>)&&(
      is_char_array_type_v<U> || is_char_pointer_type_v<U>)&&(std::is_same_v<get_char_type_t<T>,
      get_char_type_t<U>>)>>
auto strstri(T src, U needle, const std::locale &loc = std::locale{})
{
  ...
  if (std::has_facet<std::ctype<char_type>>(loc)) {
    ...
    std::transform(src_sv.cbegin(), src_sv.cend(), std::begin(src_lc), [&f](const auto ch) { return f.tolower(ch); });

    std::transform(needle_sv.cbegin(), needle_sv.cend(), std::begin(needle_lc), [&f](const auto ch) { return f.tolower(ch); });
  } else {
    std::transform(
      src_sv.cbegin(), src_sv.cend(), std::begin(src_lc), [](const auto ch) { return static_cast<char_type>(std::tolower(ch)); });

    std::transform(
      needle_sv.cbegin(), needle_sv.cend(), std::begin(needle_lc), [](const auto ch) { return static_cast<char_type>(std::tolower(ch)); });
  }
  ...
  const std::locale &loc = std::locale{})
{
  ...
  if (std::has_facet<std::ctype<char_type>>(loc)) {
    const auto &f = std::use_facet<std::ctype<char_type>>(loc);
    std::transform(src_sv.cbegin(), src_sv.cend(), std::begin(src_uc), [&f](const auto ch) { return f.toupper(ch); });
  } else {
    std::transform(src_sv.cbegin(), src_sv.cend(), std::begin(src_uc), [](const auto ch) { return static_cast<char_type>(std::toupper(ch)); });
  }
...
enum class number_base { decimal,
  hexadecimal,
  octal,
  binary };
    ...
    std::numeric_limits<remove_cv_ref_t<T>>::digits10)
{
  ...
  std::transform(buffer, buffer + buffer_len + 1, std::back_inserter(number_str), [](const auto ch) { return static_cast<char16_t>(ch); });
    ...
    std::numeric_limits<remove_cv_ref_t<T>>::digits10)
{
  ...
  std::transform(buffer, buffer + buffer_len + 1, std::back_inserter(number_str), [](const auto ch) { return static_cast<char32_t>(ch); });
...
template<
  ...
  typename = std::enable_if_t<(
    is_valid_string_type_v<T> || is_valid_string_view_type_v<T> || is_char_array_type_v<T> || is_char_pointer_type_v<T>)&&(is_valid_string_type_v<U> || is_valid_string_view_type_v<U> || is_char_array_type_v<U> || is_char_pointer_type_v<U> || is_valid_char_type_v<U>)&&(is_valid_string_type_v<V> || is_valid_string_view_type_v<V> || is_char_array_type_v<V> || is_char_pointer_type_v<V> || is_valid_char_type_v<V> || std::is_same_v<V, std::nullptr_t>)&&((std::
                                                                                                                                                                                                                                                                                                                                                                                                                                                                        is_same_v<
                                                                                                                                                                                                                                                                                                                                                                                                                                                                          get_char_type_t<T>,
                                                                                                                                                                                                                                                                                                                                                                                                                                                                          get_char_type_t<
                                                                                                                                                                                                                                                                                                                                                                                                                                                                            U>>
...
template<
  ...
  typename NeedleType,
  typename = std::enable_if_t<(
    is_valid_char_type_v<NeedleType> || is_char_pointer_type_v<NeedleType> || is_char_array_type_v<NeedleType> || is_valid_string_type_v<NeedleType> || is_valid_string_view_type_v<NeedleType>)&&has_output_stream_operator_v<std::
                                                                                                                                                                                                                                 basic_ostream<
                                                                                                                                                                                                                                   get_char_type_t<
                                                                                                                                                                                                                                     NeedleType>>,
    typename std::
