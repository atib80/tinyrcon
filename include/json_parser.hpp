...
public:
  ...
  // parser (new)
  void fast_parse(std::string *str_p = NULL, bool copy_string = false, size_t max_depth = std::numeric_limits<size_t>::max(), size_t *parse_start_str_pos = nullptr);// TODO: finish.
  ...
  RSJresource &operator[](const size_t indx);// array
...
inline int seek_next(std::string *, int, char)
{
}

inline void RSJresource::fast_parse(std::string *str_p, bool, size_t, size_t *parse_start_str_pos)
{
  ...
  while (*parse_start_str_pos < str_p->length()) {// *
    ...
    if (quote_stack.size() == 0) {// comment cannot start inside string
      ...
      if (str.compare(a, RSJlinecommentstart.length(), RSJlinecommentstart) == 0) {
        ...
        if (newline_pos != std::string::npos)
          a = static_cast<int>(newline_pos);// point to the newline character (a will be incremented)
        else// the comment continues until EOF
          a = static_cast<int>(str.length());
        continue;
      }
    }
...
inline RSJresource &RSJresource::operator[](const size_t indx)
{// returns reference
  as_array();
