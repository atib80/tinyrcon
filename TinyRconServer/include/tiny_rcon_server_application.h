...
class tiny_rcon_server_application
{
    ...
    std::unordered_map<std::string, std::string> admin_messages{
        ...
        {"automatic_remove_temp_ban_msg", "^1{ADMINNAME}: ^7{PLAYERNAME}'s ^1temporary ban ^7[start date: "
                                          "^3{TEMP_BAN_START_DATE} ^7expired on ^3{TEMP_BAN_END_DATE}]{{br}}^7has "
                                          "automatically been removed. ^5Reason of ban: ^1{REASON}"},
        {"automatic_kick_temp_ban_msg", "^1{ADMINNAME}: ^7Temporarily banned player {PLAYERNAME} ^7is being "
        ...
        {"automatic_kick_ip_ban_msg", "^1{ADMINNAME}: ^7Player {PLAYERNAME} ^7with a previously ^1banned IP "
                                      "address ^7is being automatically ^1kicked.{{br}}^5Reason of ban: "
                                      "^1{REASON} ^7| ^5Date of ban: ^1{IP_BAN_DATE}"},
        {
            "automatic_kick_city_ban_msg",
            "^1{ADMINNAME}: ^7Player {PLAYERNAME} ^7with an IP address from a "
            "^1banned city: ^5{CITY_NAME} ^7is being automatically ^1kicked.",
        },
        {"automatic_kick_country_ban_msg", "^1{ADMINNAME}: ^7Player {PLAYERNAME} ^7with an IP address from a "
                                           "^1banned country:  ^5{COUNTRY_NAME} ^7is being automatically "
                                           "^1kicked."}};
    ...
    std::unordered_map<
        std::string, std::function<void(const std::string &, time_t, const std::string &, const bool, const string &)>>
        message_handlers;
  ...
  public:
    ...
    inline game_server &get_game_server() noexcept
    {
        return server;
    }
    ...
    std::shared_ptr<tiny_rcon_client_user> &get_user_for_name(const std::string &name, const string &ip_address)
    {
        ...
        // player pd{};
        // convert_guid_key_to_country_name(cm_for_messages.get_geoip_data(),
        // ip_address, pd);
    ...
    bool is_user_admin(const std::string &name, const std::string &password) const noexcept
    {
        ...
        // player pd{};
        // convert_guid_key_to_country_name(cm_for_messages.get_geoip_data(),
        // ip_address, pd); if (cleaned_name == "admin") {
    ...
    void set_current_working_directory()
    {
        ...
        if (const auto path_len = GetModuleFileNameA(nullptr, exe_file_path.get(), max_path_limit); path_len != 0)
        {
            ...
            current_working_directory.assign(
                string(cbegin(exe_file_path_sv), cbegin(exe_file_path_sv) + exe_file_path_sv.rfind('\\') + 1));
        }
        else
        {
        ...
        protected_ip_addresses_file_path.assign(
            format("{}{}", current_working_directory, protected_ip_addresses_file_path));
        protected_ip_address_ranges_file_path.assign(
            format("{}{}", current_working_directory, protected_ip_address_ranges_file_path));
        ...
        removed_ip_range_bans_file_path.assign(
            format("{}{}", current_working_directory, removed_ip_range_bans_file_path));
        removed_banned_countries_file_path.assign(
            format("{}{}", current_working_directory, removed_banned_countries_file_path));
        removed_banned_cities_file_path.assign(
            format("{}{}", current_working_directory, removed_banned_cities_file_path));
        removed_banned_names_file_path.assign(
            format("{}{}", current_working_directory, removed_banned_names_file_path));
    }
    ...
    const std::unordered_map<std::string, std::function<void(const std::string &, time_t, const std::string &,
    ...
    void add_message_handler(
        ...
        std::function<void(const std::string &, time_t, const std::string &, const bool, const string &)>
            message_handler)
    {
        message_handlers.emplace(std::move(message_name), std::move(message_handler));
    }

    const std::function<void(const std::string &, time_t, const std::string &, const bool, const string &)> &
    get_message_handler(const std::string &message_name) const
    {
        static std::function<void(const std::string &, time_t, const std::string &, const bool, const string &)>
    ...
    tinyrcon_activities_stats &get_tinyrcon_stats_data() noexcept
    {
        return tinyrcon_stats_data;
    }
    ...
    std::string get_welcome_message(const std::string &user_name)
    {
        ...
        oss << std::format("^5Admins have so far...\n   ^1warned ^3{} ^5{},\n   ^1kicked ^3{} "
                           ...
                           no_of_kicks != 1 ? "players" : "player", no_of_tempbans,
                           no_of_tempbans != 1 ? "players" : "player");
        oss << std::format("   ^1permanently banned GUID {} ^5of ^3{} ^5{},\n   ^1banned IP {} ^5of "
                           "^3{} ^5{},\n   ^1banned IP address range(s) ^5of ^3{} ^5{},\n   "
                           "^1banned ^3{} ^1player {}^5,\n",
                           no_of_guid_bans != 1 ? "keys" : "key", no_of_guid_bans,
                           no_of_guid_bans != 1 ? "players" : "player", no_of_ip_bans != 1 ? "addresses" : "address",
                           no_of_ip_bans, no_of_ip_bans != 1 ? "players" : "player", no_of_ip_address_range_bans,
                           no_of_ip_address_range_bans != 1 ? "players" : "player", no_of_name_bans,
                           no_of_name_bans != 1 ? "names" : "name");
        oss << std::format(
            "   ^1banned {} ^5of ^3{} ^5{},\n   ^1banned {} ^5of ^3{} ^5{},\n   "
            "^1protected IP {} ^5of ^3{} ^5{},\n",
            no_of_city_bans != 1 ? "cities" : "city", no_of_city_bans, no_of_city_bans != 1 ? "players" : "player",
            no_of_country_bans != 1 ? "countries" : "country", no_of_country_bans,
            no_of_country_bans != 1 ? "players" : "player", no_of_protected_ip_addresses != 1 ? "addresses" : "address",
            no_of_protected_ip_addresses, no_of_protected_ip_addresses != 1 ? "players" : "player");
        oss << std::format("   ^1protected IP address {} ^5of ^3{} ^5{},\n   ^1protected {} ^5of "
                           ...
                           no_of_protected_ip_address_ranges != 1 ? "players" : "player",
                           no_of_protected_cities != 1 ? "cities" : "city", no_of_protected_cities,
                           no_of_protected_cities != 1 ? "players" : "player",
                           no_of_protected_countries != 1 ? "countries" : "country", no_of_protected_countries,
                           no_of_protected_countries != 1 ? "players" : "player");
