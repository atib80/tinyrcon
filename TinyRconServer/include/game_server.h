...
class game_server
{
  public:
    ...
    [[nodiscard]] uint_least16_t get_server_port() const noexcept
    {
        return port;
    }
    ...
    void set_current_number_of_players(const int new_current_number_of_players) noexcept
    {
    ...
    void set_hw_info(const int new_hw) noexcept
    {
        hw = new_hw;
    }
    ...
    void set_is_pure(const bool new_pure) noexcept
    {
        is_pure = new_pure;
    }
    ...
    void set_is_allow_anonymous_players(const bool new_is_allow_anonymous_players) noexcept
    {
    ...
    void set_is_friendly_fire_enabled(const bool new_is_friendly_fire_enabled) noexcept
    {
    ...
    void set_is_password_protected(const bool new_is_password_protected) noexcept
    {
    ...
    vector<player> &get_temp_banned_players_data() noexcept
    {
        return temp_banned_ip_addresses_vector;
    }

    vector<player> &get_removed_temp_banned_players_vector() noexcept
    {
        return removed_temp_banned_players_data;
    }

    std::unordered_map<std::string, player> &get_removed_temp_banned_players_map() noexcept
    {
    ...
    bool add_ip_address_to_temp_banned_ip_addresses(const string &new_ip_address, const player &temp_banned_player_data)
    {
    ...
    bool remove_ip_address_from_temp_banned_ip_addresses(const string &new_ip_address)
    {
        if (!temp_banned_ip_addresses_map.contains(new_ip_address))
            return false;
    ...
    vector<player> &get_removed_banned_ip_addresses_vector() noexcept
    {
        return removed_banned_ip_addresses_vector;
    }

    std::unordered_map<std::string, player> &get_removed_banned_ip_addresses_map() noexcept
    {
    ...
    bool add_ip_address_to_banned_ip_addresses(const string &new_ip_address, const player &banned_player_data)
    {
    ...
    bool remove_ip_address_from_banned_ip_addresses(const string &new_ip_address)
    {
        if (!banned_ip_addresses_map.contains(new_ip_address))
            return false;
    ...
    vector<player> &get_removed_banned_ip_address_ranges_vector() noexcept
    {
        return removed_banned_ip_address_ranges_vector;
    }

    unordered_map<string, player> &get_removed_banned_ip_address_ranges_map() noexcept
    {
        return removed_banned_ip_address_ranges_map;
    }

    bool add_ip_address_range_to_banned_ip_address_ranges(const string &new_ip_address_range,
    ...
    bool remove_ip_address_range_from_banned_ip_address_ranges(const string &new_ip_address_range)
    {
        if (!banned_ip_address_ranges_map.contains(new_ip_address_range))
            return false;
  ...
  private:
    ...
    std::unordered_map<string, player> temp_banned_ip_addresses_map;
    std::unordered_map<string, player> removed_temp_banned_ip_addresses_map;
    std::unordered_map<string, player> banned_ip_addresses_map;
    ...
    std::string rcon_password{"abc123"};
    std::string private_slot_password{"abc123"};
    ...
    std::string current_match_info{"^3Map: {MAP_FULL_NAME} ^1({MAP_RCON_NAME}^1) ^3| Gametype: "
