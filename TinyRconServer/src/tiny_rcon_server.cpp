			...
			if (parts.size() >= 2 && !parts[0].empty() && !parts[1].empty()) {
				...
				if (size_t start, next; (start = cleaned_message.find("expires in ")) != string::npos && (next = cleaned_message.rfind("Date of ban:")) != string::npos) {
					...
					if (dot_pos != string::npos) {
						cleaned_message.erase(begin(cleaned_message) + start + stl::helper::len("expires in "), begin(cleaned_message) + dot_pos);
					}

					next = cleaned_message.rfind("Date of ban:");
					if (next != string::npos) {
						cleaned_message.erase(begin(cleaned_message) + next + stl::helper::len("Date of ban:"), end(cleaned_message));
					}
				}
				  ...
				  try {
					...
					if (!is_terminate_program.load() && is_refresh_players_data_event.load()) {
						string rcon_reply;
							...
							main_app.get_connection_manager().send_and_receive_rcon_data("status", rcon_reply, main_app.get_game_server().get_server_ip_address().c_str(), main_app.get_game_server().get_server_port(), main_app.get_game_server().get_rcon_password().c_str(), main_app.get_game_server(), true, true);
