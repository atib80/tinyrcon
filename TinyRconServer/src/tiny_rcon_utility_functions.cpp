	...
	const char* file_path)
{
	...
	config_file << "\"ftp_download_file_pattern\": \"" << escape_backward_slash_characters_in_place(main_app.get_ftp_download_file_pattern()) << "\",\n";
	...
	const size_t number_of_players_to_process)
{
	...
	for (size_t i{}; i < number_of_players_to_process; ++i)
	{
		max_player_name_length =
			std::max<size_t>(count_color_codes ? stl::helper::len(players[i].player_name) : get_number_of_characters_without_color_codes(players[i].player_name), max_player_name_length);
	}
...
void parse_tinyrcon_tool_config_file(const char* configFileName)
{
	...
	{
		main_app.get_game_server().set_check_for_banned_players_time_period(json_resource["check_for_banned_players_time_interval"].template get<int>());
	}
	else
	{
		found_missing_config_setting = true;
	...
	{
		main_app.get_game_server().set_is_disable_automatic_kick_messages(json_resource["disable_automatic_kick_messages"].template get<bool>());
	}
	else
	{
		found_missing_config_setting = true;
	...
	{
		main_app.get_game_server().set_is_use_original_admin_messages(json_resource["use_original_admin_messages"].template get<bool>());
	}
	else
	{
		found_missing_config_setting = true;
	...
	{
		found_missing_config_setting = true;
		main_app.get_game_server().set_current_match_info(
	...
	{
		main_app.get_game_server().set_is_automatic_country_kick_enabled(json_resource["is_automatic_country_kick_enabled"].template get<bool>());
	}
	else
	{
		found_missing_config_setting = true;
	...
	{
		main_app.get_game_server().set_is_automatic_city_kick_enabled(json_resource["is_automatic_city_kick_enabled"].template get<bool>());
	}
	else
	{
		found_missing_config_setting = true;
...
void display_temporarily_banned_ip_addresses(const size_t number_of_last_bans_to_display, const bool is_save_data_to_log_file)
{
	...
	else
	{
		...
		for (auto& bp : temp_banned_players)
		{
			...
			char buffer2[256];
			(void)snprintf(buffer2, std::size(buffer2), "%s, %s", (stl::helper::len(bp.country_name) != 0 ? bp.country_name : bp.region), bp.city);
...
void construct_tinyrcon_gui(HWND hWnd)
{
	...
	app_handles.hwnd_download_and_upload_speed_info = CreateWindowEx(0, RICHEDIT_CLASS, nullptr, WS_VISIBLE | WS_CHILD | ES_RIGHT | ES_READONLY, screen_width - 640, screen_height - 50, 630, 30, hWnd, nullptr, app_handles.hInstance, nullptr);
	...
	set_rich_edit_control_colors(app_handles.hwnd_download_and_upload_speed_info, color::white, color::black, "Lucida Console");
	print_colored_text(app_handles.hwnd_download_and_upload_speed_info, "^3Initializing and configuring ^5Tiny^6Rcon^3...", is_append_message_to_richedit_control::yes, is_log_message::no, is_log_datetime::no);
...
bool save_tinyrcon_statistics_data(const char* file_path)
{
	...
	config_file << "\"protected_ip_addresses\": " << main_app.get_tinyrcon_stats_data().get_no_of_protected_ip_addresses() << ",\n";
	config_file << "\"protected_ip_address_ranges\": " << main_app.get_tinyrcon_stats_data().get_no_of_protected_ip_address_ranges() << ",\n";
...
void parse_banned_names_file(const char* file_path, std::vector<player>& banned_names_vector, std::unordered_map<std::string, player>& banned_names_map, const bool is_skip_removed_check)
{
	string property_key, property_value;
	// lock_guard lg{ protect_banned_players_data };
	ifstream input_file{ file_path };
	...
	else
	{
		...
		while (getline(input_file, readData))
		{
			...
			if ((is_skip_removed_check || !removed_banned_names_map.contains(parts[2])) && !banned_names_map.contains(parts[2]))
			{
				player bannedPlayerData{};
				bannedPlayerData.ip_address = parts[0];
				strcpy_s(bannedPlayerData.guid_key, std::size(bannedPlayerData.guid_key), parts[1].c_str());
...
bool add_permanently_banned_player_name(player& pd, vector<player>& banned_players_names_vector, unordered_map<string, player>& banned_players_names_map)
{
	...
	if (main_app.get_game_server().get_removed_banned_names_map().contains(pd.player_name))
	{
		...
		entries.erase(std::remove_if(begin(entries), end(entries), [&pd](const player& p)
			{ return strcmp(p.player_name, pd.player_name) == 0; }),
			end(entries));
		save_banned_ip_entries_to_file(main_app.get_removed_banned_names_file_path(), main_app.get_game_server().get_removed_banned_names_vector());
	}
...
string prepare_current_match_information()
{
	...
	if (string::npos != start) {
		current_match_info.replace(start, len("{MAP_FULL_NAME}"), get_full_map_name(main_app.get_game_server().get_current_map(), convert_game_name_to_game_name_t(main_app.get_game_name())));
	}
	...
	if (string::npos != start) {
		current_match_info.replace(start, len("{GAMETYPE_FULL_NAME}"), get_full_gametype_name(main_app.get_game_server().get_current_game_type()));
	}

	start = current_match_info.find("{GAMETYPE_RCON_NAME}");
	if (string::npos != start) {
		current_match_info.replace(start, len("{GAMETYPE_RCON_NAME}"), main_app.get_game_server().get_current_game_type());
	}
	...
	if (string::npos != start) {
		current_match_info.replace(start, stl::helper::len("{OFFLINE_PLAYERS_COUNT}"), to_string(offline_players_count));
	}
	...
	std::thread process_topplayers_request_task{ [data = d]() {
		...
		if (parts.size() >= 2) {
		  ...
		  if (parts[0] == "!s") {
			...
			const string players_stats_information{ get_top_players_stats_data(main_app.get_stats_data().get_scores_vector(), main_app.get_stats_data().get_scores_map(), static_cast<int>(n), public_message, title.c_str(), parts[1]) };
			...
			if (!exact_player_stats_information.empty()) {
				...
				print_colored_text(app_handles.hwnd_re_messages_data, exact_player_stats_information.c_str(), is_append_message_to_richedit_control::yes, is_log_message::no, is_log_datetime::yes, true, true);
			  }
	  ...
	  string public_message;
	  const string players_stats_information{ get_top_players_stats_data(main_app.get_stats_data().get_scores_vector(), main_app.get_stats_data().get_scores_map(), static_cast<int>(n), public_message, title.c_str()) };
	  print_colored_text(app_handles.hwnd_re_messages_data, players_stats_information.c_str(), is_append_message_to_richedit_control::yes, is_log_message::no, is_log_datetime::yes, true, true);
	}
...
string public_message;
const string players_stats_information{ get_top_players_stats_data(main_app.get_stats_data().get_scores_for_year_vector(), main_app.get_stats_data().get_scores_for_year_map(), static_cast<int>(n), public_message, title.c_str()) };
print_colored_text(app_handles.hwnd_re_messages_data, players_stats_information.c_str(), is_append_message_to_richedit_control::yes, is_log_message::no, is_log_datetime::yes, true, true);
}
...
string public_message;
const string players_stats_information{ get_top_players_stats_data(main_app.get_stats_data().get_scores_for_month_vector(), main_app.get_stats_data().get_scores_for_month_map(), static_cast<int>(n), public_message, title.c_str()) };
print_colored_text(app_handles.hwnd_re_messages_data, players_stats_information.c_str(), is_append_message_to_richedit_control::yes, is_log_message::no, is_log_datetime::yes, true, true);
}
...
string public_message;
const string players_stats_information{ get_top_players_stats_data(main_app.get_stats_data().get_scores_for_day_vector(), main_app.get_stats_data().get_scores_for_day_map(), static_cast<int>(n), public_message, title.c_str()) };
print_colored_text(app_handles.hwnd_re_messages_data, players_stats_information.c_str(), is_append_message_to_richedit_control::yes, is_log_message::no, is_log_datetime::yes, true, true);
}
}
} };
...
std::string get_player_stats_information_message(const std::string& player_name)
{
	...
	if (stats_data_map.contains(player_name_index)) {
		...
		for (size_t i{}; i < stats_data_vector.size(); ++i) {
			if (player_name_index == stats_data_vector[i].index_name) {
				...
				// ^5Player stats for ^7Sensei ^5-> Rank: ^11 ^5| Score: ^1123213 ^5| SPM: ^12.45 ^5| ^5Time played: ^134 hrs. 12 min.
				oss << "^7Your stats data: ^5Rank: ";
				// const size_t longest_name_length{ get_number_of_characters_without_color_codes(pd.player_name) };
				// oss << text_element{ user.player_name, longest_name_length, "^7" } << " ^5-> Rank: ";
				snprintf(buffer, std::size(buffer), "%lu", i + 1);
				oss << "^1" << buffer << " ^5Score: ";
				snprintf(buffer, std::size(buffer), "%lld", user.score);
				oss << "^1" << buffer << " ^5SPM: ";
	...
	return string{};
	// return format("^5Could not find any relevant player stats information for player name: ^7{}^5!", player_name);
}
...
void tell_message(const char* message, const int pid)
{
	...
	for (const auto& line : lines) {
		...
		string reply_buffer;
		main_app.get_connection_manager().send_and_receive_rcon_data(buffer, reply_buffer, main_app.get_game_server().get_server_ip_address().c_str(), main_app.get_game_server().get_server_port(), main_app.get_game_server().get_rcon_password().c_str(), main_app.get_game_server(), false, false);
	}
}
