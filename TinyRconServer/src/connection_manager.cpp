	...
	while (true) {
		...
		if (noOfReceivedBytes > 0U) {
			...
			if (str_starts_with(udp_packet, string_view{ "getServersResponse\n", stl::helper::len("getServersResponse\n") }, true)) {
				...
				received_reply.append(udp_packet);
				if (stl::helper::str_starts_with(received_reply, string_view{ "infoResponse\n", stl::helper::len("infoResponse\n") }, true)) break;
			}
		}
	}
			...
			if (received_reply.find(rcon_status_response_needle1) != string::npos
				...
				if (!is_server_empty_or_error_receiving_udp_datagrams) {
					...
					for (const string& player_line : lines) {
						...
						convert_guid_key_to_country_name(main_app.get_geoip_data(), players_data[pl_index].ip_address, players_data[pl_index]);
