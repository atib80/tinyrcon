...
typedef struct tagINSTANCEDATA
{
    ...
    DWORD
    selectionMode;         ///< The Selection mode (full row, cell, or row header click)
    ...
    INT columntoresizeinitsize; ///< Field to hold the initial size of a column to
                                ///< resize
        ...
        if (EN_EDITENDOK == codeNotify)
        {
            ...
            if (nullptr != lpi)
            {
                ...
                GetWindowTextA(hwndCtl, edit_buffer,
                               std::size(edit_buffer)); // DWM 2.2.1: Replaced NELEMS with len + 1
                                                        // per suggestion of Hans-Peter Kalbf
...
static LRESULT Grid_OnInsertRow(HWND, WPARAM wParam, LPARAM lParam) // DWM 2.1: Added
{
    // Author: Ferenc Hechler
    int nPos = wParam + 1;
    BOOL fSuccess = FALSE;
