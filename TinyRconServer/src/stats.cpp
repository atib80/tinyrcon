...
extern time_t get_current_time_stamp();
...
void stats::update_player_score(const player &p, const int score_diff, const time_t time_diff,
                                const time_t current_timestamp)
{
    ...
    if (!new_player_stats_data)
    {
        ...
        size_t no_of_chars_to_copy{std::min<size_t>(p.player_name_index.length(),
                                                    std::size(scores_map.at(p.player_name_index).index_name) - 1)};
        strncpy_s(scores_map.at(p.player_name_index).index_name,
                  std::size(scores_map.at(p.player_name_index).index_name), p.player_name_index.c_str(),
                  no_of_chars_to_copy);
        ...
        strncpy_s(scores_map.at(p.player_name_index).ip_address,
                  std::size(scores_map.at(p.player_name_index).ip_address), p.ip_address.c_str(), no_of_chars_to_copy);
        scores_map.at(p.player_name_index).ip_address[no_of_chars_to_copy] = '\0';
        no_of_chars_to_copy = std::min<size_t>(stl::helper::len(p.player_name),
                                               std::size(scores_map.at(p.player_name_index).player_name) - 1);
        strncpy_s(scores_map.at(p.player_name_index).player_name,
                  std::size(scores_map.at(p.player_name_index).player_name), p.player_name, no_of_chars_to_copy);
    ...
    if (!new_player_stats_data)
    {
        ...
        size_t no_of_chars_to_copy{std::min<size_t>(
            p.player_name_index.length(), std::size(scores_for_year_map.at(p.player_name_index).index_name) - 1)};
        strncpy_s(scores_for_year_map.at(p.player_name_index).index_name,
                  std::size(scores_for_year_map.at(p.player_name_index).index_name), p.player_name_index.c_str(),
                  no_of_chars_to_copy);
        scores_for_year_map.at(p.player_name_index).index_name[no_of_chars_to_copy] = '\0';
        no_of_chars_to_copy = std::min<size_t>(p.ip_address.length(), 15u);
        strncpy_s(scores_for_year_map.at(p.player_name_index).ip_address,
                  std::size(scores_for_year_map.at(p.player_name_index).ip_address), p.ip_address.c_str(),
                  no_of_chars_to_copy);
        scores_for_year_map.at(p.player_name_index).ip_address[no_of_chars_to_copy] = '\0';
        no_of_chars_to_copy = std::min<size_t>(stl::helper::len(p.player_name),
                                               std::size(scores_for_year_map.at(p.player_name_index).player_name) - 1);
        strncpy_s(scores_for_year_map.at(p.player_name_index).player_name,
                  std::size(scores_for_year_map.at(p.player_name_index).player_name), p.player_name,
                  no_of_chars_to_copy);
    ...
    if (!new_player_stats_data)
    {
        ...
        size_t no_of_chars_to_copy{std::min<size_t>(
            p.player_name_index.length(), std::size(scores_for_month_map.at(p.player_name_index).index_name) - 1)};
        strncpy_s(scores_for_month_map.at(p.player_name_index).index_name,
                  std::size(scores_for_month_map.at(p.player_name_index).index_name), p.player_name_index.c_str(),
                  no_of_chars_to_copy);
        scores_for_month_map.at(p.player_name_index).index_name[no_of_chars_to_copy] = '\0';
        no_of_chars_to_copy = std::min<size_t>(p.ip_address.length(), 15u);
        strncpy_s(scores_for_month_map.at(p.player_name_index).ip_address,
                  std::size(scores_for_month_map.at(p.player_name_index).ip_address), p.ip_address.c_str(),
                  no_of_chars_to_copy);
        scores_for_month_map.at(p.player_name_index).ip_address[no_of_chars_to_copy] = '\0';
        no_of_chars_to_copy = std::min<size_t>(stl::helper::len(p.player_name),
                                               std::size(scores_for_month_map.at(p.player_name_index).player_name) - 1);
        strncpy_s(scores_for_month_map.at(p.player_name_index).player_name,
                  std::size(scores_for_month_map.at(p.player_name_index).player_name), p.player_name,
                  no_of_chars_to_copy);
    ...
    if (!new_player_stats_data)
    {
        ...
        size_t no_of_chars_to_copy{std::min<size_t>(
            p.player_name_index.length(), std::size(scores_for_day_map.at(p.player_name_index).index_name) - 1)};
        strncpy_s(scores_for_day_map.at(p.player_name_index).index_name,
                  std::size(scores_for_day_map.at(p.player_name_index).index_name), p.player_name_index.c_str(),
                  no_of_chars_to_copy);
        scores_for_day_map.at(p.player_name_index).index_name[no_of_chars_to_copy] = '\0';
        no_of_chars_to_copy = std::min<size_t>(p.ip_address.length(), 15u);
        strncpy_s(scores_for_day_map.at(p.player_name_index).ip_address,
                  std::size(scores_for_day_map.at(p.player_name_index).ip_address), p.ip_address.c_str(),
                  no_of_chars_to_copy);
        scores_for_day_map.at(p.player_name_index).ip_address[no_of_chars_to_copy] = '\0';
        no_of_chars_to_copy = std::min<size_t>(stl::helper::len(p.player_name),
                                               std::size(scores_for_day_map.at(p.player_name_index).player_name) - 1);
        strncpy_s(scores_for_day_map.at(p.player_name_index).player_name,
                  std::size(scores_for_day_map.at(p.player_name_index).player_name), p.player_name,
                  no_of_chars_to_copy);
