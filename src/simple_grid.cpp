...
//          (1) Added support for new message SG_INSERTROW provided by Ferenc Hechler
  ...
  while (row <= g_lpInst->bottomvisiblerow) {
    ...
    case GCT_BUTTON: {
      ...
      if (g_lpInst->ELLIPSIS) {
        ...
        print_colored_text_to_grid_cell(hdc, rect, buffer, DT_END_ELLIPSIS | DT_LEFT | DT_VCENTER | DT_CENTER | DT_SINGLELINE | DT_NOPREFIX);
      } else {
        // DrawTextEx(hdc, buffer, -1, &rect, DT_LEFT | DT_VCENTER | DT_WORDBREAK | DT_EDITCONTROL | DT_NOPREFIX, NULL);
        print_colored_text_to_grid_cell(hdc, rect, buffer, DT_LEFT | DT_VCENTER | DT_WORDBREAK | DT_EDITCONTROL | DT_NOPREFIX);
      }
    } break;
    case GCT_COMBO: {
      ...
      if (g_lpInst->ELLIPSIS) {
        ...
        print_colored_text_to_grid_cell(hdc, rect, buffer, DT_END_ELLIPSIS | DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX);
      } else {
        // DrawTextEx(hdc, buffer, -1, &rect, DT_LEFT | DT_VCENTER | DT_WORDBREAK | DT_EDITCONTROL | DT_NOPREFIX, NULL);
        print_colored_text_to_grid_cell(hdc, rect, buffer, DT_LEFT | DT_VCENTER | DT_WORDBREAK | DT_EDITCONTROL | DT_NOPREFIX);
      }
    ...
    case GCT_IMAGE: {
      ...
      if (g_lpInst->GRIDHASFOCUS) {
        if ((g_lpInst->HIGHLIGHTFULLROW && row == g_lpInst->cursorrow) || (col == g_lpInst->cursorcol && row == g_lpInst->cursorrow)) {
          style = ILD_SELECTED;
        }
      }
      ImageList_DrawEx(hImageList, index, hdc, rectImage.left, rectImage.top, WIDTH(rectImage), HEIGHT(rectImage), g_lpInst->clrBackground, CLR_DEFAULT, style | ILD_TRANSPARENT);
    } break;
    case GCT_LINK: {
      InflateRect(&rect, -2, -2);
...
static void Grid_OnKeyDown(HWND hwnd, UINT vk, BOOL, int cRepeat, UINT flags)
{
  ...
  default: {
    ...
    if ((VK_F1 <= vk && vk <= VK_F24) || VK_TAB == vk ||// DWM 1.7: Added
        VK_RETURN == vk || VK_MENU == vk || VK_PAUSE == vk || VK_CAPITAL == vk || VK_INSERT == vk || VK_NUMLOCK == vk || VK_LWIN == vk || VK_RWIN == vk || VK_APPS == vk) break;
    ...
    if (iRtn < (int)NELEMS(buf)) buf[iRtn] = 0;
...
static VOID Grid_OnLButtonDown(HWND hwnd, BOOL fDoubleClick, int x, int y, UINT keyFlags)
{
  ...
  if (GetFocus() == hwnd) {
    ...
    fClickSelect = GCT_BUTTON == GetColType(g_lpInst->cursorcol) || GCT_COMBO == GetColType(g_lpInst->cursorcol) || GCT_CHECK == GetColType(g_lpInst->cursorcol);

    // if not resizing or else over a link with hand cursor
    if ((fClickSelect && 1 == g_lpInst->cursortype) || ((GCT_LINK == GetColType(g_lpInst->cursorcol) && 3 == g_lpInst->cursortype))) {
...
static VOID Grid_OnPaint(HWND hwnd)
{
  ...
  if (hbmp) {
    BitBlt(hdcOrig, ps.rcPaint.left, ps.rcPaint.top, WIDTH(ps.rcPaint), HEIGHT(ps.rcPaint), hdc, ps.rcPaint.left, ps.rcPaint.top, SRCCOPY);
...
static LRESULT Grid_OnSetItemData(HWND hwnd, WPARAM, LPARAM lParam)
{
  ...
  case GCT_CHECK:
    ...
    break;
  case GCT_IMAGE:
    snprintf(buf, NELEMS(buf), "%ld", lpSGitem->lpCurValue);// DWM 1.9: was _stprintf
    lpszValue = buf;
