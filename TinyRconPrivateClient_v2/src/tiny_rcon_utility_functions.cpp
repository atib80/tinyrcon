...
int get_selected_players_pid_number(HWND edit_control)
{
  auto selected_line_number = static_cast<int>(SendMessage(edit_control, EM_LINEFROMCHAR, (WPARAM)-1, (LPARAM)0));
  // const string selected_player_line_number_message{ format("^7You have selected line no. ^1{}\n", selected_line_number) };
  // print_colored_text(app_handles.hwnd_re_messages_data, selected_player_line_number_message.c_str());
  if (selected_line_number < 4) return -1;
  selected_line_number -= 4;
  if (selected_line_number >= (int)main_app.get_current_game_server().get_number_of_players()) return -1;
  return main_app.get_current_game_server().get_players_data()[selected_line_number].pid;
}
  ...
  for (size_t i{}; i < main_app.get_current_game_server().get_number_of_players(); ++i) {
    if (pid == players_data[i].pid) {
      ...
      if (!p.banned_by_user_name.empty()) {
        result += format(" ^3{} by admin: ^7{}", action_by_admin_message, p.banned_by_user_name);// p.action_by_user_name (admin name)
      }
      return result;
    }
  }
...
void construct_tinyrcon_gui(HWND hWnd)
{
  ...
  app_handles.hwnd_players_grid = CreateWindowEx(0, RICHEDIT_CLASS, nullptr, WS_BORDER | WS_VISIBLE | WS_CHILD | WS_VSCROLL | WS_HSCROLL | ES_MULTILINE | ES_LEFT | ES_NOHIDESEL | ES_READONLY, 10, 85, screen_width / 2 + 135, screen_height - 210, hWnd, reinterpret_cast<HMENU>(ID_PLAYERS_RICHEDIT), app_handles.hInstance, nullptr);
  if (!app_handles.hwnd_players_grid)
    FatalAppExit(0, "Couldn't create 'hwnd_players_grid' richedit control!");

  if (app_handles.hwnd_players_grid != nullptr) {
    SendMessage(app_handles.hwnd_players_grid, EM_SETBKGNDCOLOR, NULL, color::black);
    scroll_to_beginning(app_handles.hwnd_players_grid);
    set_rich_edit_control_colors(app_handles.hwnd_players_grid, color::white, color::black, "Lucida Console");
    if (g_message_data_contents.empty())
      Edit_SetText(app_handles.hwnd_players_grid, "");
    else
      print_colored_text(app_handles.hwnd_players_grid, g_message_data_contents.c_str(), is_append_message_to_richedit_control::yes, is_log_message::yes, is_log_datetime::yes);
  }
...
void display_players_data_in_players_grid(HWND players_grid)
{
  ...
  display_online_players_data(players_grid, main_app.get_current_game_server().get_players_data(), main_app.get_current_game_server().get_number_of_players());

  /*size_t longest_name_length{ 12 };
  size_t longest_country_length{ 24 };
  auto &players_data = main_app.get_current_game_server().get_players_data();
  const size_t number_of_players{ main_app.get_current_game_server().get_number_of_players() };
  if (number_of_players > 0u) {
    longest_name_length = std::max(longest_name_length, find_longest_player_name_length(players_data.cbegin(), players_data.cbegin() + main_app.get_current_game_server().get_number_of_players(), false));
    longest_country_length =
      std::max(longest_country_length,
        find_longest_player_country_city_info_length(players_data, main_app.get_current_game_server().get_number_of_players()));
  }

  ostringstream oss;
  oss << get_date_and_time_for_time_t("^5{DD}.{MM}.{Y} {hh}:{mm}:");
  const string decoration_line(62 + longest_name_length + longest_country_length, '=');
  oss << "^5\n"
      << decoration_line << "\n"
      << "^5| ";
  oss << left << setw(4) << "PID"
  ...
  if (number_of_players == 0) {
    const size_t message_len = stl::helper::len("| There are no players online.");
    oss << "^5| ^3There are no players online.";
    ...
    for (size_t i{}; i < number_of_players; ++i) {
      auto &bp = players_data[i];
      const char *next_color{ is_first_color ? "^3" : "^5" };
      oss << "^5| ^1" << left << setw(4) << bp.pid << " ^5| ";
      oss << next_color << left << setw(5) << bp.score << " ^5| ";
      oss << next_color << left << setw(5) << bp.ping << " ^5| ";
      oss << next_color << left << setw(10) << bp.guid_key << " ^5| ^7";
      ...
      (void)snprintf(buffer, std::size(buffer), "%s, %s", (len(bp.country_name) != 0 ? bp.country_name : bp.region), bp.city);

      oss << next_color << left << setw(16) << bp.ip_address << " ^5| ";
      oss << next_color << left << setw(longest_country_length) << buffer << " ^5|\n";
      is_first_color = !is_first_color;
    }
  }
  ...
  const string online_players_text{ oss.str() };
  Edit_SetText(players_grid, "");
  print_colored_text(players_grid, online_players_text.c_str(), is_append_message_to_richedit_control::yes, is_log_message::no, is_log_datetime::no, true);
  SendMessage(players_grid, EM_SETSEL, (WPARAM)0, (LPARAM)-1);
  SendMessage(players_grid, EM_SETFONTSIZE, (WPARAM)2, (LPARAM)0);
  SendMessage(players_grid, EM_SETSEL, (WPARAM)-1, (LPARAM)0);*/
}
...
void update_game_server_setting(game_server &gs, std::string key, std::string value)
{
  ...
  if (key == "pswrd") {
    ...
    if (!is_rcon_game_server(gs)) gs.set_server_name(value);
...
bool show_and_process_tinyrcon_configuration_panel(const char *title)
{
  ...
  SetWindowText(app_handles.hwnd_server_name, main_app.get_game_servers()[0].get_server_name().c_str());
...
void display_online_players_data(HWND players_grid, std::vector<player> &players_data, const size_t number_of_players)
{
  size_t longest_name_length{ 12 };
  size_t longest_country_length{ 24 };
  if (number_of_players > 0u) {
...
LRESULT CALLBACK monitor_game_key_press_events(_In_ int nCode, _In_ WPARAM wParam, _In_ LPARAM lParam)
{
  ...
  case WM_KEYDOWN:
  case WM_KEYUP:
    if (pKeyBoard->vkCode == VK_ESCAPE && is_player_being_spectated.load()) {
